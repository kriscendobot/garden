...
| `spell-out-identifiers`       | No **added** line of a changed source file (`.js`/`.mjs`/`.cjs`/`.jsx`/`.ts`/`.tsx`) authors an identifier whose camelCase/PascalCase/snake segment is on the curated abbreviation blocklist (`dir`, `cmd`, `temp`, `tmp`, `arg`, `args`, `subdir`, `cfg`, `ctx`, `idx`, `msg`, `btn`, `impl`, `mgr`, `num`, `str`, `val`, `resp`, `req`, `addr`, `db`). Strings and comments are stripped first; matching is on **whole segments**, so `directory`/`interval`/`mkdtempSync`/`tmpdir` never match. **Non-auto-fixable** (a rename touches every call site): fails with one line per distinct (file, identifier, abbreviation). Per-file escape hatch: a `spell-out-exempt` marker in the first five lines. | `endojs/endo-but-for-bots#650` (`dir`→`directory`, `makeTempRoot`→`makeTemporaryRoot`) + `#609` review `4a711718` (`makeIntervalSchedulerCmd`→`makeIntervalScheduler`, "Avoid abbreviations… It isn't making a command") + the #684 recurrence (`Addr`→`Address`). Cluster `review-misses/clusters/avoid-name-abbreviations.md`. |
| `prefer-endo-primitives`      | No **added** JavaScript/TypeScript line matches a nameless hand-rolled-primitive idiom from [`skills/build-vs-buy/idioms.tsv`](../build-vs-buy/idioms.tsv) when the same file does not import its audited Endo package: Node/WebCrypto SHA-256 (`@endo/sha256`), direct `TextEncoder`/`TextDecoder` construction (`@endo/bytes`), `atob`/`btoa` or Buffer base64 conversion (`@endo/base64`), a byte-to-hex `toString(16).padStart(2, ...)` loop (`@endo/hex`), `Uint8Array.from(... charCodeAt(...))` ASCII conversion (`@endo/ascii`), an `insist*` declaration (`@endo/errors`), or a `let resolve`/`let reject` deferred (`@endo/promise-kit`). It also fails on any added bare `Far(...)` call, recommending `makeExo` with an interface guard from `@endo/exo`; importing a package does not waive that one. Since 2026-09-24 it is a thin idiom-only shim over the build-vs-buy detector; a local `makePromiseKit` (or any copy of a distinctively named export) is the `build-vs-buy` probe's job. **Non-auto-fixable:** import and use the named package. Per-file escape hatch: `prefer-endo-primitives-exempt` in the first five lines. | Consolidated 2026-08-04 review retrospective, cluster `prefer-endo-primitives`: six misses across PRs 671, 755, 824, 836, 877, and 882. |
| `build-vs-buy`                | No **added** declaration re-authors a function another workspace package exports. The probe indexes the export surface at `--base-ref` (`scripts/jobs/export-index/ensure-export-index.sh`, a per-commit cache) and fails each unwaived **strong** name hit: a distinctive name exported by exactly one reachable package whose body matches the export's AST shape (or node-type Jaccard ≥ 0.6, or an idiom row names it). The message is `file:line name -> import { export } from 'specifier' (def path:line)`. Weak and dependency-cycle-blocked hits stay silent (the `procurer` seat judges them); a missing index passes with a note. **Non-auto-fixable:** import the export. Waivers: `// build-not-buy: <reason>` above the declaration; `build-vs-buy-exempt` in a file's first five lines. | kriskowal on `endojs/endo-but-for-bots#1336` (review `5307103246`, a copied `makePromiseKit`); design `designs/export-index-build-vs-buy.md`. Rule: [build-vs-buy](../build-vs-buy/SKILL.md). |
| `no-plain-reexport`           | No **newly-introduced** plain re-export (`export … from '…'` — named, renamed, `default as`, `export *`, `export * as ns`) in a changed source file lacks an immediately-preceding `@deprecated` JSDoc shim. A cheap grep for added `export` words gates a full **Babel** parse (the self-contained parser vendored in `skills/re-export-deprecation-policy/`, since the garden does not `npm install` at run time) that takes the set difference of qualified re-exports (after − before), so only a re-export the change *introduces* is flagged. Barrels / `index.js` are **not** exempt; type-only re-exports (`export type …`, `.d.ts`) are skipped. **Non-auto-fixable** (complying needs importer migration + a chosen `@deprecated` message pointing at the canonical original). Per-file escape hatch: `reexport-policy-exempt` in the first five lines. Doubles as the `reexport-auditor` jury seat's deterministic pre-pass. | @erights on `endojs/endo-but-for-bots#475` (follow the re-export policy and prevent future violations); design `designs/reexport-deprecation-policy-gauntlet.md`, approved PR #95. Rule: [re-export-deprecation-policy](../re-export-deprecation-policy/SKILL.md). |
| `no-nul-bytes`                | No changed text file carries a literal NUL byte (U+0000). A single NUL makes git treat the file as **binary**, so `git diff` and GitHub render only "Binary files differ" and suppress the line-level diff — hiding the file's substantive change from review. **Whole-file** (a NUL leaves no added-line ranges to scope to), across the staged/unstaged/`--base-ref` diff. **Non-auto-fixable:** spell the character with a textual escape (`\0`, `\x00`, `String.fromCharCode(0)`, `Buffer.from([0])`, …). Skips genuinely-binary paths: a curated known-binary extension denylist (images, archives, fonts, media, compiled objects), `references/`/`node_modules/`, and any file the repo declares binary via a gitattributes `-diff`/`binary` attribute (the per-file escape hatch for an intentionally-binary fixture with a text-shaped name). | A test file that embedded literal NULs went binary and hid its substantive diff from GitHub review; the fix requires textual escapes. |
...
### Why there is no broad `@typedef` occurrence probe

A lexical scanner can deterministically report that `@typedef` occurs, but it cannot decide the convention's material question: whether the declaration is exported, reused, describes a package data shape, or is instead a legitimate module-private, single-use type. Failing every occurrence would block valid local typedefs; printing every occurrence as a warning would add noise without enforcing a result. The garden therefore does not ship a broad occurrence scanner. `typedefs-belong-in-dts` mechanizes only the unambiguous whole-file shape, while the always-on typist seat judges substantial typedefs in mixed implementation files. This boundary implements the scanner request from `endojs/endo-but-for-bots#388` review `4413566645`, comment `3344477615`, without turning a context-dependent style rule into a false-positive gate.
...
- _2026-07-05_: added `typedefs-belong-in-dts` probe (`scripts/jobs/gardening/pre-push-gates/probes/typedefs-belong-in-dts.sh`) — the tier-1 deterministic gate for the convention first raised on `endojs/endo-but-for-bots#388` review `4413566645`, comments `3344377619` and `3344477615`, then repeated on `#58` review `4612637233` and `#442` review `4629047816`. It fails when a changed `**/src/**/*.js` file is a **types-only module**: a character scanner strips `//` and `/* */` (incl. JSDoc) comments while skipping string literals, and the file fires only when it declares `@typedef`/`@callback` yet has no runtime code left beyond the empty `export {}` marker. It is **narrow on purpose** (§ Pitfalls): an implementation `.js` with any runtime code never matches, so a module-private single-use inline `@typedef` is left alone; the whole-file `types.js` shape is the only thing it catches. The builder/fixer directives and always-on typist seat own the contextual cases.
...
- _2026-09-24_: added the `build-vs-buy` probe and turned `prefer-endo-primitives` into an idiom-only shim over one detector (`skills/build-vs-buy/detect.cjs`, design `designs/export-index-build-vs-buy.md`). The general mechanism is an export-name index built per base commit from every workspace package's `exports` with the vendored Babel parser; a local declaration that shares a distinctive name and body shape with an indexed export fails pre-push. The narrow signature catalog moved to data (`skills/build-vs-buy/idioms.tsv`), and its `makePromiseKit` declaration regex was deleted because the name pass covers it. Rule going forward: a duplicated *named* export needs no catalog entry; only a nameless inline idiom earns an `idioms.tsv` row. Regression coverage: `scripts/jobs/test/build-vs-buy-probe-test.sh` (including a replay of PR #1336 at `a0cc8ba5` against its base index), `scripts/jobs/test/export-index-test.sh`, and `scripts/jobs/test/review-convention-probes-test.sh`.
- _2026-09-24_: from kriskowal's review of `endojs/endo-but-for-bots#1336` (review 5307103246). `spell-out-identifiers` now includes `args` -> `arguments` (the PR added `argsShape`/`normalizeArgs`; the existing `arg` entry only matches the singular segment). A bare `args` binding cannot become `arguments`, which strict mode forbids as a binding name, so pick a descriptive name such as `argumentList` there. `prefer-endo-primitives` now fails on a copied promise kit (a local `makePromiseKit`, or a `let resolve`/`let reject` deferred) unless the file imports `@endo/promise-kit`, and on any added bare `Far(` call, recommending `makeExo`. Both appeared in the PR's test files. The maintainer said the gauntlet should have caught the `Far`. The `C-locksmith` panel hint already routes on `Far(`, but a draft build runs no gauntlet, so the check now lives at pre-push. Regression coverage: `scripts/jobs/test/review-convention-probes-test.sh`.
- _2026-09-23_: `spell-out-identifiers` now includes `db` -> `database`, closing the PR 1329 miss. `endojs/endo-but-for-bots#1329` added `openTestDb`/`db` (segment `db`) to `packages/daemon/test/registry-endo.test.js`; the panelled `stylist` seat let them through because `db` was on neither this blocklist nor the stylist brief's abbreviation example list, and the identifiers mirror an established `openTestDb`/`db` convention in the sibling `endo.test.js` — the surrounding-consistency principle argued *for* keeping them. The maintainer asked to "expand the abbreviation to Database" for newly-added identifiers regardless of local convention (comment `endojs/endo-but-for-bots#1329#issuecomment-5785807820`). The stylist brief was clarified to say the never-abbreviate check fires on freshly-authored identifiers even when they copy an abbreviated pattern from the surrounding file. Caveat: a product/platform name whose uppercase run spells `DB` (`IndexedDB`, `LevelDB`, `MongoDB`) now fires as a segment `db` — a rare false positive in this daemon codebase (its own store is `manager-database`), waivable via `spell-out-exempt` per the probe's bias-toward-firing. Regression coverage: `scripts/jobs/test/review-convention-probes-test.sh`.
- _2026-09-23_: added `no-plain-reexport` probe (`scripts/jobs/gardening/pre-push-gates/probes/no-plain-reexport.sh`) — the author-time preventive for the garden's re-export deprecation policy ([re-export-deprecation-policy](../re-export-deprecation-policy/SKILL.md)). @erights asked on `endojs/endo-but-for-bots#475` that the garden follow the re-export policy and prevent every future violation; the design `designs/reexport-deprecation-policy-gauntlet.md` (approved PR #95, five open questions resolved) specifies a three-stage detector: a cheap grep for added `export` words gates a full Babel parse taking the set difference of qualified re-exports (after − before) so only newly-introduced ones are flagged; the `reexport-auditor` jury seat is the LLM stage. Because the garden does not `npm install` at run time, the skill **vendors** a self-contained `@babel/parser` bundle and a parse helper (`reexport-parse.cjs`); the probe is one detector reused as the seat's pre-pass (`check`/`report` subcommands). Barrels are in scope (Decision 1); type-only re-exports and `.d.ts` are skipped (Decision 5); the `@deprecated`-adjacency rule is Decision 2; per-file `reexport-policy-exempt` marker. Regression coverage: `scripts/jobs/test/no-plain-reexport-probe-test.sh` and `scripts/jobs/test/seat-gate-reexport-auditor-test.sh`.
- _2026-09-12_: added `no-nul-bytes` probe (`scripts/jobs/gardening/pre-push-gates/probes/no-nul-bytes.sh`). A literal NUL byte makes git treat a file as binary, so `git diff` and GitHub's review UI render only "Binary files differ" and suppress the line-level diff — a NUL smuggled into an otherwise-textual file (a test file, precipitatingly) hides that file's substantive change from human review. The probe fails when a changed text file carries any NUL, whole-file (a NUL leaves no added-line ranges to scope to), across the staged, unstaged, and `--base-ref` committed diffs; the content is streamed straight into the detector so the NUL is never stripped by a shell variable. Non-auto-fixable: the author must spell the character with a textual escape (`\0`, `\x00`, `String.fromCharCode(0)`). It skips genuinely-binary paths — a curated known-binary extension denylist, `references/`/`node_modules/`, and any file the repo declares binary via a gitattributes `-diff`/`binary` attribute (the principled per-file escape hatch). Regression coverage in `scripts/jobs/test/pre-push-gates-test.sh`: a staged NUL and a committed `--base-ref` NUL are both rejected with the textual-escape suggestion, and a known-binary asset (`.png`) passes untouched.
