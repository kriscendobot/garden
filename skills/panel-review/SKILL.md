...
- **Default for source-touching PRs (code panel): thirty seats** (assessor, typist, stylist, packager, archivist, prover, curator, migrator, locksmith, warden, saboteur, breaker, purist, spec-keeper, wire-watcher, engine-realist, integrator, duality-auditor, benchmarker, changeset-auditor, surfacer, scribe, pruner, gateway, corner-prober, fast-checker, releaser, transplanter, coverage-auditor, orthographer). The `coverage-auditor` and the `orthographer` are mandatory seats but **cost-gated at dispatch**: each runs a deterministic pre-pass first and spends a `claude -p` only when there is something to judge — the c8 coverage pre-pass on uncovered new lines (`scripts/jobs/gardening/seat-gate-coverage-auditor.sh`), and the divergence grep against the curated word list on introduced British spellings (`scripts/jobs/gardening/seat-gate-orthographer.sh`). The `orthographer` sits on the **design panel** too (all documents).
...
1. **Garden-only prose conventions downgrade to `drop`.** Findings citing [em-dash-style](../em-dash-style/SKILL.md), [no-latin-shorthand](../no-latin-shorthand/SKILL.md), or [american-english-normalization](../american-english-normalization/SKILL.md) (all scoped to *garden-authored* prose per their own `## Scope`) downgrade to `drop` with the rationale "garden convention; not in scope on external-author PR". External contributors follow the project's own house rules (`CONTRIBUTING.md`, existing `designs/`, prior accepted PRs' prose style). The `orthographer` seat still runs and still reports on an external-author PR (the audit trail shows what was seen); the drop happens here, at aggregation.
