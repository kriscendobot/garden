...
- _2026-06-26_: the conductor's "block until CI terminal, then merge in the SAME job" need is packaged as the deterministic `scripts/jobs/gardening/ci-wait-merge.sh` (this skill's one-tick rollup logic wrapped in a real timeout/backoff loop, merging on green / reporting on red / re-enqueuing on timeout). It exists because a merge job that only ticks this skill and then exits can strand a green-but-unmerged PR if the re-invoke never fires — "waiting for CI" is not terminal (endo-but-for-bots #178, bit twice). Drive long merge-waits through that script, not a bare per-tick loop that may end mid-wait.
