...
There are **two default panels**: the code panel (twelve seats, for source-touching PRs) and the design panel (seven seats, for design-only PRs). The judge picks one per round based on the PR's file list per `roles/judge/AGENT.md` § Panel-kind discrimination.
