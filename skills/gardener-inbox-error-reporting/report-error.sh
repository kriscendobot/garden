...
## driver lane $LANE -- $STATE failure at $ISO
...
Inspect via \`git -C journal cat-file blob $TRANSCRIPT_SHA\`.
EOF
...
if ! git -C "$GARDEN_JOURNAL" diff --cached --quiet 2>/dev/null; then
  git -C "$GARDEN_JOURNAL" commit -m "inboxes(gardener): error from driver lane $LANE state $STATE" >/dev/null 2>&1 || true
  for i in 1 2 3; do
    if git -C "$GARDEN_JOURNAL" push --quiet origin HEAD:journal 2>/dev/null; then
      break
    fi
    git -C "$GARDEN_JOURNAL" fetch --quiet origin journal 2>/dev/null || true
    git -C "$GARDEN_JOURNAL" rebase origin/journal >/dev/null 2>&1 || {
