#!/bin/bash
# C-purist — fires on intrinsic-shaping and hand-rolled Endo primitive signals.
set -uo pipefail
BASE=${BASE:-origin/master}
PATTERN="defineProperty\\([^)]*enumerable|Object\\.freeze\\(|\\bharden\\b|\\bpassStyleOf\\b|\\bRemotable\\w*\\b|createHash[[:space:]]*\\([[:space:]]*['\"]sha-?256|subtle\\.digest[[:space:]]*\\([[:space:]]*['\"]SHA-?256|new[[:space:]]+Text(Encoder|Decoder)[[:space:]]*\\(|(^|[^[:alnum:]_\$])(atob|btoa)[[:space:]]*\\(|\\.toString[[:space:]]*\\([[:space:]]*16[[:space:]]*\\)[[:space:]]*\\.padStart[[:space:]]*\\([[:space:]]*2|Uint8Array\\.from.*charCodeAt[[:space:]]*\\(|\\b(const|let|var|function)[[:space:]]+insist[A-Z_a-z0-9\$]*|base64::|BASE64_(STANDARD|URL_SAFE)"
