...
How to ferry a completed garden-side pull request to its upstream governance repository. Covers the three procedure shapes a ferry takes (first-time, re-ferry with recompute, fast-forward append), the attribution rewrite that swaps bot authors for the named human, the trailer-strip and body-edit disciplines, and the scope boundary that separates the ferry's job from a rebase/conflict-resolution step.

Canonical for the **ferry** job: carry a PR upstream (the v1 "boatman" function). A ferry is a job a gardener claims off the board ([job-board](../job-board/SKILL.md)); it requires the credentialed host plus an identity switch (see § Preconditions). The skill assumes [pr-formation](../pr-formation/SKILL.md) for the upstream PR's title and body discipline; this skill covers the git-level mechanics around the push.

## When to use

- A `ferry #N` job is claimed (the triager maps a "ferry #N" / "carry #N upstream" PR-comment directive to a ferry job; the gardener claims and runs it). The verb names this procedure.
...
Not for: master-merge conflict resolution (a rebase / [conflict-resolution](../conflict-resolution/SKILL.md) gardening step), title-only updates on an existing upstream PR without a content change (out of scope; see § Scope boundary).

## Preconditions

- `gh auth status` shows `kriskowal` as the active identity on this host. Without kriskowal credentials, stop and message the liaison via the [message-bus](../message-bus/SKILL.md); the bot identity must not push to a primary upstream repo. A ferry must be issued from the host that holds the kriskowal credentials; a gardener that finds itself on the wrong host stops at this precondition check and surfaces the gap rather than pushing under the bot identity.
- The job carries `identity_switch_authorized: true`. Every other gardening step commits under the bot identity; only the ferry overrides the identity pin at commit time.
- The job names the source PR (`<fork-owner>/<repo>#<n>` and source branch), the upstream repo (`<owner>/<repo>` and target branch), and the human author identity (`Kris Kowal <kris@cixar.com>` by convention).
...
The choice between shapes is driven by the relationship between source-PR state, upstream-PR state (if any), and upstream master tip. Diagnose the shape from the job's parameters and the current state of both PRs before starting.
...
Source commits typically carry the bot author (e.g. `endolinbot <main.barn5084@fastmail.com>`) or mixed bot/human attributions. The job is to rewrite every commit's author and committer to the named human, typically `Kris Kowal <kris@cixar.com>`.
...
The local-config plus `--amend --reset-author --no-edit` is the canonical pattern, reaffirmed across every multi-commit ferry.
...
3. The committer becomes the ferry's local kriskowal identity from the amend. The asymmetry (author preserved, committer is kriskowal) is correct and standard.
4. Verify each commit's author individually against the job's per-commit author table.

The job for a multi-author ferry should provide a per-commit author table. If it does not, ask the liaison via the [message-bus](../message-bus/SKILL.md) before guessing; do not infer authorship from commit history alone (the source may already have flattened the attribution).
...
This is a **standing** discipline. Run it on every ferry, regardless of whether a prior inspection claimed the bodies were clean. A #73 ferry surfaced a case where a preliminary inspection (eyeballing the first 20 lines of each body via `gh api`) missed a `Co-Authored-By: Claude Opus 4.7 (1M context) <noreply@anthropic.com>` trailer that lived below the first paragraph. The standing `interpret-trailers --parse` caught it; without that discipline, the trailer would have shipped upstream.
...
The general PR-body discipline lives in [pr-formation](../pr-formation/SKILL.md). The ferry applies that skill and adds these ferry-specific rules to the PR body:
...
The "frame for the upstream audience" rule for title and body voice is part of the same discipline; this skill captures the specific edits the ferry's content drives.

## Identity discipline

- The push to upstream happens under kriskowal credentials, gated by `identity_switch_authorized: true` on the job. Verify `gh auth status` shows kriskowal as active before pushing. If it does not, stop and message the liaison via the [message-bus](../message-bus/SKILL.md); the host is wrong.
- Comments on **primary upstream repos** (`endojs/endo`, `agoric/agoric-sdk`) under the kriskowal identity are **forbidden** by the standing identity-discipline rule. The ferry routes any upstream explanatory comment through a [message-bus](../message-bus/SKILL.md) message to the liaison; a bot-identity comment is posted on the garden's own repo instead, or the maintainer posts upstream.
- Comments on the **garden's own repo** (`endojs/endo-but-for-bots`) can be posted directly under whichever identity is authenticated on the host. The standing repo authorization permits both kriskowal and kriscendobot. The job should say "post under whichever identity is authenticated" rather than presuming the host's setup; on the credentialed host only kriskowal is authenticated, and jobs that said "under the bot identity" produced surfaced misframings.

## Branch naming

- **First-time ferry.** Pick a sensible default: `kriskowal-<topic>` or `<scope>-<topic>` mirroring the source's convention.
...
The ferry's responsibility ends at "the upstream head matches the source's content". Two adjacent concerns are explicitly **out of scope**:

1. **Master-merge conflict resolution.** A `MERGEABLE: CONFLICTING` status on the upstream PR after a ferry is a rebase / [conflict-resolution](../conflict-resolution/SKILL.md) gardening step, not the ferry's. The ferry surfaces the status in its completion report but does not attempt to rebase the upstream branch onto current master.
2. **Title and description updates on the upstream PR.** Default is "leave the existing title and body unchanged". The ferry edits title or body only when:
   - The job explicitly asks for it ("update the title and description").
...
When in doubt, leave the upstream PR's title and body alone and surface the question in the completion report.

## No-op handling

When the job asks to ferry a PR but the source and upstream are at the same head (`gh compare` reports `ahead: 0, behind: 0, files_changed: 0`), record the no-op in the job's completion report rather than spinning up the full ferry mechanics. The source-side auto-sync pattern (the bot rebases its source PR onto the ferry's rewritten history, bringing the two heads into byte-for-byte agreement) is common enough that this case appears regularly.

If the ferry discovers the no-op state during its own pre-flight inspection (e.g., the job was posted before the auto-sync converged), complete the job naming the no-op state and exit without pushing. Do not invent a no-op commit to "register" the ferry.

## Verification checklist

Run these in order before completing the ferry job. The checklist is the executable form of "the upstream head matches the source's content with clean attribution".

1. **Identity.** `gh auth status` shows kriskowal active.
2. **Authorization.** The job carries `identity_switch_authorized: true`.
...
6. **Shape-specific.**
   ...
   - Shape 2 (recompute): force-push landed; approval state recorded in completion report.
...
7. **CI status.** `gh pr checks <n> -R <upstream>` recorded in the completion report (pending / passing / failing). The ferry does not wait for CI to finish before completing; CI-driven follow-up is a separate shepherd gardening step. See [pr-ci-watch](../pr-ci-watch/SKILL.md).
8. **Garden-side cross-link comment.** The garden-side PR carries exactly one tagged cross-link comment in the canonical shape `Mirror of <upstream-PR-URL> (head <short-SHA>).` Post (or PATCH the existing one on a re-ferry) under the authenticated identity on the garden repo. Find the existing one via `gh api repos/<owner>/<name>/issues/<N>/comments --jq '.[] | select(.user.login == "kriscendobot" and (.body | startswith("Mirror of ")))'`; if found, edit in place (`gh api -X PATCH /repos/.../issues/comments/<id> -f body=...`); else create.
...
- _2026-05-15_: the precipitating bug for the trailer-strip discipline being standing-rather-than-conditional was the #73 ferry, where a "preliminary inspection shows clean commit bodies" framing was wrong because the inspector eyeballed only the first 20 lines of each body. The standing `interpret-trailers --parse` caught a `Co-Authored-By: Claude` trailer further down. Standing discipline means "every ferry, every commit, always", not "when it might be needed".
- _2026-06-02_: the precipitating case for the verify-tracking-ref precondition was the #387 re-ferry (endo-but-for-bots#387 -> endo#3294). `worktrees/endojs-endo.git`'s `origin/master` sat stale across four ferries because the bare clone's `remote.origin.fetch` was empty, so plain `git fetch origin` only advanced FETCH_HEAD. `git ls-remote` showed the live tip differed; forcing `git fetch origin +refs/heads/master:refs/remotes/origin/master` corrected the ref. The root cause is fixed at bare-clone creation; this precondition is the defense in depth for clones made before that step existed.
- _2026-06-24_: migrated from v1. The boatman role and its liaison/steward dispatch became the **ferry** job a gardener claims off the [job-board](../job-board/SKILL.md); the steward-side comment-relay became a [message-bus](../message-bus/SKILL.md) hop to the liaison. The credentialed-host + identity-switch discipline is preserved verbatim.
