  ...
  for base in "${STALE[@]}"; do
    ...
    if [ "$should_park" -eq 1 ]; then
      ...
      if [ "$outage" -ne 1 ] && [ "$count" -ge "${GARDEN_KIMI_FALLBACK_AFTER:-1}" ]; then
        # Respect the role's canonical tier FLOOR. reroute_job_model refuses (rc 2)
        # to demote below the floor role_tier_floor names, so a designer/builder job
        # is never dropped to a tier that cannot design or build (the
        # proposal-compartments-xs designer doom, 2026-08-17). On a refusal the job
        # requeues UNCHANGED at its floor tier and, if it keeps failing there, dooms
        # with an accurate tier for a human to triage rather than after several
        # wasted impossible-tier cycles.
        #
        # There is no longer an anthropic ceiling-suppression case here: under the
        # opus55-tier Option B (2026-09-23) an automatic mentor job is served AT
        # mentor (claude-opus-5-5) by an anthropic worker too, so a mentor failure is
        # genuine evidence about mentor and the ordinary role-floor reroute applies to
