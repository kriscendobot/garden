...
so `reputation-reduce.sh` instead prices that event at `duration_secs x` the rate
below. `duration_secs` is present on **100% of reputation events** and is measured
by the garden, not reported by a provider, so it is never censored.
...
from the only arms that carry a real dollar ledger: the 79 `reputation/events/*.md`
whose `agentic_dollars` is a number rather than `censored`, each divided into its own
`duration_secs`. That is a direct measurement of the quantity this card exists to
...
they arrived first: the measured autonomous rate is $0.0072/s, ~39x the anchor. The
