...
BP_BASE=$'claude-endolin1\tanthropic\tweekly-tokens\t143000000\tmanual-fit\t2026-09-01\n'
...
rc="$(run_setter claude-oros 100000000 manual-fit 2026-09-17)"
resync
if [ "$rc" = 2 ] && ! bp_has claude-oros && [ "$(lv_row oros-studio-garden-ce242c49)" = ABSENT ]; then
  ok "missing physical-cap row rejects promotion and lands neither file"
else
  bad "expected reject with no mutation (rc=$rc, lv=$(lv_row oros-studio-garden-ce242c49))"
fi

# 2. Same, but --monk-cap (and --cleric-cap) supplied: atomic upsert of both files.
seed_state "$BP_BASE" "$LV_BASE"
rc="$(run_setter claude-oros 100000000 manual-fit 2026-09-17 --monk-cap 3 --cleric-cap 1)"
resync
last_files="$(git -C "$WORK" show --name-only --format= HEAD)"
if [ "$rc" = 0 ] && bp_has claude-oros && [ "$(lv_row oros-studio-garden-ce242c49)" = "3 1" ] \
  ...
  bad "atomic upsert failed (rc=$rc, lv=$(lv_row oros-studio-garden-ce242c49), files=$(tr '\n' ',' <<<"$last_files"))"
fi

# 3. Host row already present and valid, no --monk-cap: validate-only, row untouched.
seed_state "$BP_BASE" "$LV_BASE"
rc="$(run_setter claude-endolin1 150000000 manual-fit 2026-09-17)"
resync
if [ "$rc" = 0 ] && [ "$(lv_row endolin-garden-ece02cb4)" = "4 4" ] && bp_has claude-endolin1; then
  ok "an existing valid physical-cap row is validated and left unchanged"
else
  bad "validate-only path mutated or failed (rc=$rc, lv=$(lv_row endolin-garden-ece02cb4))"
fi

# 3b. --monk-cap overrides an existing row (update path).
seed_state "$BP_BASE" "$LV_BASE"
rc="$(run_setter claude-endolin1 150000000 manual-fit 2026-09-17 --monk-cap 5)"
resync
if [ "$rc" = 0 ] && [ "$(lv_row endolin-garden-ece02cb4)" = "5 4" ]; then
  ok "--monk-cap updates an existing physical-cap row, preserving the cleric cap"
else
  bad "update path failed (rc=$rc, lv=$(lv_row endolin-garden-ece02cb4))"
fi
...
rc="$(run_setter claude-oros 100000000 manual-fit 2026-09-17 --monk-cap 2)"
resync
if [ "$rc" = 2 ] && [ "$(lv_row oros-studio-garden-ce242c49)" = ABSENT ] && ! bp_has claude-oros; then
...
rc="$(run_setter claude-oros 100000000 manual-fit 2026-09-17 --monk-cap 4)"
resync
if [ "$rc" = 2 ] && [ "$(lv_row oros-studio-garden-ce242c49)" = ABSENT ]; then
...
rc="$(run_setter claude-oros 100000000 manual-fit 2026-09-17)"
resync
if [ "$rc" = 0 ] && bp_has claude-oros && [ ! -f "$WORK/config/worker-leveling" ]; then
...
seed_state "$BP_BASE" EMPTY
rc="$(run_setter claude-oros 100000000 manual-fit 2026-09-17 --monk-cap 3)"
resync
if [ "$rc" = 2 ] && ! bp_has claude-oros && [ ! -f "$WORK/config/worker-leveling" ]; then
...
seed_state "$BP_BASE" "$LV_BASE"
rc="$(run_setter claude-oros 5000000 placeholder)"
resync
if [ "$rc" = 0 ] && bp_has claude-oros && [ "$(lv_row oros-studio-garden-ce242c49)" = ABSENT ]; then
...
seed_state "$BP_BASE" "$LV_BASE"
rc="$(run_setter codex-endolin 100 manual-fit 2026-09-17 --kind percent)"
resync
rc2="$(run_setter claude-oros - temporary-key 2026-09-17 --kind unmetered)"
resync
if [ "$rc" = 0 ] && bp_has codex-endolin && [ "$rc2" = 0 ] && bp_has claude-oros; then
