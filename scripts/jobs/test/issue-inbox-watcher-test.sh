...
# ----------------------------------------------------------------------------
# REAP2 — a straggler that forks AFTER a zero-read must still be felled: ONE zero-read
...
hr; echo "REAP2 — straggler forked after the first zero-read is still felled"; hr
...
LATE_SERVER=$!
BARE_REAP2="$TR/reap2.git"; seed_bare "$BARE_REAP2"
FIX_REAP2="$TR/fix-reap2.tsv"; : > "$FIX_REAP2"
GARDEN_ISSUE_CGROUP_PROCS_FILE="$LATE_PROCS" run_watcher "$TR/state-reap2" "$BARE_REAP2" "$FIX_REAP2" \
  "$TR/post-reap2.log" "$TR/msg-reap2.log" "$TR/err-reap2.log" || true
