...
[ "$(model_dispatch_tier anthropic claude-opus-5-5)" = mentor ] && ok "Opus 5.5 is mentor" || bad "Opus 5.5 tier"
...
[ "$(tier_model_for_provider mentor anthropic)" = claude-opus-5-5 ] && ok "mentor selects Opus 5.5 for Anthropic (first-match ceiling)" || bad "mentor Opus 5.5 selection"
...
[ "$(resolve_model_tier anthropic opus55)" = claude-opus-5-5 ] && ok "opus55 alias binds Opus 5.5" || bad "opus55 binding"
[ "$(resolve_model_tier anthropic opus5)" = claude-opus-5 ] && ok "opus5 alias still binds Opus 5" || bad "opus5 binding"
...
[ "$(model_dispatch_tier openai gpt-6-astra)" = mentat ] && ok "GPT-6 Astra is mentat" || bad "GPT-6 Astra tier"
[ "$(tier_model_for_provider mentat openai)" = gpt-6-astra ] && ok "mentat selects GPT-6 Astra for OpenAI (first non-Anthropic mentat)" || bad "mentat OpenAI selection"
[ "$(tier_model_for_provider mentat anthropic)" = claude-fable-5 ] && ok "mentat still selects Fable for Anthropic (unaffected)" || bad "mentat Anthropic selection"
[ -z "$(tier_model_for_provider mentat fireworks)" ] && ok "mentat has no Fireworks model (provider without a mentat row fails closed)" || bad "mentat Fireworks boundary"
