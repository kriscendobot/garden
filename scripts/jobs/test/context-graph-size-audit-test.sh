...
mkdir -p "$MAIN/roles/test" "$MAIN/context/topic" "$JOURNAL/projects/demo" \
  "$JOURNAL/library/sections" "$JOURNAL/periodicals"

printf '%s\n' '# Main' '[Extra](extra.md)' >"$MAIN/CLAUDE.md"
...
grep -Fq 'Walked 14 reachable documents and flagged 1 reorganization candidates.' "$REPORT"
