...
# ============================================================================
hr; echo "R — cgroup straggler sweep waits until the cgroup is EMPTY (not one-shot)"; hr
# Two stragglers in their OWN sessions stand in for gh-forked git helpers that escaped
# the source's process group; the test-only GARDEN_DEPENDABOT_CGROUP_PROCS_FILE fixture
# lists them, and the EXIT-path sweep must have felled BOTH by the time the watcher
# returns.
...
FIX_R="$TR/fix-r.tsv"; : > "$FIX_R"
GARDEN_DEPENDABOT_CGROUP_PROCS_FILE="$CGPROCS" run_dep "$TR/state-r" "$BARE_R" "$FIX_R" || true
