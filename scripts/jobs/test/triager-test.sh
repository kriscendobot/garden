...
# now BOUNDED (timeout + retry).  Once that retry budget is exhausted, ANY failure
# from a present clone alerts the maintainer and degrades to a clean skip-this-tick:
# connectivity and remote configuration faults alike can be repaired or retried on a
# later tick, and neither should crash-loop the unit. We drive each case with a scoped
# `git` shim (real for everything except `git ... fetch`), reproducing a failure
...
# M1 — TRANSIENT stderr signature (Connection timed out, rc 1): alert and skip.
...
grep -Fq "WARN: triager: fetch failed for $SLUG (unreachable/offline?); skipping this tick, retry next" "$MOUT" && ok "fetch skip logs the stable WARN (retry next tick)" || bad "fetch WARN missing (out: $(cat "$MOUT"))"
...
[ "$(wc -l < "$ALERTS")" -eq 1 ] && grep -Fq "triager-fetch-failed-$SLUG" "$ALERTS" && ok "transient fetch failure alerts with the per-slug throttle key" || bad "fetch alert missing or wrong (out: $(cat "$ALERTS"))"
...
grep -Fq "WARN: triager: fetch failed for $SLUG (unreachable/offline?); skipping this tick, retry next" "$MOUT" && ok "rc-124 skip logs the stable WARN" || bad "rc-124 WARN missing (out: $(cat "$MOUT"))"
...
# M3 — STRUCTURAL fetch error: alert and skip too; a present clone's failed refresh
# must never make the unit fail.
...
env PATH="$TR/fetch-shim-structural:$PATH" GIT_FETCH_CALLS="$FETCH_CALLS" \
    ...
    "$JOBS/triager.sh" "$SLUG" >>"$MOUT" 2>&1
rc=$?; set -e
[ "$rc" -eq 0 ] && ok "structural fetch error skips instead of failing the unit" || bad "tick exit = $rc (want 0 for a present-clone fetch failure)"
[ "$(wc -l < "$FETCH_CALLS")" -eq 2 ] && ok "structural error retried through GARDEN_FETCH_RETRIES before skipping" || bad "fetch attempts = $(wc -l < "$FETCH_CALLS") (want 2)"
grep -Fq "WARN: triager: fetch failed for $SLUG (unreachable/offline?); skipping this tick, retry next" "$MOUT" && ok "structural failure logs the stable WARN" || bad "structural WARN missing (out: $(cat "$MOUT"))"
[ "$(wc -l < "$ALERTS")" -eq 1 ] && grep -Fq "triager-fetch-failed-$SLUG" "$ALERTS" && ok "structural fetch failure alerts with the per-slug throttle key" || bad "structural fetch alert missing or wrong (out: $(cat "$ALERTS"))"
! grep -q "FATAL: fetch failed for $SLUG" "$MOUT" && ok "no FATAL on a structural fetch failure" || bad "structural failure still died FATAL (out: $(cat "$MOUT"))"
