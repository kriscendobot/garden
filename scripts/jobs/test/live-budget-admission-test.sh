...
serr="$(snaperr claude-oros)"
[ "$(snaprc claude-oros)" = 0 ] && ok "window-only drift trusts the publisher's window (rc 0)" || bad "window-only drift rc=$(snaprc claude-oros), expected 0"
snapout="$(bash -c 'set -uo pipefail; source "$1/common.sh"; meter_now() { echo 100000; }; meter_remote_snapshot_total "$2" claude-oros 500 9000 2>/dev/null' _ "$JOBS" "$SD")"
[ "$snapout" = 42 ] && ok "window-only drift still yields the snapshot spend" || bad "window-only drift spend=$snapout, expected 42"
[ "$serr" = "snapshot window trusted file=claude-oros/h1 window=8000/9000" ] && ok "window-only drift notes both windows on stderr" || bad "window drift note: $serr"
printf 'subscription: claude-oros\ncap: 999\nwindow_start_epoch: 8000\nspend: 42\nsampled_at_epoch: 99990\n' > "$SD/budget/live/claude-oros/h1"
serr="$(snaperr claude-oros)"
[ "$(snaprc claude-oros)" = 9 ] && [[ "$serr" == *"diverged=cap,window "* ]] && ok "window drift with cap drift stays rc 9, naming both" || bad "cap+window drift: rc=$(snaprc claude-oros) $serr"
printf 'subscription: claude-oros\ncap: 500\nwindow_start_epoch: 08\nspend: 42\nsampled_at_epoch: 99990\n' > "$SD/budget/live/claude-oros/h1"
[ "$(snaprc claude-oros)" = 9 ] && ok "malformed window reads rc 9" || bad "malformed window rc=$(snaprc claude-oros), expected 9"
