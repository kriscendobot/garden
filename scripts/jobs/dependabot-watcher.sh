...
write_full_body() {  # write_full_body <pr> [<pkg> <old> <new>]
  ...
  printf 'through the conductor deterministic spine (maintainer-approval gate intact);\n'
...
write_superseded_body() {  # write_superseded_body <pr> <pkg> <old> <new> <peer-pr>
  ...
  printf '    clear, and the successor is blocked only on a maintainer approval, that\n'
