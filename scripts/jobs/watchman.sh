...
#
# The aggressive checkout only fast-forwards a CLEAN worktree; a dirty main2 is
# left alone with a warning (we never clobber local edits). Disable with
...
if [ "$up_sha" != "$local_sha" ] && [ "$GARDEN_AGGRESSIVE_CHECKOUT" = "1" ]; then
  if git -C "$GARDEN_ROOT" merge-base --is-ancestor "$local_sha" "$up_sha" 2>/dev/null; then
    if [ -z "$(git -C "$GARDEN_ROOT" status --porcelain 2>/dev/null)" ]; then
      if git -C "$GARDEN_ROOT" merge --ff-only "origin/$GARDEN_MAIN_BRANCH" >/dev/null 2>&1; then
        ...
        log "WARN: ff-merge to origin/$GARDEN_MAIN_BRANCH failed"
      fi
    else
      log "WARN: $GARDEN_MAIN_BRANCH worktree dirty; skipping aggressive checkout"
    fi
  fi
fi
