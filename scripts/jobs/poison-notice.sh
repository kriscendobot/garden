#!/bin/bash
# poison-notice.sh — the reaper's amend-or-post-KEYED maintainer notice for a
# poisoned job. A sibling of inbox-send.sh, addressed to the maintainer inbox.
#
# Usage: poison-notice.sh <base> <signature> [body-file]
#   <base>        the poisoned job's basename (the spine).
#   <signature>   a NORMALIZED failure signature for the poison event. The reaper
...
# maintainer message per poison event — 37 near-identical POISON reports in one
# morning. This helper collapses that flood: it keeps a KEYED record of the open
# POISON notice for a given job+condition and AMENDS it (bumps an occurrence count,
...
# (inbox/maintainer/unread/poison-<base>-<signature>.md), so amend-or-post is a
...
# it), the unread file is gone, so the next poison of the same job posts a FRESH
...
# the keyed `poison_signature`/`poison_base` live in the message frontmatter so a
# re-poison can find the file and bump the count deterministically. Posting is
...
GARDEN_TAG="poison-notice"

base="${1:?usage: poison-notice.sh <base> <signature> [body-file]}"
signature="${2:?usage: poison-notice.sh <base> <signature> [body-file]}"
body_src="${3:-}"
...
key="$(printf 'poison-%s-%s' "$base" "$signature" | tr -c 'A-Za-z0-9._-' '-')"
case "$key" in -*|'') die "illegal poison key derived from '$base'/'$signature'";; esac
...
else BODY="(no poison detail supplied)"; fi
...
# bumped count. The keyed fields (poison_base/poison_signature) let a later amend
...
compose() {  # compose <count> <first_seen>
  ...
  printf -- '---\n'
  if [ "$count" -gt 1 ]; then
    printf 'POISON notice — occurrence #%s (first seen %s, latest %s).\n' \
      "$count" "$first_seen" "$now"
    printf 'This job has been poison-parked %s times for the same condition (%s);\n' \
      "$count" "$signature"
...
for attempt in $(seq 1 "${GARDEN_POST_ATTEMPTS:-50}"); do
  ...
  if [ -e "$DIR/$REL" ]; then
    ...
    git -C "$DIR" add "$REL"
    if commit_and_push "$DIR" "poison-notice($key) amended → #$count by $GARDEN"; then
      log "amended maintainer poison notice '$key' (occurrence #$count)"; exit 0
    fi
  else
    ...
    git -C "$DIR" add "$REL"
    if commit_and_push "$DIR" "poison-notice($key) posted by $GARDEN"; then
      log "posted fresh maintainer poison notice '$key'"; exit 0
    fi
  fi
  log "poison-notice '$key' lost a push race (attempt $attempt); re-syncing"
  backoff "$attempt"
done
die "could not deliver poison notice '$key' after retries"
