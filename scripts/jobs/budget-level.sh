...
snapshot="$({
 ...
 while IFS=$'\t ' read -r pool provider third fourth fifth sixth _seventh _; do
   ...
   if [ -r "$mapping" ]; then
     ...
     # Rolling-deploy compatibility: former rows carried the one owning host in
     # column three.
     host="$third"; printf 'P\t%s\t%s\t%s\t%s\n' "$pool" "$host" "$cap" "${prov:-}"
   fi
 done <"$pools"
...
if [ "$mv" -eq 1 ];then
 ...
 for((i=0;i<n;i++));do
  ...
  if [[ "$allocation_cutoff" =~ ^[0-9]+$ ]];then
   allocation_spend="$(meter_remote_snapshot_total "$DIR" "$pool" "$cap" "$allocation_cutoff" 2>/dev/null || true)"
   if ! [[ "$allocation_spend" =~ ^[0-9]+$ ]]&&[ "$h" = "$GARDEN" ];then allocation_spend="$(meter_subscription_window_total "$pool" "$DIR" 2>/dev/null || true)";fi
  fi
  if [[ "$allocation_spend" =~ ^[0-9]+$ ]];then
   allocation_bias="$(subscription_pacing_bias "$pool" "$allocation_spend" "$cap" "$DIR")"
   # Up to 20x weight near reset: with three pools and a six-monk envelope this
   # can move all three discretionary slots to a high-slack subscription (1->4)
   # without violating the fleet ceiling.
   weight="$(subscription_allocation_weight "$cap" "$allocation_bias")"
  fi
...
if [ "$cv" -eq 1 ];then
 ...
 codex_status="$(meter_quota_status codex-endolin "$DIR")"
 for h in "${eligible[@]}";do cur="$(read_desired_count "$DIR/hosts/$h" clerics 2>/dev/null)"||{ pool_failure codex-endolin "$h" read-host-clerics "$?";continue;};target="${ctarget[$h]}";[ "$codex_status" != backoff ]||target="$GARDEN_BUDGET_LEVEL_MIN";apply_target codex-endolin "$h" cleric "$cur" "$target" "shared codex subscription demand active=$A queue=$Q quota=$codex_status fleet-envelope=$cf target=$target" "$Q" "$cf" config/worker-leveling eligible-queue-demand;done
