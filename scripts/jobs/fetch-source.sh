...
#      sandbox (recurring connection-refused / timeout), so the fetch must fall
...
#   2. if the direct fetch fails to produce bytes (the recurring case is a
#      connection refusal / timeout from the sandbox), asks the Wayback
...
#   3. writes the fetched bytes (to the given output path, else a temp file) and
...
#   source_effective_url=<the URL actually fetched (direct or the id_ form)>
#   source_fetched_via=direct|wayback
...
#
# EXIT CODES
#   0  bytes fetched (direct or via the archive fallback) and hashed
#   1  both the direct fetch and the archive fallback failed to produce bytes
#   2  usage error
#
...
#
# This script makes network calls (a direct fetch, then possibly the Wayback
# availability API and an archive fetch). It writes ONLY to the output path (or a
# temp file) and never to the journal or any garden tree.
...
if _curl "$url" && [ -s "$out" ]; then
  ...
  rc=$?
  log "direct fetch failed (curl rc=${rc:-?}); falling back to the Internet Archive"
  : >"$out"   # discard any partial body the failed direct fetch left behind

  # --- 2a. ask the Wayback availability API for the closest capture ----------
  ...
  # --- 2b. fetch the ORIGINAL bytes via the id_ form -------------------------
