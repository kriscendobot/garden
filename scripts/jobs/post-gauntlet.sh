...
#                    [--max-iterations N] [--max-stage-retries N]
#                    [--by ROLE] <g> <pr-url>
#
...
#   --max-stage-retries N  give-up bound on retryable stage-job deaths (default 2).
#                      Separate from max_iterations and max_resumes: this is for a
#                      stage job that died, not a panel/fix round or a CI wait.
...
Usage:
  post-gauntlet.sh [--build-job <base>] [--probe] [--kind feature|probe]
                   [--max-iterations N] [--max-resumes N]
                   [--max-stage-retries N] [--by ROLE] <g> <pr-url>
...
case "$max_stage_retries" in
  ''|*[!0-9]*) die "illegal --max-stage-retries: '$max_stage_retries' (want a non-negative integer)";;
esac
...
for attempt in $(seq 1 "${GARDEN_POST_ATTEMPTS:-50}"); do
  ...
  if commit_and_push "$DIR" "gauntlet($base) recorded [$kind, pr #$pr_number] by $GARDEN"; then
    log "recorded gauntlet '$base' ($kind, $repo#$pr_number, max_iterations=$max_iterations, max_resumes=$max_resumes, max_stage_retries=$max_stage_retries)"
    exit 0
  fi
