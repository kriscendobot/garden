...
# rc and skip cleanly on ANY nonzero rc — mirroring triager.sh (b320648e47). The
# temporary-unavailable rc (GARDEN_OFFLINE_RC) is cursor-get's shared journal-outage
# latch: skip QUIETLY, since the host already owns one cooldown warning and re-warning
# per repo per tick is the herd this latch exists to suppress. Any OTHER nonzero rc is
# a loud structural/authentication failure and still WARNs.
...
BRANCH_OP_VERBS="rebase retcon refresh shepherd conduct merge gauntlet americanize deslop"
...
classify() {  # classify <body-file> <surface> <author>; sets VERB (+PRIMARY_VERB); rc 0=verb 2=ambiguous 1=none
  ...
  if [ -z "$detected_verb" ] && { [ -n "$imperative" ] || [ -n "$mentions_bot" ]; }; then
    local v
    for v in rebase retcon refresh shepherd americanize deslop; do
...
write_job_body() {  # write_job_body <out> <verb> <surface> <author> <pr> <url> <body-file> [primary-verb] [comment-id]
  ...
  fi
  {
    ...
    if [ "$verb" = attention ]; then
      printf '%s\n%s\n%s\n\n' '---' 'handler-budget-role: review' '---'
    elif [ "$verb" = americanize ] || [ "$verb" = deslop ]; then
      # The americanizer/deslopper run the deterministic apply-then-re-grep loop over a
      # vetted candidate digest — the seat baked in the judgment, so what's left is
      # applying the vetted spelling swap / cliché rewrite — so both ride the expedient
      # `myrmidon` tier (skills/model-selection/SKILL.md). The grep search-gate is
      # enforced by the claiming gardener (which has a project worktree), so a clean grep
      # is a cheap no-op, never speculative work.
    ...
    case "$verb" in
      rebase|retcon|refresh|shepherd|gauntlet|pinbase|americanize|deslop) ;;
      *) preflight_instruction "$pr" "$cid" "$author" ;;
    esac
  } > "$out"
}
...
run_source() {
  ...
  if command -v timeout >/dev/null 2>&1; then
    ...
    "${sid[@]}" timeout --signal=TERM --kill-after="$GARDEN_COMMENT_KILL_AFTER" "${GARDEN_COMMENT_SOURCE_TIMEOUT_SECS}s" \
