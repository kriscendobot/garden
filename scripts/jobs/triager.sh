...
if [ "$fetch_rc" -ne 0 ]; then
  # A present clone can fail to refresh for reasons outside the triager's control.
  # Leave its cursor untouched and retry next tick instead of failing the unit.
  # alert_maintainer is throttled, so a persistent outage remains visible without
  # creating an alert storm.
  pmsg="triager: fetch failed for $slug (unreachable/offline?); skipping this tick, retry next"
  log "WARN: $pmsg"
  alert_maintainer "triager-fetch-failed-${slug//[^A-Za-z0-9._-]/_}" "$pmsg"
  exit 0
fi
