...
per-feed-watcher daemons. The maintainer who prefers a script over
remembering unit names runs these; the systemd unit files themselves
live at [`scripts/systemd/`](../systemd/).
...
| `logs.sh`             | tail the systemd journal for the units; `--feed` filters       |
| `config.sh.example`   | template for the host-local config (which feeds)               |
...
```sh
# Edit the host-local config to declare which feeds you want.
cp scripts/daemons/config.sh.example scripts/daemons/config.sh
$EDITOR scripts/daemons/config.sh
...
```sh
# A single feed watcher is misbehaving; restart it.
systemctl --user restart garden-watcher@endo-but-for-bots.service
...
# Disable a feed permanently.
scripts/daemons/stop.sh --disable-too
# ... then remove the feed slug from config.sh and re-run start.sh.
```
...
- **`systemctl: command not found`.** The host is not running
  ...
  no-op with a message in that case; the watcher scripts still work
  when invoked by hand (`scripts/watcher/<feed>/watcher.sh`).
- **`config.sh: file not found`.** Copy `config.sh.example`.
  Without `config.sh`, `start.sh` and `stop.sh` exit cleanly with
  a message.
- **Unit failed: see `systemctl --user status garden-watcher@<feed>`.**
  The watcher's `EXIT` trap appended a section to
...
- A health-check loop. The watcher itself escalates to the
  gardener inbox on unexpected exit; the daemons scripts only
  report on systemd's view of unit state.
- A way to launch a watcher outside of systemd. For ad-hoc
  invocation use `scripts/watcher/<feed>/watcher.sh` directly.
