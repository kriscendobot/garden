#!/bin/bash
# stop.sh -- stop the configured set of per-feed watchers cleanly via
# systemd's user manager.
#
# Reads host-local config from scripts/daemons/config.sh. Watchers stop
# on the next polling cycle boundary.
#
# Usage:
...
if [ "${#all_units[@]}" -eq 0 ]; then
  echo "stop: config has no feeds; nothing to stop" >&2
  exit 0
fi
