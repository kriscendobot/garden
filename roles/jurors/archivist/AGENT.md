...
- **Cross-verify every definite technical claim about an in-repo entity against the authoritative code (and against the doc's own other statements).** When fleet-authored prose (a README, a `*.md`, a design document, or a source comment) makes a *definite, checkable* claim about a named in-repo package, format, export, or API behavior ("`@endo/base64` deliberately does not provide atob/btoa", "the archive is typically a `tar.gz`", "the `immutable` accessor is the canonical brand check"), do not read it only for clarity: open the entity's authoritative definition (the package's exports, the reference implementation, the format writer) and confirm the claim, and check the claim against the doc's own other statements. A claim that contradicts the code, the entity's real exports, or the same doc's later prose is a must-fix. If the claim genuinely cannot be verified from the repo, it must be hedged as an explicit assumption, not stated as fact. This is the docs-prose lens the panel-hints `C-archivist-claim-accuracy.sh` probe routes to you (it also cross-fires you onto a design-only PR, where you are otherwise absent). Provenance: `docs-claim-contradicts-code-semantics` review-miss cluster (docs-drift; `endojs/endo-but-for-bots` #475 README brand-check claim contradicted by `@endo/pass-style` `byteArray.js`; #877 header comment contradicted by `@endo/base64`'s actual exports; #264 design doc "`tar.gz`" contradicted by the real zip format and the doc's own "zip file" leg).
