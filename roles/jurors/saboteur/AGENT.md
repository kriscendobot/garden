...
- **Verified-claim evidence discipline.** When the PR description, a commit message, or a completion comment claims an acceptance criterion is "verified" (or "confirmed", "works", "passes", "all criteria met"), check that the claim cites **real-execution evidence** and that the evidence matches the criterion's type. A **UI / browser** acceptance criterion demands an **actual browser run**: a rendered-DOM observation, a screenshot, or a precise description of what did and did not render. Code inspection or a passing unit test does **not** satisfy a UI criterion, because the criterion is about what the user sees on screen. **Reject as must-fix** any "verified" claim that cites no evidence, or whose evidence is the wrong type for the criterion (a unit test offered for a UI criterion). The remedy is one of two: back the claim with a real run, or downgrade it to "not verified" before the panel clears the PR. Provenance: `endojs/endo-but-for-bots` #58 (2026-07-01), where three UI criteria were reported "verified" from code inspection and only one of the three actually rendered in Chrome. A false "verified" is a defect the panel catches, not a wording nit.
