...
The code-panel seat that reads for **knowledge-capture closure** and **PR-communication closure**: every maintainer ask in the PR's history to "note this in standing orders", "leave a record for future builders", or "add this to CLAUDE.md" has produced either a standing-orders edit in the PR's diff or a `message: panel → gardener` proposing the edit; and every push that responded to a maintainer directive or review closed the loop with a top-level summary comment, not inline replies alone.
...
- **Completion-summary closure.** A second, communication-side surface alongside the note-this asks. For each maintainer directive or review on the PR that drew a responding push (a fixer addressing review, a weaver rebasing, a shepherd driving CI green, a builder pushing follow-up work), check that the doer posted a **top-level summary comment** after the push (`.../issues/<N>/comments`), not inline thread replies alone. The summary should name the head SHA, what changed, anything declined and why, and the verification status, per [pr-completion-summary-comment](../../../skills/pr-completion-summary-comment/SKILL.md). **Open** when a responding push landed but no top-level summary comment followed it (inline-only replies, or a silent push). Raise a finding naming the directive, the responding SHA(s), and the missing summary. Disposition default: `summary-fix` (post the summary this round). This is the gap PR #474 surfaced (2026-06-25): the fixer posted the inline replies and pushed the fix but no after-the-fact summary, and the maintainer expected one. Skip the check on repos where commenting is unauthorized (the summary belongs in the completion report there, not on the PR).
- **The scribe does not write the notes.** Closure is the calling role's job (a fixer claiming a `summary-fix` job will land the standing-orders edit or post the missing summary). The scribe's lens is whether the closure exists, not whether to author the closure.
