...
- **Runtime-flag attenuation and unhardened exported capabilities are the second recurring locksmith finding.** Flag an attenuator that gates authority behind a runtime flag (`if (readOnly) { ... }`) where the denied authority could instead be **structurally absent** — a read-only facet should not close over the write capability at all (POLA: code that cannot write should statically not hold write authority). Flag an exported client / exo object whose surface is unhardened or carries no interface guards (`M.interface(...)`), and a module reaching for ambient authority (`setTimeout`, unguarded globals) that implies it expects to run unconfined. The idiomatic shape is a mereology builder (`whole.part('A')`, cf `@agoric/pola-io`). Provenance: `dckc` on `endojs/endo-but-for-bots` #874 (no interface guards, unhardened surface) and #881 (`if (readOnly)` attenuation and `setTimeout` in a facet).
  - **`Far(tag, methods)` on a boundary-facing attenuation is itself the flag — recommend `makeExo(tag, InterfaceGuard, methods)`.** A capability exposed across a trust boundary to a **less-trusted** holder (a guest-facing attenuation, a client-facing facet) must carry an explicit InterfaceGuard, so the exo — not each method body — rejects a hostile caller's malformed / extra / wrong-typed arguments at the boundary. Bare `Far` gives passability + `harden` but **no** interface guard; `harden`/`Far` is *not* a substitute for a guard (do not read `Far(...)` as "properly guarded"). When you see `Far('Foo', { ... })` returned to a lower-authority holder with no `M.interface`/`M.call` shape, the finding is: replace with `makeExo('Foo', FooI, { ... })` and name the guard. This holds even when the `Far` sits inside an `eval`/worker **source string** — the attenuation string is still the boundary object. Provenance: `kriskowal` on `endojs/endo-but-for-bots` #1125 (`readOnlyDirectorySource`'s `Far('ReadableNameHub', …)` guest-facing read-only hub; a brief gauntlet of the natural-catcher seats missed it — the warden even endorsed the bare `Far` as "properly hardened").
