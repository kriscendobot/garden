...
- **`harden` / `Far` is not an interface guard — never endorse bare `Far` as "properly hardened" and stop there.** `Far(tag, methods)` and `harden(...)` deliver passability and deep-freeze; they do **not** reject a hostile caller's malformed arguments the way an InterfaceGuard does. For a boundary-facing object handed to a *less-trusted* holder, "hardened" is necessary but not sufficient — the guard-vs-no-guard question is the locksmith's to press (`makeExo(tag, InterfaceGuard, methods)` over bare `Far`), and your harden-discipline pass must not paper over its absence by pronouncing the object safe. Provenance: `endojs/endo-but-for-bots` #1125 (the warden seat endorsed `Far('ReadableNameHub', …)` as "properly hardened via `Far`", masking the missing interface guard).
