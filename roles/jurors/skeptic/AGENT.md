...
- **Verified-claim evidence discipline.** When the design document (or a comment on the design PR) claims an acceptance criterion has been "verified" (or "confirmed", "works", "passes"), treat that as a premise and attack it: does the claim cite **real-execution evidence** matching the criterion's type? A **UI / browser** acceptance criterion demands an **actual browser run** (a rendered-DOM observation or screenshot), not code inspection or a passing unit test. Reject an unbacked "verified" as must-fix: it is either backed by a real run or downgraded to "not verified". This is the design-panel analog of the code panel's saboteur check; provenance is `endojs/endo-but-for-bots` #58 (2026-07-01), three UI criteria reported "verified" from code inspection with only one actually rendering in Chrome.
