...
- **Abbreviated identifiers** in freshly-authored names. The maintainer standing-rejects plainly-abbreviated identifiers and asks, review after review, that they be spelled out in full — **even when the abbreviation is unambiguous**. Flag any freshly-authored function, variable, parameter, or file name whose word or camelCase/PascalCase/snake segment is a bare abbreviation: `dir`→`directory`, `Cmd`→`Command`, `Temp`/`tmp`→`Temporary`, `Arg`→`argument`, `subDir`→`subdirectory`, `ctx`→`context`, `cfg`→`configuration`, `idx`→`index`, `msg`→`message`, `req`/`resp`→`request`/`response`, and the like. Established platform/domain names stay as the platform spells them (`os.tmpdir()`, `fs.mkdtempSync`, `WeakMap`, `URL`) — those are not findings. This is a **mechanical never-abbreviate check**, not a judgment call; the deterministic [pre-push-gates](../../../skills/pre-push-gates/SKILL.md) `spell-out-identifiers` probe catches it on gated pushes, and this seat is the backstop for PRs that skip the gate (maintainer-authored, or a promoted probe). Cite `file:line` and the spelled-out replacement; must-fix. Provenance: PR `endojs/endo-but-for-bots#650` (`dir`, `makeTempRoot`) and `#609` (`makeIntervalSchedulerCmd`, "Avoid abbreviations… It isn't making a command"); cluster `review-misses/clusters/avoid-name-abbreviations.md`.
