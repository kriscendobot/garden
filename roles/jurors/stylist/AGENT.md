...
- **Abbreviated identifiers** in freshly-authored names. The maintainer standing-rejects plainly-abbreviated identifiers and asks, review after review, that they be spelled out in full — **even when the abbreviation is unambiguous**. Flag any freshly-authored function, variable, parameter, or file name whose word or camelCase/PascalCase/snake segment is a bare abbreviation: `dir`→`directory`, `Cmd`→`Command`, `Temp`/`tmp`→`Temporary`, `Arg`→`argument`, `subDir`→`subdirectory`, `ctx`→`context`, `cfg`→`configuration`, `idx`→`index`, `msg`→`message`, `req`/`resp`→`request`/`response`, `db`/`Db`→`database`/`Database` (e.g. `openTestDb`→`openTestDatabase`), and the like. Established platform/domain names stay as the platform spells them (`os.tmpdir()`, `fs.mkdtempSync`, `WeakMap`, `URL`) — those are not findings. This is a **mechanical never-abbreviate check**, not a judgment call; the deterministic [pre-push-gates](../../../skills/pre-push-gates/SKILL.md) `spell-out-identifiers` probe catches it on gated pushes, and this seat is the backstop for PRs that skip the gate (maintainer-authored, or a promoted probe). **Local convention does not exempt a fresh abbreviation.** The never-abbreviate check fires on a freshly-authored identifier *even when it copies an abbreviated pattern already present in the surrounding file* — an existing `db`/`openTestDb` in a sibling test is not a license to add more; the maintainer wants new identifiers spelled out regardless. (This is the one place the "consistency with surrounding code" principle above yields: consistency argues for matching existing *spelled-out* names, never for propagating an existing abbreviation.) Cite `file:line` and the spelled-out replacement; must-fix. Provenance: PR `endojs/endo-but-for-bots#650` (`dir`, `makeTempRoot`), `#609` (`makeIntervalSchedulerCmd`, "Avoid abbreviations… It isn't making a command"), and `#1329` (`openTestDb`/`db`, "expand the abbreviation to Database" — comment `#issuecomment-5785807820`, the miss that added `db` to the check); cluster `review-misses/clusters/avoid-name-abbreviations.md`.
