...
- [typist-friendly-code-points](../../../skills/typist-friendly-code-points/SKILL.md): the hard-to-type code-point rule this seat backstops on the code panel.
...
- **Typist-hostile code points in the changed text.** Per [typist-friendly-code-points](../../../skills/typist-friendly-code-points/SKILL.md), flag any added line (code comments, markdown, doc prose in the diff) that carries a symbol or punctuation code point that is difficult for a typist to produce: arrows (`→` U+2192 and kin), `…`, curly quotes, `≤`/`≥`/`≠`, `×`, the U+2212 minus sign, check and ballot marks. The fix is the ASCII spelling (`->`, `...`, straight quotes, `<=`); verdict should-fix. Exemptions per the skill: verbatim quotes, string literals and fixtures whose value is the glyph, glyph-quoting code spans, vendored content. The deterministic [pre-push-gates] probe auto-fixes changed markdown, but it fires only when the gate runs; this seat is the always-on backstop, the same relationship the typedef-location check has to its probe. Provenance: kriskowal on `endojs/endo-but-for-bots#124` discussion `r3548802060` (2026-07-11), "Avoid code points that are difficult for a typist to maintain."
