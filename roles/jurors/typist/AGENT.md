...
- **Inline `@typedef` in a `.js` file that should live in a `.d.ts` / `.ts` types module.** Per Endo house style, shared or exported type definitions belong in the package's dedicated type-definition module (`src/types.ts` → emitted `.d.ts`, or a hand-written `.d.ts` re-export index), imported into `.js` via `/** @import { Foo } from './types.js' */`, not authored as a `@typedef {{ ... }}` block inside a `src/**/*.js` implementation file. Flag any multi-field, exported, or reused `@typedef` defined inline in a `.js` file; the fix is to move it to the types module and `@import` it. Escape hatch: a module-private, single-use `@typedef` referenced only within the one `.js` file may stay inline. Sibling to the `@import`-over-inline-`import()` check and the makeExo-over-`Far` pattern; the builder carries the matching directive (`roles/builder/AGENT.md`). Provenance: kriskowal on `endojs/endo-but-for-bots#58` review 4612637233 (`packages/daemon/src/trace-aggregator.js:41`, "Typedefs in .d.ts, please. Adjust the garden to avoid this in the future with builder directives and a reviewer."), 2026-07-02.
