...
- **Ownership-map reconstruction (multi-component / multi-layer designs).** When the design spans two or more components or architectural layers, do not judge each layer's local consistency in isolation — **reconstruct the ownership map across the boundary and challenge it**, exactly the map the designer must state per [ownership-map](../../../skills/ownership-map/SKILL.md). For each boundary the design crosses, answer four questions from the design's own text: **(1) who owns persistent/durable state** (each durable datum — snapshot, transcript, store — belongs to exactly one layer); **(2) who owns the commit/discard decision**; **(3) who owns restart/replay**; **(4) who owns execution classification** (deciding *how* execution ended, distinct from deciding *what to do* about it). Then run the **inner/outer naming check**: flag any API or type name that assigns an outer-layer lifecycle concept (crank, commit, transaction, snapshot, replay, embargo, checkpoint, rollback, durable, persist) to an inner mechanism that only evaluates and runs to quiescence — an engine returns an *execution* result, not a *crank* result. A responsibility claimed by two layers, a durable datum with no single owner, or an inner type named for an outer concept is a **must-fix**: the ambiguity locks a wrong boundary into the implementation. A design that carries an **explicit, coherent** ownership map is not faulted merely for naming multiple layers — confirm the map and move on. Grounding: `endojs/endo-but-for-bots#1018` put an engine-side `CrankOutcome` at the Ironhorse `Machine` seam while the supervisor owned transcript, embargo, commit, restart, and replay; six design-panel rounds (this seat among them) checked local consistency without ever assembling the map that showed the conflict (review-miss cluster `architectural-boundary-ownership`; the correction renamed it `ExecutionOutcome`). **The panel hands this seat a deterministic ownership-map pre-pass** (`scripts/jobs/gardening/ownership-map-signal.sh`) as trusted DATA when a design spans ≥2 layers: it names the layers, whether an ownership-map section is present, and fused inner/outer name candidates. Treat it as a lens-firing prompt, not a verdict — the call above is yours.
