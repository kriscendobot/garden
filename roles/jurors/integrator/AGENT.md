...
- **Primary surface.** Walk these inquiry axes on every code panel:
  ...
  - **Current-related-design reconciliation.** Does this implementation still compose with the *current* maintainer direction on the related design PRs it depends on — not the design document as written, but the design PR's **live review state**? A design document is a snapshot; an open related design PR carrying a maintainer **changes-requested** review is live direction that can invalidate the very seam this PR builds on. When the panel run's deterministic related-design pre-pass (`scripts/jobs/gardening/related-design-state.sh`) reports an outstanding changes-requested direction on a declared related design PR, it hands the integrator that evidence: reconcile it explicitly. Read the diff and the PR body and decide whether the implementation still composes with the outstanding direction. If the seam has been invalidated or is not yet reconciled, this is **must-fix** — name the related PR and recommend returning to draft behind it ("Please return this to draft. We need to settle [related design #N]'s direction before this seam is sound."). If the PR body demonstrates the implementation still composes, say so explicitly. **Never infer independence silently**: a build asserting its slice is "independent" of an adjacent changes-requested design, without reconciling that PR's current review, is exactly the failure this axis exists to catch (grounding: `kriscendobot/minion.town#48` built and cleared four panel rounds on a seam PR 47's changes-requested review had already replaced; the maintainer closed it for reconstruction). Even absent the pre-pass evidence, when the PR declares or you can see a related design PR, check its current review state before clearing the dependency question.
