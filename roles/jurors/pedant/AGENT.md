...
- [typist-friendly-code-points](../../../skills/typist-friendly-code-points/SKILL.md): the project's hard-to-type code-point rule, layered on top of Chicago Manual.
...
- **Layered project rules.** On top of Chicago Manual, enforce the garden's own prose rules: no em-dashes per `skills/em-dash-style/SKILL.md` (rewrite as period, parentheses, or colon); paths within one document tree are relative per `skills/relative-paths/SKILL.md`; diagrams use mermaid, not ASCII or line-art, per kriskowal on `endojs/endo-but-for-bots` PR #238 inline comment id=3237804603 (2026-05-17). The mermaid rule covers any architecture, sequence, state-machine, or capability illustration; exceptions are inline directional arrows in prose or code comments, pre-existing ASCII diagrams in untouched files (do not propose retrofit), and tabular or terminal-log captures (those are data, not diagrams). No banner horizontal rules in code comments inside the design's code blocks, per `skills/no-comment-banners/SKILL.md`: a comment line whose body is four or more repeated `-=*~_` characters and nothing else (`// ----`) is decorative noise the project forbids. Provenance: kriskowal on `endojs/endo-but-for-bots#503` review `4573212313` (2026-06-25). Hard-to-type code points are avoided per `skills/typist-friendly-code-points/SKILL.md`: arrows (`→`), `…`, curly quotes, `≤`/`≥`/`≠` and similar symbols become their ASCII spellings (`->`, `...`, straight quotes, `<=`); this overrides the Chicago Manual preference for typographic quotes and ellipses, the same project-wins shape as the em-dash override. Provenance: kriskowal on `endojs/endo-but-for-bots#124` discussion `r3548802060` (2026-07-11). These are not Chicago Manual prescriptions; they are project conventions the pedant carries.
