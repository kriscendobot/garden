...
**Never put a comment/PR/issue body on a shell command line — pass it as a file (shell-injection discipline).** When you post any body to GitHub, do **not** interpolate the body *text* into a shell word — not `gh pr comment N --body "…"`, not `gh api … -f body="…"`, not a here-string. Bash performs command substitution on backticks and `$(…)` while it parses that command line, **before `gh` ever runs**, so any `` `word` `` in the body is executed as a command and replaced by its (usually empty) output. That is what silently deleted every `` `inline-code` `` span from a review reply on `endojs/endo-but-for-bots#475` (the identifiers `bytesFromText`, `encodeUtf8`, … were run as commands → `command not found` → empty → collapsed text). The corruption is the visible symptom; the underlying flaw is a **command-injection sink**: the same interpolation would execute an attacker's `` `$(…)` `` if untrusted text (a quoted-in reviewer comment, a commit message, a branch or file name) ever reached it — potential remote code execution on the bot host, not merely garbled markdown. No downstream `gh` wrapper can undo this, because the shell has already evaluated the line before the wrapper sees the argv (it only ever sees the post-substitution, already-collapsed string). The fix is structural and lives at the point of generation: **write the body to a file and hand the file to `gh`**, so the shell never evaluates the content —
  * `gh pr comment N --body-file reply.md` · `gh issue comment N --body-file reply.md` · `gh pr review N --body-file reply.md`;
  * for an inline review reply, `gh api …/comments -F body=@reply.md` (or `--input payload.json`). `-F`/`--field` with a leading `@` reads the file; `-f`/`--raw-field` does **not** dereference `@` (`skills/pr-review-thread-replies/SKILL.md` § "Never pass a file path as the body value").

Do not rely on `-f body="$(cat reply.md)"` as your safety story: command-substitution output is not itself re-scanned, so file *content* is safe there, but the form is one keystroke from the unsafe literal that caused the bug — prefer passing the file to `gh` directly. The fleet's `gh` wrapper already normalizes `--body`/`--body-file`/`-f body=` into a temp `--body-file` for the provenance footer, so the file-based path is the well-trodden one. Bounding, not eliminating, the exposure: the § Monitoring safety constraint keeps the untrusted-text population small (only comment/PR text from trusted-or-sender-gated sources reaches a role's context), but that is defense-in-depth around this sink, never a licence to interpolate a body onto a command line.
...
**Project scope: IronHorse pause LIFTED (2026-09-18).** From 2026-09-09 to 2026-09-18 IronHorse work was PAUSED at zero priority while IronHorse underwent a large external refactor. A trusted maintainer (kriskowal) confirmed on garden issue [#91](https://github.com/kriscendobot/garden/issues/91) that the refactor is complete and explicitly lifted the pause; the journal record (`pauses/ironhorse.md`) has been deleted, so `scripts/jobs/pause-project.sh status ironhorse` reports not paused and the fleet may again claim, post, promote, orchestrate, and run IronHorse implementation, review, test262, optimization, and follow-up work. **One narrower gate persists:** the continuous fuzz timer and service (`garden-ironhorse-fuzz.{timer,service}`) remain in `PAUSED_UNITS` and re-arming that lane is a separate, deliberate operator action gated by the checklist in [`context/operations/ironhorse-fuzz.md`](../context/operations/ironhorse-fuzz.md) § Re-arming — the general lift is a prerequisite for that checklist, not authority to resume the fuzz campaign on its own. Lift authority: kriskowal, 2026-09-18, garden issue [#91](https://github.com/kriscendobot/garden/issues/91) (originally paused 2026-09-09 by kumavis). The journal-backed enforcement that carried the pause lives on regardless of any project: mechanism [`designs/project-pause-enforcement.md`](../designs/project-pause-enforcement.md), predicate `project_pause_hit` in `scripts/jobs/common.sh`, armed/lifted with `scripts/jobs/pause-project.sh`.
...
## Pre-existing equivalent surface before adding a new one

Before you add a **public surface** — an exported API function, a CLI verb, or a path/route branch — first **locate the pre-existing equivalent surface** the codebase already exposes, and either **compose with it** or **state in the PR body why you diverge**. The check is concrete: before you export `streamGrep` beside an eager `grep`, add a `write <mount> <path...>` verb whose slash-path traversal `cat`/`ls` already reach, or split a pet-name string into segments in the daemon when the CLI adapter's `parsePetNamePath` already owns that translation, `git grep` the operation's stem and the route it walks across the whole repo, read the sibling you find, and mirror its composition/decomposition and its layer ownership. Two failure shapes this prevents: (1) **redundancy** — a new surface duplicating a route or layer the sibling already owns, so the same behavior is now reachable two ways; (2) **incoherent fusion** — a new surface fusing concerns the sibling keeps orthogonal (a fused `streamGrep(pattern, { glob })` where the eager twin exposes the seam `grep(pattern, glob(g))`), or placing a responsibility in a layer the sibling keeps elsewhere. Mirror the sibling, or justify the divergence in one line; do not author the surface as if the sibling did not exist.

Why this is a rule and not a memory aid: a redundant or incoherently-fused public surface is checkable at authoring time (the sibling is in the repo), so one that reaches the maintainer is a review the fleet should not have needed, and public surface is expensive to walk back once it ships. The signal ("a new export/verb/route sharing a stem or route with an existing sibling") is mechanically detectable and the `panel-hints` `C-curator-sibling-surface.sh` probe surfaces it, but *whether the sibling is truly equivalent and how to compose with it* requires reading both, so the responsibility sits with you at authoring time; the review-cycle backstop is the curator seat (its brief § Trace the pre-existing sibling surface). Source: `existing-cli-surface-equivalence` review-miss cluster (`endojs/endo-but-for-bots` #658, #897, #1085), where a mount-path reader branch duplicated the CLI's slash-path traversal, pet-name path translation was placed in the daemon Exo layer the CLI adapter already owned, and `streamGrep`/`streamGlob` fused glob+grep instead of mirroring the eager sibling's composition seam — each a new public surface authored without tracing its pre-existing equivalent.
