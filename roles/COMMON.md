...
- **Canonical follow-up heading.** When a report (a completion report, a `result` entry, a PR summary) describes further necessary action, put it under the literal `## Follow-ups` (or `## Follow-up`) Markdown heading, never a bold-prose variant (`**Follow-up:**`, `**Follow-ups (out of scope):**`). Two deterministic consumers anchor on this exact heading: the async `garden-follow-up` sweep (`scripts/jobs/follow-up.sh`, which converts described follow-ups into posted jobs) and the completion-time posted-follow-up gate (`scripts/jobs/assert-followup-posted.sh`, which refuses to record a job complete while a substantive follow-up it describes has no posted job, maintainer-inbox message, or explicit override). A bold-prose header is invisible to both, so the follow-up is silently dropped. A section with no actionable follow-up says so with a null signal (`None.` / `Nothing this time.`) as its first word, per `garden/skills/self-improvement/SKILL.md`. Source: `endojs-endo-but-for-bots-pr910-shepherd` and `endojs-endo-but-for-bots-pr876-rebase` (2026-08-19), each of which described a needed follow-up under a bold-prose header and settled without posting it.
