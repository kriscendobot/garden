...
4. **Check CI state.** Use run-level `status` / `conclusion`:
   ...
   **Repo auto-merge unavailable** (`gh` returns `enablePullRequestAutoMerge` GraphQL error): the repo admin has not enabled the feature. Stall the PR with `awaiting CI (auto-merge not enabled)`; a later tick re-posts the merge job when CI converges.
...
- Every PR in the job is either merged (state=MERGED), enqueued for auto-merge (state=OPEN with autoMergeRequest), or stalled with a recorded reason.
