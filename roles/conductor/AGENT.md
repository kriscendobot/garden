...
Linearize merges. Take a PR (or a queue of them) one at a time: establish the exact remote head contains the live base, safely rebase and lease-push when it does not, validate CI on that resulting head, then `gh pr merge --merge` to create a merge commit. The merge-commit shape preserves the PR's commits as a discrete cluster on the base history, attributable and unit-revertible upstream.
...
1. **Enter the isolated project worktree, fetch, and survey.** Use the checkout returned by `ensure-project-worktree.sh` for this job and run the merge spine from there (or set `GARDEN_PR_WORKTREE` to it). `git fetch <remote> <base> <head>`; compute behind / ahead / conflict per [rebase-hygiene-audit]. The survey is explanatory; the deterministic freshness gate in step 4 is authoritative at merge time.
...
4. **Rebase through the spine, check CI on that head, require current approval, and CARRY THE MERGE TO COMPLETION — do not end the job while merely waiting.** "Waiting for CI" is **not** a terminal state. The deterministic spine for this whole step is `scripts/jobs/gardening/ci-wait-merge.sh <owner/name> <N>`, run from the isolated project worktree. Before it accepts any CI, it unfreezes, synchronizes the worktree to the exact remote PR head, invokes `safe-rebase.sh` against a freshly fetched live base, and lease-pushes a rewrite. It binds every accepted rollup to the resulting `headRefOid`; at terminal green it repeats the live-base check and, if the base moved during CI, rebases again and restarts the wait. It then merges **in the same job** (run it; do not hand-roll a wait-then-exit).

   Ordinary calls independently require `reviewDecision=APPROVED` plus a non-stale `APPROVED` review from a login in `journal2:maintainers/allowlist`; if that list is empty, the documented bootstrap owners are `kriskowal` and `erights`. **A rebase never preserves that signature.** The reviewed commit IDs and their integration with the intervening base changed, so an approval on the pre-rebase head is intentionally stale. The first invocation finishes CI but stalls `merge blocked: no maintainer approval`; after the maintainer approves the rebased head, a later invocation's freshness check is a no-op and it may merge. If the base moved again, another rebase and re-approval are required. This keeps `pr-maintainer-approval-gh.sh`'s exact-head invariant intact; no agent force-push inherits a human signature.

   Only the botanist's explicit `--dependabot-auto-merge` call may omit that approval, after the spine proves the live author is `dependabot[bot]` and the repository is bot-owned. It uses the same rebase and post-rebase CI stages. The spine's exit code is the outcome — 0 merged, 2 already closed, 3 CI red (→ stall `ci red: needs shepherd`), 4 watch timed out (→ re-enqueue, still unmerged), 1 merge/rebase blocked. A `reason=needs-weave` rebase refusal stalls `needs weave`; do not resolve it as conductor. If ordinary-path approval is absent, stale because the head changed, dismissed, or from a non-maintainer, stall `merge blocked: no maintainer approval`; do not merge or auto-merge. If you drive the states by hand instead:
...
- **Only merge CI-green PRs that carry the required authority.** The ordinary path requires an **effective** `APPROVED` review by a journal maintainer — a still-standing approval, even one submitted on an earlier commit: a rebase or follow-up push does **not** stale it, and the merge spine deliberately does not require the review's `commit_id` to equal the current head. What revokes it is a GitHub review **dismissal** or the maintainer's own later **CHANGES_REQUESTED** (the veto). CI freshness is a separate, still-enforced gate (next norm): green must belong to the current head. An APPROVED + red PR is not a merge candidate; it is a shepherd job. A green human-authored PR without approval stalls `merge blocked: no maintainer approval`. The explicit botanist Dependabot path substitutes its completed criteria gate for the signature, but never for CI or the CHANGES_REQUESTED veto.
- **Rebase before accepting CI and recheck the base after CI.** A pre-rebase green run is not evidence. The rollup OID must equal the head published by the freshness gate, and terminal green is followed by one more live-base check; a changed head restarts CI. A non-lockfile conflict is `needs weave`, never conductor judgment.
