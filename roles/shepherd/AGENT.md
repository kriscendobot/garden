...
- _2026-06-18_: SES-init gotcha on `endo-but-for-bots#461` (shepherd 25c34a). Test files using `import 'ses'; import '@endo/eventual-send/shim.js'` do not call `lockdown()`, so `harden` is undefined at runtime. This was invisible before the `exo-stream` migration because nothing invoked `harden` in test-touched code. After the migration, `iterateReader()` from `@endo/exo-stream/iterate-reader.js` calls `harden()` internally; all eight affected component test files immediately failed with `ReferenceError: harden is not defined`. Fix: replace the two-line pattern with `import '@endo/init/debug.js'`. When a migration adds a new `@endo/exo-stream` consumer adapter, scan all test files in the changed packages for the two-line SES pattern and replace before pushing.
