...
- _2026-08-27_: Node 24.19.0 introduced an upstream native-addon worker-teardown regression. The signature is `RemoveEnvironmentCleanupHook` asserting `(env) != nullptr`, with a native addon's destructor (for example `Statement::~Statement()` from `better-sqlite3`) below it; downstream daemon-spawn and rejected-promise failures are secondary. A retry can change which test fails but does not clear the class. Until the [Node 24 backport](https://github.com/nodejs/node/pull/65042) ships, pin the affected Node 24 CI leg to 24.18.1 rather than changing application code or upgrading the addon; preserve required-check names by keeping the matrix label at `24.x` and selecting the exact runtime in the setup step. Verify the setup log says `node: v24.18.1` and the full leg passes before calling it green.
