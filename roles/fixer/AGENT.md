...
- [sibling-family-sweep](../../skills/sibling-family-sweep/SKILL.md): when a fix generalizes an operation across a family of sibling sites, enumerate every sibling and verify each was converted before pushing. See the operating norm below.
...
- **Spell any identifier the fix introduces out in full; do not abbreviate.** The never-abbreviate rule applies to fixer-authored names exactly as it does to builder-authored ones (`directory` not `dir`, `command` not `Cmd`, `temporary` not `Temp`/`tmp`, `context` not `ctx`, `request`/`response` not `req`/`resp`, etc.), even when unambiguous, because the maintainer standing-rejects abbreviations. When the review comment *is* "spell out `<name>`", that is a rename to satisfy, not to defend. The [pre-push-gates] `spell-out-identifiers` probe fails the gate on the abbreviated-segment shape; a file that genuinely must carry a domain abbreviation opts out with a `spell-out-exempt` marker in its first five lines.
- **Move substantial JavaScript typedefs to the package's declaration module.** When fixing or extending a `.js` implementation, put an exported, shared, reused, or multi-field object `@typedef` in the package's `types.d.ts` / `types.ts`, then pull it into the JavaScript use site with a top-of-file `/** @import { Foo } from './types.js' */`. Convert a typedef-only `.js` module to a declaration module and repoint the package's `types` export condition. A genuinely module-private, single-use typedef may remain beside the implementation. The [pre-push-gates] `typedefs-belong-in-dts` probe blocks the deterministic typedef-only-module case; the fixer must judge mixed implementation files because lexical occurrence alone cannot distinguish the legitimate escape hatch. This is the fix-loop counterpart of the builder directive and typist review rule. Provenance: `endojs/endo-but-for-bots#388` review `4413566645`, comments `3344377619` and `3344477615`.
- **Never add a bare plain re-export; a re-export must be a `@deprecated` compatibility shim pointing importers at the original, and importers must be migrated to the original.** When fixing or extending module exports, do not leave a plain `export … from '…'` (named, renamed, `default as`, `export *`, `export * as ns`) that re-emits another module's binding unchanged as a live second import path. A re-export is allowed only as a deprecated shim: a `@deprecated` JSDoc immediately above it naming the canonical original module, with importers migrated to that original. Deprecating-and-migrating is the fix; there is no auto-fixing role because it needs a chosen message and importer migration, which is judgment. Barrels / `index.js` are not exempt; type-only re-exports (`export type …`, `.d.ts`) are. See [re-export-deprecation-policy]. The [pre-push-gates] `no-plain-reexport` probe enforces this deterministically; `reexport-policy-exempt` in a file's first five lines is the per-file escape hatch. This is the fix-loop counterpart of the builder directive and the `reexport-auditor` seat. Provenance: @erights on `endojs/endo-but-for-bots#475`, design `designs/reexport-deprecation-policy-gauntlet.md` (approved PR #95).
...
- **A fix that generalizes an operation across a family of siblings converts every sibling.** When the fix spreads one operation across sibling sites *within this PR's lane* — twin packages (`packages/hex` / `packages/base64`), duplicated helper copies (`packages/harden/make-hardener.js` / `packages/ses/src/make-hardener.js`), paired constructors that jointly maintain one invariant (`packages/immutable-arraybuffer/src/lib.js`), or any N sites sharing one dispatch shape — enumerate every sibling (`git grep` the operation's identifiers and both the old and new dispatch predicates across the repo) and convert each the same way, or record why one legitimately differs. A skipped or divergent sibling is exactly the miss this guards: the maintainer's "scan this PR for this kind of inconsistency" on #1099 (`harden/make-hardener.js` on `isMutableTypedArray`, its ses copy still on `isTypedArray`; `hex/src/encode.js` on `.buffer.immutable`, its base64 twin on `ArrayBuffer.isView`). A sibling that lives in a *different* PR's lane stays a surfaced cross-PR follow-up per the norm above; siblings inside this PR are in-lane and get converted now. Full discipline: [sibling-family-sweep](../../skills/sibling-family-sweep/SKILL.md); the panel counterpart is the breaker's sibling-enumeration lens.
