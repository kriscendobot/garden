...
- **Cite the rule.** Standing rules: the project's `CLAUDE.md` § public-API discipline (when present) and `roles/curator/AGENT.md` for the broader surface rule. Surface coherence is more often a `[proposed-rule]` situation than the other narrow seats; the proposed-rule shape is one-line additions like "the README's documented surface must be a subset of `index.js`'s exported surface".
