...
9. **Land every library/project write through the lander, regenerate the sections index, journal a `result`, and complete the job.** Land each library and project content file with **`scripts/jobs/land-journal-edit.sh <journal2-relative-path>`** (body from a body-file or stdin) — the only sanctioned way to land a library/project content edit (see *Land content edits through the lander* below). Then, **as the final landing step, regenerate `sections/README.md`:**

   ```sh
   ../../scripts/jobs/regenerate-sections-index.sh        # no args = --land
   ```

   It syncs the `origin/journal2` tip, rebuilds the auto-generated flat index from your just-landed section files (preserving the preamble and the hand-curated `## Historical ingest log` verbatim), and lands the result through the same producer-clone sync+CAS path. It is **idempotent** (lands nothing when the index is already current) and **refuses to land an incomplete index** (a `kind: index` parent whose child list points at a missing file fails with a `DANGLING` report — write the missing child, the same defect class step 8 catches, and re-run). This is the deterministic projection that replaces hand-editing the index; running it last is what keeps the backstop from ever going stale. Then post the `result` with `scripts/jobs/journal-entry.sh result`. The `result` lists each source ingested (with section-count) or skipped (with matching sha), each topic/concept page touched, each follow-on job posted, any deferred backlog, the integrity-gate verdict (step 8), and that the sections index was regenerated. Then complete the job (`doin → tada`) per [job-board](../../skills/job-board/SKILL.md). End the report with `Self-improvement: ...`.
...
- The affected hand-maintained README indexes match their file sets: the library's `sources/`, `topics/`, `concepts/` indexes and `keywords.md`; the project's README index. (`sections/README.md` is not in this list — it is regenerated, not hand-maintained.)
...
- `sections/README.md` was regenerated as the final landing step (`regenerate-sections-index.sh`) and lands current — never hand-edited.
