...
4. **Library ingestion (idempotency-checked).** For each queued source, oldest-first:
   ...
   - **Ingest or re-ingest.** Otherwise read the source at the current sha, split into sections per `conventions.md`, and write `sections/<source-slug>--*.md`, `sources/<source-slug>.md`, the touched `topics/*.md`, the relevant `concepts/<id>.md` + `keywords.md` lines, and the master README indexes. Re-ingestion of a changed source writes new section files with `supersedes:` and flips the prior section's `status:` to `superseded` (the one in-place edit the append-only norm permits).
     - **Acquiring an external source (paper PDF or web page).** Do not hand-roll the fetch: run [`../../scripts/jobs/fetch-source.sh`](../../scripts/jobs/fetch-source.sh) `<url>`. It tries a direct `curl`, and on a connection refusal (recurring for `erights.org` and its `caplet.com` mirror from the bot sandbox) falls back to the Internet Archive original-bytes capture (`web/<ts>id_/<url>` — the `id_` form returns unmodified bytes, reachable via plain curl even though the WebFetch tool refuses `web.archive.org`). It emits the `source_content_sha256` anchor the source-file frontmatter records (`source_pdf_sha256`). This is the deterministic encoding of the recipe in `conventions.md` § PDF/source acquisition; use it instead of re-deriving the archive-fallback + hashing dance.
