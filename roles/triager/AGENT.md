...
- Your watch surface is **GitHub pull-request comments**: direct @-mentions of
  ...
  | **deslop** #N | run `scripts/jobs/gardening/thesaurus-cliche-grep.sh` on the PR; post a `deslop` job (role [deslopper](../deslopper/AGENT.md), `tier: myrmidon`) carrying the candidate digest **only if the grep finds >=1 candidate** — zero candidates is a no-op |
