...
withdrawn_at: 2026-09-17T00:02:42Z
...
role: builder
posted_by: producer
posted_at: 2026-08-15T01:24:26Z
---
...
dispatch: automatic
---
# Implement Intl.RelativeTimeFormat — close the intl402/RelativeTimeFormat slice

**Family slice:** `intl402/RelativeTimeFormat` (~67 cases).

**Measured diagnosis:** `typeof Intl.RelativeTimeFormat === 'undefined'` — entirely unimplemented. `new Intl.RelativeTimeFormat('en').format(-1,'day')` → Ironhorse throws `TypeError` → `abort-value-differs`.

**Deliverable:**
- Register `Intl.RelativeTimeFormat` in `create_intl` and implement the constructor with options `localeMatcher`, `numeric` (always/auto), `style` (long/short/narrow); prototype `format(value, unit)`, `formatToParts(value, unit)`, `resolvedOptions()`, static `supportedLocalesOf`. Handle unit singular/plural normalization and the `numeric:'auto'` special forms (yesterday/today/tomorrow/last week/etc. for `en`).
- Drive every case to `IronhorseOnlyComplete` with correct values; lock exact values (numeric always/auto × style × representative units and signs, formatToParts structure, resolvedOptions key order) with Rust regression tests.
...
<!-- garden-annotation: key=67344d1d5edd by=producer at=2026-08-16T06:49:47Z -->
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:42:28Z -->
