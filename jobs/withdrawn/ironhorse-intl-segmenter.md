...
withdrawn_at: 2026-09-17T00:03:09Z
...
role: builder
posted_by: producer
posted_at: 2026-08-15T01:24:54Z
---
...
dispatch: automatic
---
# Complete Intl.Segmenter — close the intl402/Segmenter residual slice

**Family slice:** `intl402/Segmenter` (~29 residual cases).

**Measured diagnosis:** Segmenter is PARTIAL — `[...new Intl.Segmenter('en',{granularity:'word'}).segment('hi there')].length` → `3` completes. 29 cases remain unclosed. Triage each residual with `dual_run` and close.

**Deliverable:**
- Close the residual Segmenter surface: `granularity` (grapheme/word/sentence) option validation + abrupt completions, `segment()` returning a `%Segments%` object, `containing(index)`, the `%SegmentsIterator%` (`[Symbol.iterator]`, `next`), the segment-data record shape (`segment`/`index`/`input`/`isWordLike`), `resolvedOptions`, constructor + prototype prop-desc, static `supportedLocalesOf`. Drive every case to `IronhorseOnlyComplete` with correct values.
- Extend the Rust regression tests for the newly-closed paths (grapheme cluster boundaries, word `isWordLike`, sentence boundaries, `containing`).
...
<!-- garden-annotation: key=67344d1d5edd by=producer at=2026-08-16T06:49:56Z -->
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:42:36Z -->
