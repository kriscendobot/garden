...
withdrawn_at: 2026-09-16T23:58:46Z
...
role: builder
posted_by: producer
posted_at: 2026-08-15T01:24:17Z
---
...
dispatch: automatic
---
# Intl namespace + getCanonicalLocales/supportedValuesOf + Array/BigInt toLocaleString bridge — close intl402/Intl slice

**Family slice:** `intl402/Intl` (~51 cases: the `Intl` namespace object, `Intl.getCanonicalLocales`, `Intl.supportedValuesOf`) plus `intl402/Array/prototype/toLocaleString`, `intl402/BigInt/prototype/toLocaleString`, and `intl402/String/prototype/localeCompare` bridge cases. **Run AFTER `ironhorse-intl-numberformat`** (Array/BigInt toLocaleString call element `toLocaleString` → NumberFormat).

**Measured diagnosis:** `Intl.getCanonicalLocales` and `Intl.supportedValuesOf` are present (`typeof === 'function'`). `[1,2,3].toLocaleString()` → `Unsupported("Array.prototype.toLocaleString:locale-stringification")` while the **oracle completes** (XS has a default Array toLocaleString) → this is a genuine `covered` opportunity, not a host exclusion. `123n.toLocaleString('en')` → BothAbort `TypeError` (depends on the NumberFormat bridge).

**Deliverable:**
- Implement `Array.prototype.toLocaleString` locale-stringification (currently an `Unsupported` opcode) so it completes; certify against the oracle where XS completes (**covered**).
- Close the `intl402/Intl` namespace slice (prop-desc of `Intl`, `getCanonicalLocales` edge cases, `supportedValuesOf` categories) to `IronhorseOnlyComplete`/covered.
- Wire `BigInt.prototype.toLocaleString` through the NumberFormat bridge from the numberformat child.
- Rust regression tests locking the exact values / structure.
...
<!-- garden-annotation: key=67344d1d5edd by=producer at=2026-08-16T06:49:09Z -->
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:41:52Z -->
