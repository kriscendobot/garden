...
withdrawn_at: 2026-09-16T23:59:05Z
...
role: builder
posted_by: producer
posted_at: 2026-08-15T01:24:31Z
---
...
dispatch: automatic
---
# Implement Intl.DurationFormat — close the intl402/DurationFormat slice

**Family slice:** `intl402/DurationFormat` (~89 cases). **Run AFTER `ironhorse-intl-numberformat`** and after RelativeTime/ListFormat exist (DurationFormat composes NumberFormat + ListFormat).

**Measured diagnosis:** `typeof Intl.DurationFormat === 'undefined'` — entirely unimplemented. `new Intl.DurationFormat('en').format({hours:1,minutes:2})` → Ironhorse throws `TypeError` → `abort-value-differs`.

**Deliverable:**
- Register `Intl.DurationFormat` in `create_intl` and implement the constructor with `style` (long/short/narrow/digital) and the per-unit display options (`years`..`nanoseconds`, each `long/short/narrow/numeric/2-digit` as applicable) plus `fractionalDigits`, `numberingSystem`, `localeMatcher`; prototype `format(duration)`, `formatToParts(duration)`, `resolvedOptions()`, static `supportedLocalesOf`. Validate the Temporal-style duration record (ToDurationRecord) and its sign/range rules.
- Drive every case to `IronhorseOnlyComplete` with correct values; lock exact values (style matrices, per-unit display, digital style, formatToParts structure, resolvedOptions key order) with Rust regression tests.
...
<!-- garden-annotation: key=67344d1d5edd by=producer at=2026-08-16T06:49:28Z -->
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:42:08Z -->
