...
withdrawn_at: 2026-09-16T23:59:11Z
...
role: builder
posted_by: producer
posted_at: 2026-08-15T01:24:50Z
---
...
dispatch: automatic
---
# Complete Intl.ListFormat — close the intl402/ListFormat residual slice

**Family slice:** `intl402/ListFormat` (~45 residual cases).

**Measured diagnosis:** ListFormat core is already proven (child-20 value-locked English/Spanish types, styles, formatToParts, resolvedOptions in `tests/intl_formatters.rs`). 45 official cases remain unclosed — option-validation / abrupt-completion / prop-desc / poison-getter / iterable-coercion edges the focused tests did not exercise. Triage each residual with `dual_run` and close.

**Deliverable:**
- Close the residual ListFormat surface: `type`/`style` option validation + abrupt completions, iterable coercion of the list argument (non-string element TypeError, `undefined`/`null` handling), constructor prop-desc, `format`/`formatToParts`/`resolvedOptions` bound-getter and prop-desc semantics, static `supportedLocalesOf`. Drive every case to `IronhorseOnlyComplete` with correct values.
- Extend the Rust regression tests for the newly-closed paths.
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:42:14Z -->
