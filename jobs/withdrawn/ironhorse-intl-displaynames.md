...
withdrawn_at: 2026-09-16T23:58:58Z
...
role: builder
posted_by: producer
posted_at: 2026-08-15T01:24:21Z
---
...
dispatch: automatic
---
# Implement Intl.DisplayNames — close the intl402/DisplayNames slice

**Family slice:** `intl402/DisplayNames` (~43 cases).

**Measured diagnosis:** `typeof Intl.DisplayNames === 'undefined'` — entirely unimplemented; `Object.getOwnPropertyDescriptor(Intl,'DisplayNames')` is absent. `new Intl.DisplayNames(undefined,{type:'language'})` → Ironhorse throws `TypeError` → `abort-value-differs`. The bulk of the slice is option-validation / abrupt-completion / poison-getter / prop-desc / resolvedOptions structure tests (these do NOT need heavy CLDR data); only `prototype/of/*-valid` cases need display-name tables.

**Deliverable:**
- Register `Intl.DisplayNames` in `create_intl` and implement the constructor with option processing + validation for `localeMatcher`, `style` (narrow/short/long), `type` (language/region/script/currency/calendar/dateTimeField), `fallback` (code/none), `languageDisplay` (dialect/standard), including the spec's abrupt-completion order and the "type is undefined → TypeError" rule.
- Implement `of(code)` for each type and `resolvedOptions()`; wire prop-desc (`writable/enumerable/configurable`) to spec.
- Drive every case to `IronhorseOnlyComplete` with correct values; lock exact values (option matrices, resolvedOptions key order, representative `of()` outputs for `en`) with Rust regression tests.
...
<!-- garden-annotation: key=67344d1d5edd by=producer at=2026-08-16T06:49:23Z -->
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:42:01Z -->
