...
withdrawn_at: 2026-09-16T23:59:17Z
...
role: builder
posted_by: producer
posted_at: 2026-08-15T01:24:41Z
---
...
dispatch: automatic
---
# Complete Intl.Locale — close the intl402/Locale residual slice

**Family slice:** `intl402/Locale` (~93 residual cases).

**Measured diagnosis:** Locale is PARTIAL — `new Intl.Locale('en-US').toString()` → `'en-US'`, `.maximize().toString()` → `'en-Latn-US'`, `new Intl.Locale('en-US-u-hc-h12').hourCycle` → `'h12'` all complete. But 93 cases still `abort-value-differs`. Triage each residual with `dual_run` and close.

**Deliverable:**
- Close the residual Locale surface: constructor tag parsing + option overrides (`language`/`script`/`region`/`calendar`/`collation`/`hourCycle`/`caseFirst`/`numeric`/`numberingSystem`), the getter properties (`baseName`, `language`, `script`, `region`, `calendar`, `caseFirst`, `collation`, `hourCycle`, `numeric`, `numberingSystem`), `maximize`/`minimize` likely-subtags, and `toString` canonicalization; option-validation/abrupt-completion ordering. Drive every case to `IronhorseOnlyComplete` with correct values.
