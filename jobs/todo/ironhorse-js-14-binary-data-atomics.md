...
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-08-14T15:34:05Z cleared=none -->
...
---
handler-timeout: 7200
# Complete ArrayBuffer, SharedArrayBuffer, DataView, TypedArray, and Atomics
...
`test/built-ins/{ArrayBuffer,SharedArrayBuffer,DataView,TypedArrayConstructors,TypedArray,Atomics}/**` and all cases tagged `Atomics` or `SharedArrayBuffer`.

Implementation scope:
Implement buffer storage/detachment/resizing/transfer/species, integer-indexed exotic objects, every typed-array element type including BigInt, DataView endian/coercion behavior, and deterministic single-agent Atomics semantics. For truly multi-agent cases, implement the harness agent contract or document a standards-grounded host exclusion rather than pre-skipping the whole feature.
