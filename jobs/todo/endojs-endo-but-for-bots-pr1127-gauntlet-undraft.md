...
# Gauntlet stage: UNDRAFT — endojs/endo-but-for-bots PR #1127

You are the FINAL stage of a staged gauntlet (endojs-endo-but-for-bots-pr1127-gauntlet). The panel passed. Un-draft the PR.

1. Idempotence: `gh pr view https://github.com/endojs/endo-but-for-bots/pull/1127 --json isDraft,state`. If the PR is already ready
...
2. Advisory appellate pass (advisory only — it never blocks the un-draft): a light
   ...
   gate on it.
3. `gh pr ready https://github.com/endojs/endo-but-for-bots/pull/1127` to un-draft (kind=feature — a probe never reaches this stage).
