...
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-08-25T12:40:03Z cleared=none -->
...
---
handler-timeout: 10800
# js-26 MILESTONE — parser/compiler-unimplemented constructs + Annex-B parse (parse-or-decode)

Consolidates the `ironhorse-js-26-cl-parser-annexb` child (from `residual-closure`) into a
per-family milestone with a real budget, minted by
`ironhorse-js26-milestone-consolidation` (2026-08-22).

## Scope — the `parse-or-decode` causal family (parser/compiler level, distinct from VM aborts)
Close the `parse-or-decode` / compiler-unimplemented residuals across
`language/function-code`, `language/global-code`, `language/statements/class`, and the
`annexB/language` PARSE surface (as opposed to the runtime `with`-opcode surface, which is
its own milestone — `ironhorse-js26-milestone-with-opcode`). This is parser/coder work in
`ironhorse-compile`: constructs the front end currently declines to parse or decode, and
the Annex-B grammar extensions (legacy octal/`\8`/`\9`, HTML-comment tokens,
labelled-function-declaration, `RegExpIdentifierName`, etc.) where the gap is a parse
error rather than a VM opcode.

Anything failing at the VM/runtime level (not parse-or-decode) is OUT of scope — leave it
to the milestone or multi-day cluster that owns its causal reason; do not relabel.
