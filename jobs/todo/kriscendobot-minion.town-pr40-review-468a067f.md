...
# Review directive on kriscendobot/minion.town PR #40

A trusted maintainer/contributor REVIEW on #40. Treat the WHOLE review
...
Review: https://github.com/kriscendobot/minion.town/pull/40#pullrequestreview-4934499617
...
trailing number in the Review URL above), each with its file:line + text:
  gh api --paginate repos/kriscendobot/minion.town/pulls/40/comments --jq '[.[]|select(.pull_request_review_id==REVIEW_ID)]'
and re-fetch the review body itself:
  gh api repos/kriscendobot/minion.town/pulls/40/reviews/REVIEW_ID --jq .body
...
[APPROVED] Please always use “garden”. The word “fleet” is a Claude-ism we can avoid because we already had our own name. We need to emphasize that rendering vitals must not consume agent tokens. Producing content for the vitals should also not consume agent tokens. Telem

## BEFORE you edit — run the recheck preflight (deterministic)

A peer may have already resolved this feedback. Run, from the garden root:

  scripts/jobs/gardening/pr-feedback-preflight.sh kriscendobot/minion.town 40 4934499617 kriskowal
