...
dispatch: automatic
---
# Review directive on kriscendobot/minion.town PR #39

A trusted maintainer/contributor REVIEW on #39. Treat the WHOLE review
...
trailing number in the Review URL above), each with its file:line + text:
  gh api --paginate repos/kriscendobot/minion.town/pulls/39/comments --jq '[.[]|select(.pull_request_review_id==REVIEW_ID)]'
and re-fetch the review body itself:
  gh api repos/kriscendobot/minion.town/pulls/39/reviews/REVIEW_ID --jq .body
...
[INLINE-REVIEW] [CHANGES_REQUESTED] Please post a job to plan out metering for weblet *usage*. That is, an account to charge credits for usage. We should also plan to surface statistics about the rate of spending from a guest’s accounts, and annotated subaccounts, so customers 

## BEFORE you edit — run the recheck preflight (deterministic)

A peer may have already resolved this feedback. Run, from the garden root:

  scripts/jobs/gardening/pr-feedback-preflight.sh kriscendobot/minion.town 39 4910891844 kriskowal
