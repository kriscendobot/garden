...
You are ONE stage of a staged gauntlet (endojs-endo-but-for-bots-mount-stream-glob-grep-build-gauntlet). Do ONLY the clean stage, then STOP.
...
2. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh endojs-endo-but-for-bots-mount-stream-glob-grep-build-gauntlet-clean <pr-head-owner>/<repo-name> <pr-head-branch>`.
