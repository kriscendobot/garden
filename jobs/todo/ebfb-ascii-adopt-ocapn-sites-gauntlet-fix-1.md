...
# Gauntlet stage: FIX round 1 — endojs/endo-but-for-bots PR #980

You are ONE stage of a staged gauntlet (ebfb-ascii-adopt-ocapn-sites-gauntlet). Apply the panel's must-fix items ONCE,
push, watch CI, then STOP — do NOT re-run the panel (the driver re-posts panel-2).

1. Get an ISOLATED project checkout of the PR head:
   `/home/kris/garden2/scripts/jobs/ensure-project-worktree.sh ebfb-ascii-adopt-ocapn-sites-gauntlet-fix-1 endojs/endo-but-for-bots <pr-head-branch>`.
2. Read the LATEST panel verdict on https://github.com/endojs/endo-but-for-bots/pull/980 (the request-changes `gh pr review` the
