...
model: opus
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-07-18T19:31:04Z -->

---
model: opus
---
# Stage-10c child 3/5 — SES worker-bundle boot, gap round 6 (target: `lockdown()` completes)

**Provenance:** continuation of stage-10c child 2 (`xs2rust-endor-stage10c-ses-boot-r5`), which ran
immediately before you in this serial orchestration. **READ ITS TADA REPORT FIRST**
(`journal/jobs/tada/xs2rust-endor-stage10c-ses-boot-r5.md`) — it names the exact halt signature, the
attributed SES source line, and the isolated dual-run form where it stopped. Resume EXACTLY there. Also read
the r3/r4 tadas (`journal/jobs/tada/xs2rust-endor-stage10b-ses-boot-r3.md`, `-r4.md`) for the method and the
oracle-ceiling notes.

## The work

1. Same method as r1–r5: close the frontier gap, promote an isolated oracle-reaching boot-gate form, re-drive
   the bundle, **push per gap**.
2. **Target for this round:** the SES bundle's `lockdown()` call COMPLETES in the composed environment (the
   assert-prelude method), i.e. the bundle evaluates to its end or to a halt clearly PAST lockdown (worker
   bootstrap surface). If lockdown completes, drive the daemon worker shim's remaining boot surface
   (`ses_boot.js` through its final statement) as far as the window allows.
3. If the frontier is still short of lockdown when the window nears its end, that is an honest remainder:
   report the exact final halt signature + attributed source line + reproducing dual-run form, and count the
   boot-gate total. Do NOT burn the window on any single gap without checkpointing — push each verified item
   as it lands.
