...
# Gauntlet stage: UNDRAFT — kriscendobot/minion.town PR #17

You are the FINAL stage of a staged gauntlet (kriscendobot-minion.town-pr17-gauntlet). The panel passed. Un-draft the PR.

1. Idempotence: `gh pr view https://github.com/kriscendobot/minion.town/pull/17 --json isDraft,state`. If the PR is already ready
...
2. Advisory appellate pass (advisory only — it never blocks the un-draft): a light
   ...
   gate on it.
3. `gh pr ready https://github.com/kriscendobot/minion.town/pull/17` to un-draft (kind=feature — a probe never reaches this stage).
