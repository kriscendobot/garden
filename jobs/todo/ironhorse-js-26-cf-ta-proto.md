...
dispatch: automatic
---
# js-26 cf: %TypedArray%.prototype methods + element access coercion

**Scope:** the method-shaped bulk of `built-ins/TypedArray` (~1347 cases): the
`%TypedArray%.prototype` operations — `copyWithin, fill, set, subarray, slice, indexOf,
lastIndexOf, includes, join, reverse, sort, toSorted, with, at, find*, map, filter, reduce*,
forEach, every, some, keys, values, entries, byteLength/byteOffset/length getters,
Symbol.toStringTag` — plus per-element get/set numeric coercion (ToNumber, ToBigInt for
64-bit views), detached-buffer TypeError, and canonical-numeric-index-string behavior.

**Engine sites:** interp.rs `Native::TypedArray` method dispatch + the TypedArray element
read/write paths; the abstract `ValidateTypedArray`/`IntegerIndexedElementGet/Set` shape.
This is the single largest child — expect to re-decompose by method family
(mutators / iterators / index-search / reducers / accessors).
