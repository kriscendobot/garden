...
model: fable
---
<!-- garden-promoted-from-plan: gate=blocked priority=normal at=2026-07-17T18:26:08Z -->
...
it.**), and `-s24` (2026-07-17: **stage-8 halt recovery.** The stage-8 orchestration HALTED at
child 3/6 `xs2rust-endor-stage8-cxs-baseline`: 5 claims in 52 minutes, every handler killed
inside a ~11:30–12:40Z transient API/usage-cap outage window (s24's own first claim died in 2s
in the same window), so the reaper poisoned it (`deadline-overrun`, requeue_cycles 5) and the
serial-halt swept children 4–6. s24 diagnosed infra-not-spec (fleet healthy again from
~13:00Z), retired the poisoned plan file (s9/stage-4b precedent), and re-dispatched the four
unbuilt children as serial-halt orchestration **`xs2rust-endor-build-stage8b`** — children in
order: `xs2rust-endor-stage8-cxs-baseline-r2` (re-cut with recovery context),
`-class-construction`, `-boot-surface-remainder`, `-gate-remeasure` (bodies carried verbatim
from the stage-8 originals, renumbered). s24 detail you need: (a) stage-8 children 1–2
(daemon-bundle-imports, boot-generators) COMPLETED before the halt; their tada reports are on
the board and their commits are on the branch (pre-rebase tip `65180ad877` ≡ post-rebase
`d35a2dfb14`, subjects `daemon: make git backend + host spawner injectable…`,
`docs(endo-rust): mark daemon-bundle Node-import blocker resolved`, `feat(endo-daemon):
restore worker/SES boot bundle generators + XS worker entry`) — so the whole-stage-8 review
covers stage-8 children 1–2 PLUS the four stage-8b children. (b) The dead cxs-baseline child
left reusable caches in scratch worktree `project-wt-xs2rust-endor-stage8-cxs-baseline-5cd7f36a`
(node_modules, `c/moddable` at the oracle pin, the 3 generated gitignored bundles, 770M root
`target/` incl. the release `endor` bin) and a completed-but-INVALID `test:rust` run
(`$HOME/tmp/s8-test-rust.log`, 12:10Z: 279 failed / 65 skipped, 549 × `endo.sock not ready`).
INVALID because the long scratch-worktree path pushes the daemon's per-test socket path to
126 bytes — over the AF_UNIX `sun_path` limit; `test/channel.test.js` caps at
`MAX_UNIX_SOCKET_PATH = 90` and truncates test-dir names, but the fixed overhead
(`<worktree>/packages/daemon/tmp` ≈ 100 chars) already exceeds the cap, so every daemon spawn
fails identically regardless of engine. The r2 child is instructed to re-measure from a
short-real-path `git clone --shared` at `$HOME/tmp/s8cxs` (symlinks do NOT work — Node resolves
real paths). **This AF_UNIX constraint binds EVERY future `test:rust` measurement, including
stage 9's Rust-engine finish-line runs: always measure from a short real path, and treat any
mass `endo.sock not ready` result as a path-length artifact until excluded.**)
You were parked `blocked_on: xs2rust-endor-build-stage8b` and promoted because it reached a
...
FETCH_HEAD`), read the stage-8b orchestration completion record and each child's
`journal/jobs/tada/xs2rust-endor-stage8-*.md` — the four stage-8b children (`-cxs-baseline-r2`,
`-class-construction`, `-boot-surface-remainder`, `-gate-remeasure`) plus the two stage-8
children that completed before the halt (`-daemon-bundle-imports`, `-boot-generators`). If a
child vanished without a tada report, check `git log --all -- jobs/` for reaper poisoning —
diagnose before re-dispatching (distinguish a real spec defect from another transient outage
window by the kill pattern: constant-elapsed early deaths across both hosts = outage; a
serial-halt failure surfaces in the orchestration record). Program state:
...
- **Stage 8 (daemon groundwork + engine boot-surface remainder): built across the original
  stage-8 orchestration (children 1–2) and the stage-8b recovery orchestration you were blocked
  on (the remaining four). Your job now (s25): the whole-stage-8 review.** Per the binding rule
  ...
  + park s26 blocked on it, the same shape as s22→s23. On green + review-clean: POST the formal
  ...
  **decide and dispatch stage 9** from the live tree. Stage 9 MUST include: (a) the **Debugger
  ...
  opus children, one-2400s sizing, tada-only reporting), decomposed YOURSELF; park s26
  `--blocked --blocked-on <that orchestration>` carrying this spec with an updated Supervisor
  state. Remember the AF_UNIX short-path constraint when specifying any stage-9 `test:rust`
  child.
