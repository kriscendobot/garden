...
# Gauntlet stage: FIX round 1 — kriscendobot/minion.town PR #88

You are ONE stage of a staged gauntlet (kriscendobot-minion.town-pr88-gauntlet). Apply the panel's must-fix items ONCE,
...
1. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh kriscendobot-minion.town-pr88-gauntlet-fix-1 <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
2. Read the LATEST panel verdict on https://github.com/kriscendobot/minion.town/pull/88 (the request-changes `gh pr review` the
...
4. Watch CI to terminal, BOUNDED (same as the clean stage):
   `GARDEN_CI_DEADLINE_SECS=3600 \
     scripts/jobs/gardening/ci-wait-merge.sh kriscendobot/minion.town 88 --no-merge`
   - rc 0 (GREEN): success.
...
<!-- garden-transient-elapsed: kind=exit0 through=0 values=763 -->
