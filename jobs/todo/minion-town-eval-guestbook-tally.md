...
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-09-05T14:10:04Z cleared=none -->

# Evaluation 4/8: dynamic durable state — guestbook shared tally
...
## This evaluation (prefix: `ev4-`)

**Deliverable.** A second, differently-shaped test of server-authoritative
state, to see whether evaluation 3's finding generalizes: publish a
brand-new "guestbook" clip whose page shows a shared tally and a sign
action. Two DISTINCT visitors — separate Playwright browser contexts with no
shared cookies or storage — each perform the sign action; a third fresh
context then sees a tally of 2 (and both entries, if you render entries).
The difference from the odometer: state here is mutated by an explicit
visitor ACTION (a form post, a fetch — whatever the publishable surface
supports), not by mere page load, and must be read back coherently by a
different visitor than the ones who wrote it.

**Required verification.** One Playwright script driving all three contexts
end to end, asserting the third context observes both signatures; plus
`curl` evidence of the tally endpoint/page before and after. If the
publishable surface turns out to support no visitor-initiated mutation at
all, demonstrate the nearest expressible shape, and report the boundary with
