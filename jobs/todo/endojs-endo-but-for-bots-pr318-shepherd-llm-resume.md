# shepherd on endojs/endo-but-for-bots PR #318 (llm lint-ceiling resume)

Repo: endojs/endo-but-for-bots (bot-pushable; bot-repo only, no upstream endojs/endo touch).

PR #318 (base per its head) was blocked by the typescript-eslint project-service lint scaling
...
Task: rebase PR #318 onto current `origin/llm` and drive CI to green. Its base is live `llm`.
