...
# Gauntlet stage: CLEAN — kriscendobot/minion.town PR #24

You are ONE stage of a staged gauntlet (minion-town-ocapn-runahead-pin-gauntlet). Do ONLY the clean stage, then STOP.

1. Idempotence first. `gh pr view https://github.com/kriscendobot/minion.town/pull/24 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `/home/kris/garden2/scripts/jobs/ensure-project-worktree.sh minion-town-ocapn-runahead-pin-gauntlet-clean kriscendobot/minion.town <pr-head-branch>`
   (the head branch is `gh pr view https://github.com/kriscendobot/minion.town/pull/24 --json headRefName -q .headRefName`).
...
5. Watch CI to a terminal state, BOUNDED so this handler is never killed mid-wait:
   `GARDEN_CI_DEADLINE_SECS=3600 \
     /home/kris/garden2/scripts/jobs/gardening/ci-wait-merge.sh kriscendobot/minion.town 24 --no-merge`
   - rc 0 (GREEN): success.
