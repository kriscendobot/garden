...
Blocked on `minion-town-endo-daemon-pin-refresh-20260921` landing (the pinned
Endo daemon must actually carry `EndoGuest.accept` before this can be verified
...
2. Rebase the branch onto current `main` (it's `CONFLICTING`; the pin-refresh
   PR from the predecessor job should be merged into `main` by now — confirm
   before you start, and if it is not yet, re-park yourself blocked on it
   rather than proceeding against the stale pin).
