# shepherd on endojs/endo-but-for-bots PR #320 (llm lint-ceiling resume)

Repo: endojs/endo-but-for-bots (bot-pushable; bot-repo only, no upstream endojs/endo touch).

PR #320 (base per its head) was blocked by the typescript-eslint project-service lint scaling
...
Task: rebase PR #320 onto current `origin/llm` and drive CI to green. Its base is the frozen snapshot `llm-b1c3f4d`; unfreeze it to live `llm` as part of the rebase.
