...
# Gauntlet stage: CLEAN — endojs/endo-but-for-bots PR #969

You are ONE stage of a staged gauntlet (ironhorse-full-test262-automation-gauntlet). Do ONLY the clean stage, then STOP.

1. Idempotence first. `gh pr view https://github.com/endojs/endo-but-for-bots/pull/969 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `/home/kris/garden2/scripts/jobs/ensure-project-worktree.sh ironhorse-full-test262-automation-gauntlet-clean endojs/endo-but-for-bots <pr-head-branch>`
   (the head branch is `gh pr view https://github.com/endojs/endo-but-for-bots/pull/969 --json headRefName -q .headRefName`).
