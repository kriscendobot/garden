...
gauntlet: ebfb-pr977-gauntlet-rerun
...
# Gauntlet stage: CLEAN — endojs/endo-but-for-bots PR #977

You are ONE stage of a staged gauntlet (ebfb-pr977-gauntlet-rerun). Do ONLY the clean stage, then STOP.

1. Idempotence first. `gh pr view https://github.com/endojs/endo-but-for-bots/pull/977 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `/home/kris/garden/scripts/jobs/ensure-project-worktree.sh ebfb-pr977-gauntlet-rerun-clean endojs/endo-but-for-bots <pr-head-branch>`
   (the head branch is `gh pr view https://github.com/endojs/endo-but-for-bots/pull/977 --json headRefName -q .headRefName`).
...
4. If you changed anything, push follow-ups to the PR head with
   `/home/kris/garden/scripts/jobs/gardening/safe-push-pr-head.sh`.
5. Watch CI to a terminal state, BOUNDED so this handler is never killed mid-wait:
   `GARDEN_CI_DEADLINE_SECS=3600 \
     /home/kris/garden/scripts/jobs/gardening/ci-wait-merge.sh endojs/endo-but-for-bots 977 --no-merge`
   - rc 0 (GREEN): success.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-13T04:30:43Z
