...
# Review directive on kriscendobot/minion.town PR #37

A trusted maintainer/contributor REVIEW on #37. Treat the WHOLE review
...
Review: https://github.com/kriscendobot/minion.town/pull/37#discussion_r3781472038
...
trailing number in the Review URL above), each with its file:line + text:
  gh api --paginate repos/kriscendobot/minion.town/pulls/37/comments --jq '[.[]|select(.pull_request_review_id==REVIEW_ID)]'
and re-fetch the review body itself:
  gh api repos/kriscendobot/minion.town/pulls/37/reviews/REVIEW_ID --jq .body
...
Endo has its own ocap mail system. On the one hand, rate limiting, circuit breaking, junk filtering, and pay-to-post are generic patterns that should be implemented at the Endo level of abstraction so they can be brought to bear on all mail delivery. What we seek from this design

## BEFORE you edit — run the recheck preflight (deterministic)

A peer may have already resolved this feedback. Run, from the garden root:

  scripts/jobs/gardening/pr-feedback-preflight.sh kriscendobot/minion.town 37 3781472038 kriskowal
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-14T06:16:51Z
