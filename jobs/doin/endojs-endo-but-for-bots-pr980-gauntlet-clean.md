...
# Gauntlet stage: CLEAN — endojs/endo-but-for-bots PR #980

You are ONE stage of a staged gauntlet (endojs-endo-but-for-bots-pr980-gauntlet). Do ONLY the clean stage, then STOP.

1. Idempotence first. `gh pr view https://github.com/endojs/endo-but-for-bots/pull/980 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `/home/kris/garden2/scripts/jobs/ensure-project-worktree.sh endojs-endo-but-for-bots-pr980-gauntlet-clean endojs/endo-but-for-bots <pr-head-branch>`
   (the head branch is `gh pr view https://github.com/endojs/endo-but-for-bots/pull/980 --json headRefName -q .headRefName`).
...
5. Watch CI to a terminal state, BOUNDED so this handler is never killed mid-wait:
   `GARDEN_CI_DEADLINE_SECS=3600 \
     /home/kris/garden2/scripts/jobs/gardening/ci-wait-merge.sh endojs/endo-but-for-bots 980 --no-merge`
   - rc 0 (GREEN): success.
...
claim:
  ...
  provider: openai
  model: 
  claimed_at: 2026-08-14T12:59:11Z
