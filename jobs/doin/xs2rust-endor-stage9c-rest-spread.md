...
model: opus
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-07-18T08:46:03Z -->

---
model: opus
---
# Stage-9c child 1/9 — rest parameters + spread call (the `Unsupported("arguments")` surface)

**Provenance:** stage-9b's template-cache child (commit `38ed9c3549b`) landed the tagged-template
`template_cache` and named this as its honest remainder: the SES assert shim's **literal**
`Fail`/`details` definitions — `(t, ...s) => String.raw(t, ...s)` — still halt on
`Unsupported("arguments")`. Rest parameters and spread calls are also a prerequisite for the
`Reflect.apply`/`construct` trampolines (child 3 after you) and the eventual-send shim body
(child 5).

## The work

Implement, transliterated from and annotated against the XS C sources (`xsCode.c` codegen,
`xsRun.c` opcodes — the `XS_CODE_ARGUMENT*`/spread machinery):

1. **Rest parameters** — `function (t, ...s) {}` and arrow-function rest, binding the trailing
   arguments as a fresh array.
2. **Spread call** — `f(...args)` argument-list spreading (iterator-driven, per the oracle's
   semantics), including `new f(...args)` if it shares the machinery cleanly.
3. **Array-literal spread** (`[...xs]`) only if it falls out of the same iterator plumbing —
   otherwise name it an honest remainder; do not force it.

**Scope honesty:** rest params + spread call are the gate; land them first, push, then widen.

**Coverage:** grow the curated corpus (`cases/language/stage9c-rest-spread/` or similar) with
dual-run cases — rest arity/empty-rest, spread of arrays/iterables/strings, spread with holes,
throwing iterators (BothAbort agreement), spread-into-new. Bump `CORPUS_PROGRAM_COUNT`. If the
assert shim's literal `Fail`/`details` shapes are now dual-runnable, add a boot-gate step (or
convert an existing named skip) proving `(t, ...s) => String.raw(t, ...s)` agrees end-to-end.
...
claim:
  ...
  claimed_at: 2026-07-18T08:46:10Z
