...
# Gauntlet stage: FIX round 3 — kriscendobot/minion.town PR #48

You are ONE stage of a staged gauntlet (build-minion-town-git-content-substrate-gauntlet). Apply the panel's must-fix items ONCE,
...
1. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh build-minion-town-git-content-substrate-gauntlet-fix-3 <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
2. Read the LATEST panel verdict on https://github.com/kriscendobot/minion.town/pull/48 (the request-changes `gh pr review` the
...
4. Watch CI to terminal, BOUNDED (same as the clean stage):
   `GARDEN_CI_DEADLINE_SECS=3600 \
     scripts/jobs/gardening/ci-wait-merge.sh kriscendobot/minion.town 48 --no-merge`
   - rc 0 (GREEN): success.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-18T05:56:17Z
