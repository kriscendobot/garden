<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-07-03T12:56:30Z -->

---
model: opus
---
# Builder: xs2rust-endor stage 3 (4/7) — text, numbers, Math, JSON (PR #600)
...
You are child 4 of the serial `xs2rust-endor-build-stage3` orchestration. Ground truth
...
**Deliverable:** `String.prototype` over the CESU-8 chunk representation (indexing/
`charCodeAt`/slice-family/case/`indexOf`-family per the pin, with `XS_STRING_METERING`
and the chunk-byte metering exact); Number (constructor + statics + `toString` radix
paths); Math (every function, **canonical NaN** and the pin's exact libm-vs-inline
choices — determinism here is consensus-critical); `parseInt`/`parseFloat`; JSON
(`parse` — on oracle-compiled input per the stage-5 boundary — and `stringify`, with
XS's traversal order and metering).

**Acceptance bar:** `built-ins/{String,Number,Math,JSON}` dual-run sections: covered
agrees bit-exactly INCLUDING computrons, divergent **0**, skips named. Report per-section
before/after verbatim. Corpus programs exercising string building in loops (the metering
hot path), Math determinism corners (NaN canonicalization, -0), and JSON round-trips,
bit-exact.
...
claim:
  host: endolinbot2
  gardener: 17
  claimed_at: 2026-07-03T13:43:29Z
