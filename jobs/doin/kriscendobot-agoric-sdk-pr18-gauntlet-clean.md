...
# Gauntlet stage: CLEAN — kriscendobot/agoric-sdk PR #18

You are ONE stage of a staged gauntlet (kriscendobot-agoric-sdk-pr18-gauntlet). Do ONLY the clean stage, then STOP.

Garden script names below are repo-relative. Resolve them against THIS claiming
worker's `$GARDEN_ROOT` (known by `scripts/jobs/common.sh`), never against the
posting host's garden root.

1. Idempotence first. `gh pr view https://github.com/kriscendobot/agoric-sdk/pull/18 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh kriscendobot-agoric-sdk-pr18-gauntlet-clean <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
<!-- garden-transient-elapsed: kind=signature through=2 values=4,4 -->
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-31T22:58:57Z
