...
1. **DESIGN.** Post a `designer` job (`model: fable`) carrying the brief above, to open a design PR under
   `designs/`. (Prefix it with a `researcher` pass if that is the standing convention.)
...
outstanding, **persist your progress and re-post a continuation of yourself** (same basename,
`model: fable`, with a short state note of which stage you are in and what you are waiting on) so the
supervision survives a restart — never drop the lifecycle on the floor. Journal each stage transition.
...
claim:
  host: endolinbot2
  gardener: 10
  claimed_at: 2026-07-02T19:02:11Z
