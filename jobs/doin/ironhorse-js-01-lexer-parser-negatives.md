...
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=high at=2026-08-08T05:28:07Z cleared=none -->
...
---
handler-timeout: 7200
# Close lexer, parser, and early-error correctness gaps
...
`test/language/identifiers/**`, all `negative.phase=parse|resolution` cases in `test/language/**`, and `test/language/statements/{const,let,using,await-using}/syntax/**`.

Implementation scope:
Fix the Unicode identifier table/version behavior that over-accepted the 16 executed `start-unicode-*` cases. Close parser/scoper/coder early-error mismatches surfaced by child 00, including the `for (...; ...; i = ...)` parser/coder control-flow loop behind the three hangs. Keep compiler diagnostics typed enough for the harness to compare SyntaxError versus ReferenceError.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-08T05:28:11Z
