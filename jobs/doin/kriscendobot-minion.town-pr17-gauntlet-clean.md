...
# Gauntlet stage: CLEAN — kriscendobot/minion.town PR #17

You are ONE stage of a staged gauntlet (kriscendobot-minion.town-pr17-gauntlet). Do ONLY the clean stage, then STOP.

Garden script names below are repo-relative. Resolve them against THIS claiming
worker's `$GARDEN_ROOT` (known by `scripts/jobs/common.sh`), never against the
posting host's garden root.

1. Idempotence first. `gh pr view https://github.com/kriscendobot/minion.town/pull/17 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh kriscendobot-minion.town-pr17-gauntlet-clean <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-17T23:13:43Z
