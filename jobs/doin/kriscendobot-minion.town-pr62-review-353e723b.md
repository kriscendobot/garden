...
# Review directive on kriscendobot/minion.town PR #62

A trusted maintainer/contributor REVIEW on #62. Treat the WHOLE review
...
Review: https://github.com/kriscendobot/minion.town/pull/62#pullrequestreview-5072254610
...
trailing number in the Review URL above), each with its file:line + text:
  gh api --paginate repos/kriscendobot/minion.town/pulls/62/comments --jq '[.[]|select(.pull_request_review_id==REVIEW_ID)]'
and re-fetch the review body itself:
  gh api repos/kriscendobot/minion.town/pulls/62/reviews/REVIEW_ID --jq .body
...
[APPROVED] I believe there is also relevant code for provisioning guests with the `MAIN` worker that should be renamed `@main`. This may require upstream changes to endo for endowing indelible/special names. Please link the latest work toward that. 

## BEFORE you edit — run the recheck preflight (deterministic)

A peer may have already resolved this feedback. Run, from the garden root:

  scripts/jobs/gardening/pr-feedback-preflight.sh kriscendobot/minion.town 62 5072254610 kriskowal
...
claim:
  ...
  provider: openai
  model: 
  claimed_at: 2026-08-31T23:10:48Z
