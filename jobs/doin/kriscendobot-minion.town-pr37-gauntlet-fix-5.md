...
# Gauntlet stage: FIX round 5 — kriscendobot/minion.town PR #37

You are ONE stage of a staged gauntlet (kriscendobot-minion.town-pr37-gauntlet). Apply the panel's must-fix items ONCE,
...
1. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh kriscendobot-minion.town-pr37-gauntlet-fix-5 <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
2. Read the LATEST panel verdict on https://github.com/kriscendobot/minion.town/pull/37 (the request-changes `gh pr review` the
...
4. Watch CI to terminal, BOUNDED (same as the clean stage):
   `GARDEN_CI_DEADLINE_SECS=3600 \
     scripts/jobs/gardening/ci-wait-merge.sh kriscendobot/minion.town 37 --no-merge`
   - rc 0 (GREEN): success.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-19T02:47:14Z
