...
You are ONE stage of a staged gauntlet (ebfb-guest-unconfined-from-tree-gauntlet). Do ONLY the clean stage, then STOP.
...
2. Get an ISOLATED project checkout of the PR head:
   `/home/kris/garden2/scripts/jobs/ensure-project-worktree.sh ebfb-guest-unconfined-from-tree-gauntlet-clean endojs/endo-but-for-bots <pr-head-branch>`
...
5. Watch CI to a terminal state, BOUNDED so this handler is never killed mid-wait:
   `GARDEN_CI_DEADLINE_SECS=3600 \
     /home/kris/garden2/scripts/jobs/gardening/ci-wait-merge.sh endojs/endo-but-for-bots 977 --no-merge`
   - rc 0 (GREEN): success.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-13T02:56:07Z
