...
dispatch: automatic
---
# Review directive on kriscendobot/minion.town PR #34

A trusted maintainer/contributor REVIEW on #34. Treat the WHOLE review
...
Review: https://github.com/kriscendobot/minion.town/pull/34#pullrequestreview-4901573048
...
trailing number in the Review URL above), each with its file:line + text:
  gh api --paginate repos/kriscendobot/minion.town/pulls/34/comments --jq '[.[]|select(.pull_request_review_id==REVIEW_ID)]'
and re-fetch the review body itself:
  gh api repos/kriscendobot/minion.town/pulls/34/reviews/REVIEW_ID --jq .body
...
[APPROVED] Approved. Please follow-up with the DNS record needed on the `ocap.site` nameserver to direct traffic to the gateway. Proceed to build, deploy, and validate. 

## BEFORE you edit — run the recheck preflight (deterministic)

A peer may have already resolved this feedback. Run, from the garden root:

  scripts/jobs/gardening/pr-feedback-preflight.sh kriscendobot/minion.town 34 4901573048 kriskowal
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-10T22:56:57Z
