...
# Gauntlet stage: PANEL round 6 — endojs/endo-but-for-bots PR #1281

You are ONE stage of a staged gauntlet (ses-node26-lockdown-permits-gauntlet). Run EXACTLY ONE panel round, post the
...
1. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh ses-node26-lockdown-permits-gauntlet-panel-6 <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
2. Run the panel in SINGLE-ROUND mode against that worktree:
   `GARDEN_PANEL_SINGLE_ROUND=1 \
     scripts/jobs/gardening/panel.sh <worktree> 1281 <base-ref>`
...
3. Post the aggregate (in $GARDEN_PANEL_RUNDIR) as a `gh pr review` on https://github.com/endojs/endo-but-for-bots/pull/1281 — the
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-09-16T02:47:17Z
