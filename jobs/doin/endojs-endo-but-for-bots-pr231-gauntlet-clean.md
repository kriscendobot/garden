...
# Gauntlet stage: CLEAN — endojs/endo-but-for-bots PR #231

You are ONE stage of a staged gauntlet (endojs-endo-but-for-bots-pr231-gauntlet). Do ONLY the clean stage, then STOP.

Garden script names below are repo-relative. Resolve them against THIS claiming
worker's `$GARDEN_ROOT` (known by `scripts/jobs/common.sh`), never against the
posting host's garden root.

1. Idempotence first. `gh pr view https://github.com/endojs/endo-but-for-bots/pull/231 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh endojs-endo-but-for-bots-pr231-gauntlet-clean <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
<!-- garden-transient-elapsed: kind=exit0 through=0 values=526 -->
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-31T03:33:49Z
