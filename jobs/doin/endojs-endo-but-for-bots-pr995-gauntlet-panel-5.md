...
# Gauntlet stage: PANEL round 5 — endojs/endo-but-for-bots PR #995

You are ONE stage of a staged gauntlet (endojs-endo-but-for-bots-pr995-gauntlet). Run EXACTLY ONE panel round, post the
...
1. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh endojs-endo-but-for-bots-pr995-gauntlet-panel-5 <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
2. Run the panel in SINGLE-ROUND mode against that worktree:
   `GARDEN_PANEL_SINGLE_ROUND=1 \
     scripts/jobs/gardening/panel.sh <worktree> 995 <base-ref>`
...
3. Post the aggregate (in $GARDEN_PANEL_RUNDIR) as a `gh pr review` on https://github.com/endojs/endo-but-for-bots/pull/995 — the
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-16T09:26:10Z
