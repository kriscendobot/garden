...
# Gauntlet stage: CLEAN — kriscendobot/minion.town PR #93

You are ONE stage of a staged gauntlet (minion-town-clip-content-store-gc-build-gauntlet). Do ONLY the clean stage, then STOP.

Garden script names below are repo-relative. Resolve them against THIS claiming
worker's `$GARDEN_ROOT` (known by `scripts/jobs/common.sh`), never against the
posting host's garden root.

1. Idempotence first. `gh pr view https://github.com/kriscendobot/minion.town/pull/93 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh minion-town-clip-content-store-gc-build-gauntlet-clean <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-09-04T23:23:24Z
