...
dispatch: automatic
---
# Implement Intl.NumberFormat (value/metering parity) — close the intl402/NumberFormat slice

**Family slice:** `intl402/NumberFormat` (~221 cases) plus the `Number.prototype.toLocaleString` / `BigInt.prototype.toLocaleString` bridges that route through NumberFormat. **Run FIRST** — this is foundational for the toLocaleString bridges and DurationFormat.

**Measured diagnosis:** `typeof Intl.NumberFormat === 'undefined'` — the constructor is entirely unimplemented. `new Intl.NumberFormat('en').format(1234.5)` → Ironhorse throws `TypeError` (undefined is not a constructor) → `abort-value-differs`. `(12345.678).toLocaleString('en')` → `OracleOnlyComplete` (XS HAS a default `Number.prototype.toLocaleString`) while Ironhorse throws `TypeError` → `ironhorse-aborted`.

**Deliverable:**
- Register `Intl.NumberFormat` in `create_intl` and implement the constructor + prototype: `format` (bound getter, per spec), `formatToParts`, `formatRange`, `formatRangeToParts`, `resolvedOptions`, and static `supportedLocalesOf`, with full option processing: `style` (decimal/currency/percent/unit), `currency`/`currencyDisplay`/`currencySign`, `unit`/`unitDisplay`, `notation` (standard/scientific/engineering/compact) + `compactDisplay`, `signDisplay`, `useGrouping`, `minimumIntegerDigits`, min/max fraction & significant digits, `roundingIncrement`, `roundingMode`, `roundingPriority`, `trailingZeroDisplay`. Drive every `intl402/NumberFormat` case to `IronhorseOnlyComplete` with correct values.
- Bridge `Number.prototype.toLocaleString` and `BigInt.prototype.toLocaleString` through the same formatter. Where XS's default (no-Intl) `toLocaleString` completes, certify those as genuine **`covered`** (BothComplete); where the case observes Intl-specific formatting, they land in `oracle-host-missing-intl`.
- Add Rust regression tests locking exact values for representative option matrices (decimal grouping, currency USD/EUR/JPY, percent, unit, scientific/compact notation, signDisplay, significant-digit rounding, formatToParts, resolvedOptions key order).

**Note:** this is the largest single family; if over budget, sub-decompose (e.g. `numberformat-core-decimal`, `numberformat-currency-unit`, `numberformat-notation-rounding`, `numberformat-tolocalestring-bridge`) under a nested halt-on-failure orchestration and hand off.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-15T03:33:13Z
