...
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-09-05T14:13:04Z cleared=none -->

# Evaluation 5/8: `evaluate` sandbox boundary
...
## This evaluation (prefix: `ev5-`)

**Deliverable.** A capability map of `evaluate`'s sandbox boundary, graded
against what its own description communicates. Produce a table: capability
attempted → observed result → did the tool description predict it? Probe at
minimum, each with a small bounded expression:

- Ordinary ECMAScript builtins and whether hardened-JavaScript artifacts are
  visible (`harden`, `Compartment`, frozen intrinsics — does mutating
  `Array.prototype` throw?).
- Ambient authority: `fetch`/network, `process`/filesystem, `require` or
  dynamic `import`, timers (`setTimeout`), `Date.now()` and `Math.random()`
  behavior (deterministic? censored? live?).
- Whatever endowment/argument mechanism the schema exposes: can a value from
  the guest's own name store be passed in, and do mutations made inside the
  evaluation persist back to the store?
- Result marshalling: return a plain object, a function, a Promise, and
  something deliberately unserializable — record each result or error shape.
- Error legibility: throw an exception inside evaluated source and record
  exactly what the caller sees (message, stack or its absence).

**Restraint (live shared daemon).** No unbounded loops, no large
allocations, no probes intended to exhaust or crash the worker. Keep every
probe's compute to roughly a second. This evaluation maps the boundary; it
does not stress-test it.

**Required verification.** The table itself, with literal expressions and
literal results. Cross-check at least one negative finding (e.g. no `fetch`)
twice with differently-phrased probes so the report does not rest on one
misread error.
