...
# Gauntlet stage: FIX round 1 — kriscendobot/minion.town PR #81

You are ONE stage of a staged gauntlet (build-minion-town-invitation-only-guest-onboarding-gauntlet). Apply the panel's must-fix items ONCE,
...
1. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh build-minion-town-invitation-only-guest-onboarding-gauntlet-fix-1 <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
2. Read the LATEST panel verdict on https://github.com/kriscendobot/minion.town/pull/81 (the request-changes `gh pr review` the
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-09-02T09:31:02Z
