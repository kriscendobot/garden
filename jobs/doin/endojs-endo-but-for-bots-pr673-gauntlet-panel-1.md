...
# Gauntlet stage: PANEL round 1 — endojs/endo-but-for-bots PR #673

You are ONE stage of a staged gauntlet (endojs-endo-but-for-bots-pr673-gauntlet). Run EXACTLY ONE panel round, post the
...
1. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh endojs-endo-but-for-bots-pr673-gauntlet-panel-1 <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
2. Run the panel in SINGLE-ROUND mode against that worktree:
   `GARDEN_PANEL_SINGLE_ROUND=1 \
     scripts/jobs/gardening/panel.sh <worktree> 673 <base-ref>`
...
3. Post the aggregate (in $GARDEN_PANEL_RUNDIR) as a `gh pr review` on https://github.com/endojs/endo-but-for-bots/pull/673 — the
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-31T22:37:16Z
