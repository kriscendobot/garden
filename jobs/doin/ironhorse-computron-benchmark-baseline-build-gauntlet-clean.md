...
You are ONE stage of a staged gauntlet (ironhorse-computron-benchmark-baseline-build-gauntlet). Do ONLY the clean stage, then STOP.
...
2. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh ironhorse-computron-benchmark-baseline-build-gauntlet-clean <pr-head-owner>/<repo-name> <pr-head-branch>`.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-09-16T05:41:20Z
