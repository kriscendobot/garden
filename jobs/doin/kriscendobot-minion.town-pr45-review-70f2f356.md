...
# Review directive on kriscendobot/minion.town PR #45

A trusted maintainer/contributor REVIEW on #45. Treat the WHOLE review
...
Review: https://github.com/kriscendobot/minion.town/pull/45#pullrequestreview-5119105749
...
trailing number in the Review URL above), each with its file:line + text:
  gh api --paginate repos/kriscendobot/minion.town/pulls/45/comments --jq '[.[]|select(.pull_request_review_id==REVIEW_ID)]'
and re-fetch the review body itself:
  gh api repos/kriscendobot/minion.town/pulls/45/reviews/REVIEW_ID --jq .body
...
[CHANGES_REQUESTED] @kriscendobot We use `ava` in this house. Please also propose how this will integrate with `@endo/ertp`. The execution environment for billing will be an Endo confined worker, so please layer this architecture such that we are using Endo as the durable persist

## BEFORE you edit — run the recheck preflight (deterministic)

A peer may have already resolved this feedback. Run, from the garden root:

  scripts/jobs/gardening/pr-feedback-preflight.sh kriscendobot/minion.town 45 5119105749 kriskowal
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-09-05T01:05:19Z
