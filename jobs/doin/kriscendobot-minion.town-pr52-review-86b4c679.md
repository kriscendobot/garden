...
# Review directive on kriscendobot/minion.town PR #52

A trusted maintainer/contributor REVIEW on #52. Treat the WHOLE review
...
Review: https://github.com/kriscendobot/minion.town/pull/52#pullrequestreview-5024688157
...
trailing number in the Review URL above), each with its file:line + text:
  gh api --paginate repos/kriscendobot/minion.town/pulls/52/comments --jq '[.[]|select(.pull_request_review_id==REVIEW_ID)]'
and re-fetch the review body itself:
  gh api repos/kriscendobot/minion.town/pulls/52/reviews/REVIEW_ID --jq .body
...
[INLINE-REVIEW] [APPROVED] This seems incomplete. I’m expecting to see a caplet `export const make` function that gets used to introduce an unconfined caplet to the daemon as a basis for endowing the sites registry to the host and consequently each subsequent guest as `@sites`.

## BEFORE you edit — run the recheck preflight (deterministic)

A peer may have already resolved this feedback. Run, from the garden root:

  scripts/jobs/gardening/pr-feedback-preflight.sh kriscendobot/minion.town 52 5024688157 kriskowal
...
claim:
  ...
  provider: openai
  model: 
  claimed_at: 2026-08-27T05:13:05Z
