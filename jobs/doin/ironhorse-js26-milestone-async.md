...
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-08-25T12:22:03Z cleared=none -->
...
---
handler-timeout: 14000
# js-26 MILESTONE — async residue (fromAsync + async-generator/for-await + Promise + await/AsyncFunction)

Consolidates the four `ironhorse-js-26-ch-async-fromasync-*` children
(`a-residual`, `b-asyncgen`, `c-promise`, `d-await`) into ONE per-family milestone with a
real budget, minted by `ironhorse-js26-milestone-consolidation` (2026-08-22).

## Ordered scope (prerequisite-first — the async machinery is shared)
1. **`Array.fromAsync` residual** (~18 remaining cases): close the last pre-existing engine
   gaps in the fromAsync async-iteration protocol (sync-iterable fallback, await of each
   value, mapper invocation, abrupt-completion close).
2. **async-generator + `for-await-of` + async-dispose** residue: the shared async-iteration
   plumbing the rest of the family leans on — resolve this before the leaf residuals.
3. **`built-ins/Promise`** residue: constructor/resolve/reject/then/all/allSettled/any/race
   residual observable behaviors.
4. **`await` / `async function` language residue + `AsyncFunction`**: await suspension/resume
   observable semantics and the `AsyncFunction` constructor/metadata.

Order matters: (2) is the prerequisite the fromAsync and await leaves cascade off. Land the
shared async-iteration/await plumbing first, then the leaves clear more cheaply.
...
claim:
  ...
  provider: openai
  model: 
  claimed_at: 2026-08-25T12:22:08Z
