...
model: opus
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-07-18T13:04:04Z -->

---
model: opus
---
# Stage-10 child 1/7 — function `.prototype` reads (+ own-property surface on function objects)

**Provenance:** stage-9c child 5 (handled-promise shim body, commit `85131468b1`) found and named
this as a prerequisite: **reading a function's `.prototype` own property returns `undefined` on
endor** — intrinsic or user function alike. `Promise.prototype` reading `undefined` makes the
shim's `isSafePromise` (`getPrototypeOf(p) === Promise.prototype`) vacuously false, and any SES
bundle boot will trip over it early and often. Same family: `Reflect.ownKeys(fn)` misses
`length`/`name`/`prototype`.

## The work

Transliterate from the XS C sources (`xsFunction.c` — `fxGetPrototypeFromConstructor`, the
function-instance property surface):

1. **`.prototype` reads** on user functions (the auto-minted prototype object with its
   `constructor` back-link) and on intrinsic constructors (`Promise.prototype`,
   `Object.prototype`, etc. — identity-equal to the real intrinsic prototype objects endor
   already holds).
2. **`.length` and `.name` reads** on function objects where the soundness gate permits
   (endor knows both from `FuncInfo`); keep the known uniform intrinsic `.name` gap honest if
   it cannot be closed cleanly — do not fake values.
3. **`Reflect.ownKeys`/`Object.getOwnPropertyNames` over functions** listing what is now
   readable, only if it falls out of the same surface cleanly; otherwise name it a remainder.

Watch metering: property reads on function exotics go through the same metered GET path; keep
dual-run bit-exactness (result AND computron) on every covered form, or gate what cannot agree.

**Coverage:** curated corpus `cases/language/stage10-function-prototype/` with dual-run cases
(user fn `.prototype`, `constructor` round-trip, intrinsic ctor `.prototype` identity via
`getPrototypeOf(new C())`, `Promise.prototype` identity, arrow functions having NO prototype);
bump `CORPUS_PROGRAM_COUNT`. Add/convert a boot-gate step if a named skip now greens.
...
claim:
  ...
  claimed_at: 2026-07-18T13:04:08Z
