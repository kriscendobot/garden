...
# Gauntlet stage: FIX round 1 — kriscendobot/minion.town PR #17

You are ONE stage of a staged gauntlet (kriscendobot-minion.town-pr17-gauntlet). Apply the panel's must-fix items ONCE,
...
1. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh kriscendobot-minion.town-pr17-gauntlet-fix-1 <pr-head-owner>/<repo-name> <pr-head-branch>`.
   Resolve the head owner and branch with `gh pr view https://github.com/kriscendobot/minion.town/pull/17 --json headRepositoryOwner,headRefName`;
   do not pass the base repo when the PR head belongs to a fork.
2. Read the LATEST panel verdict on https://github.com/kriscendobot/minion.town/pull/17 (the request-changes `gh pr review` the
...
4. Watch CI to terminal, BOUNDED (same as the clean stage):
   `GARDEN_CI_DEADLINE_SECS=3600 \
     scripts/jobs/gardening/ci-wait-merge.sh kriscendobot/minion.town 17 --no-merge`
   - rc 0 (GREEN): success.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-17T23:44:28Z
