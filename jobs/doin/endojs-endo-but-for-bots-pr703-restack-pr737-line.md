...
Repo: endojs/endo-but-for-bots. PR: https://github.com/endojs/endo-but-for-bots/pull/703
(head `build/sturdyref-bridge-5-foreign-internalization`, base `build/sturdyref-bridge-4-ocapn-singleton`).
...
part of that orchestration and still sit on the pre-restack line (compare shows #703
...
Task (weaver):
1. Verify the predecessor landed: `build/sturdyref-bridge-4-ocapn-singleton` head descends
   from `build/sturdyref-bridge-1-bytes-wire-read`'s restacked head. Identify #703's OLD
   merge-base (the old bridge-4 tip its own commits sit on) — e.g. via
   `git merge-base` / `git range-diff` — so the rebase carries ONLY #703's own commits.
2. `git rebase --onto origin/build/sturdyref-bridge-4-ocapn-singleton <old-base>
   build/sturdyref-bridge-5-foreign-internalization` in an ISOLATED project worktree keyed by
   THIS job's base (`scripts/jobs/ensure-project-worktree.sh <base> endojs/endo-but-for-bots
   build/sturdyref-bridge-5-foreign-internalization`).
...
4. GitHub base unchanged (it already names bridge-4). Push `--force-with-lease`; keep DRAFT;
   comment the new head + test evidence on #703.
5. CONFINEMENT TESTS ARE LOAD-BEARING: keep the sturdyref confinement suites green
   ...
   reveal off the surface; unlinkability) plus cut 5's own foreign-internalization facet-seam
