...
tier: mentor
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-09-16T11:04:04Z cleared=none -->
...
dispatch: automatic
---
Rebase PR #774 (build/sturdyref-shim-first-wins) onto live `llm` as part of the sturdyref stack modernization.

THIS STEP'S PARAMETERS:
  PR:               #774
  BRANCH:           build/sturdyref-shim-first-wins
  OLD-OWN-HEAD-SHA: 1fb7a203a4333e94cafc2309b3a9b0dca3e0e31e
  NEW-PARENT-REF:   origin/llm   (the already-rebased parent; fetch it fresh)
  OLD-PARENT-HEAD-SHA: 103faab721eb197e3b902ca0f751d1511edabf3f

  Rebase command:
      git fetch origin
      git rebase --onto origin/llm 103faab721eb197e3b902ca0f751d1511edabf3f 1fb7a203a4333e94cafc2309b3a9b0dca3e0e31e
  Force-push:
      git push --force-with-lease=build/sturdyref-shim-first-wins:1fb7a203a4333e94cafc2309b3a9b0dca3e0e31e origin HEAD:build/sturdyref-shim-first-wins

BASE-FIELD FIX (required): #774's PR base field currently points at the snapshot
`llm-da209e5-endo-ascii`. After the rebase + force-push, set it to live llm:
    gh pr edit 774 --repo endojs/endo-but-for-bots --base llm
#774 is almost entirely NEW files (packages/sturdyref/*); only tsconfig.composite.json
and yarn.lock overlap llm, so conflicts should be minimal.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-09-16T11:04:18Z
