...
# Gauntlet stage: CLEAN — kriscendobot/garden PR #73

You are ONE stage of a staged gauntlet (kriscendobot-garden-pr73-gauntlet). Do ONLY the clean stage, then STOP.

Garden script names below are repo-relative. Resolve them against THIS claiming
worker's `$GARDEN_ROOT` (known by `scripts/jobs/common.sh`), never against the
posting host's garden root.

1. Idempotence first. `gh pr view https://github.com/kriscendobot/garden/pull/73 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `scripts/jobs/ensure-project-worktree.sh kriscendobot-garden-pr73-gauntlet-clean <pr-head-owner>/<repo-name> <pr-head-branch>`.
   Resolve the head owner and branch with `gh pr view https://github.com/kriscendobot/garden/pull/73 --json headRepositoryOwner,headRefName`;
...
5. Watch CI to a terminal state, BOUNDED so this handler is never killed mid-wait:
   `GARDEN_CI_DEADLINE_SECS=3600 \
     scripts/jobs/gardening/ci-wait-merge.sh kriscendobot/garden 73 --no-merge`
   - rc 0 (GREEN): success.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-09-04T05:36:02Z
