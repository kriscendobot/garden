...
# Gauntlet stage: CLEAN — kriscendobot/finbot PR #5

You are ONE stage of a staged gauntlet (finbot-progress-20260730-020502-gauntlet). Do ONLY the clean stage, then STOP.

1. Idempotence first. `gh pr view https://github.com/kriscendobot/finbot/pull/5 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `/home/kris/garden2/scripts/jobs/ensure-project-worktree.sh finbot-progress-20260730-020502-gauntlet-clean kriscendobot/finbot <pr-head-branch>`
   (the head branch is `gh pr view https://github.com/kriscendobot/finbot/pull/5 --json headRefName -q .headRefName`).
...
5. Watch CI to a terminal state, BOUNDED so this handler is never killed mid-wait:
   `GARDEN_CI_DEADLINE_SECS=3600 \
     /home/kris/garden2/scripts/jobs/gardening/ci-wait-merge.sh kriscendobot/finbot 5 --no-merge`
   - rc 0 (GREEN): success.
...
claim:
  ...
  provider: anthropic
  model: 
  claimed_at: 2026-08-01T09:51:08Z
