...
# Review directive on endojs/endo-but-for-bots PR #1226

A trusted maintainer/contributor REVIEW on #1226. Treat the WHOLE review
...
Review: https://github.com/endojs/endo-but-for-bots/pull/1226#pullrequestreview-5299330782
...
trailing number in the Review URL above), each with its file:line + text:
  gh api --paginate repos/endojs/endo-but-for-bots/pulls/1226/comments --jq '[.[]|select(.pull_request_review_id==REVIEW_ID)]'
and re-fetch the review body itself:
  gh api repos/endojs/endo-but-for-bots/pulls/1226/reviews/REVIEW_ID --jq .body
...
[INLINE-REVIEW] [CHANGES_REQUESTED] @kriscendobot The garden itself recently upgraded its `claude` `-p` to JSONLD output stream and also began to parse out more of the metadata that is useful for programmatic usage of the `claude` child process. Please take these details into acc

## BEFORE you edit — run the recheck preflight (deterministic)

A peer may have already resolved this feedback. Run, from the garden root:

  scripts/jobs/gardening/pr-feedback-preflight.sh endojs/endo-but-for-bots 1226 5299330782 kriskowal
...
claim:
  ...
  provider: openai
  model: 
  claimed_at: 2026-09-24T03:30:10Z
