...
# Gauntlet stage: CLEAN — endojs/endo-but-for-bots PR #986

You are ONE stage of a staged gauntlet (endojs-endo-but-for-bots-pr986-gauntlet). Do ONLY the clean stage, then STOP.

1. Idempotence first. `gh pr view https://github.com/endojs/endo-but-for-bots/pull/986 --json isDraft,state,statusCheckRollup`. If the
...
2. Get an ISOLATED project checkout of the PR head:
   `/home/kris/garden2/scripts/jobs/ensure-project-worktree.sh endojs-endo-but-for-bots-pr986-gauntlet-clean endojs/endo-but-for-bots <pr-head-branch>`
   (the head branch is `gh pr view https://github.com/endojs/endo-but-for-bots/pull/986 --json headRefName -q .headRefName`).
...
claim:
  ...
  provider: openai
  model: 
  claimed_at: 2026-08-14T06:44:45Z
