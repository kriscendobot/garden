...
model: opus
---
<!-- garden-promoted-from-plan: gate=orchestrated priority=normal at=2026-07-06T12:19:05Z -->

---
model: opus
---
# Stage-4 child: generator functions and the iteration protocol closure
...
## Scope (child 3/8 — depends on child 2 for generator methods in classes)

Port generators (the pin's `xsGenerator.c` sync half):

- `XS_CODE_GENERATOR_FUNCTION`, `yield`, `yield*`: generator instance state (suspended-start /
  suspended-yield / executing / completed) as heap state in the slot arena, frame save/resume of
  the interpreter activation (stack segment + pc + scope) — design this deliberately: it is the
  same machinery async/await (child 4) resumes on, so keep the suspended-activation
  representation reusable.
- `%GeneratorPrototype%` `next`/`return`/`throw` full semantics (including throw-into-suspended,
  return-with-finally unwinding through the catch/finally jump chain).
- `yield*` delegation to arbitrary iterables (protocol-faithful: `throw`/`return` forwarding).
- Generator method forms: `function*` declarations/expressions, object-literal `*m()`, class
  `*m()` (on child 2).
- **Metering**: suspension/resumption cost as endor's own calibrated frozen entries (deterministic
  per release; oracle computron agreement advisory).

## Acceptance focus

Dual-run `built-ins/GeneratorFunction`, `built-ins/GeneratorPrototype` (if present in the tree:
