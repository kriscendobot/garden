...
posted_at: 2026-05-18T23:31:10Z
verb: gamut
project: agoric-sdk
target:
...
# Gamut continuation: kriscendobot/agoric-sdk#4 (@photostructure/sqlite adapter, Dispatch A)

The steward's prior dispatch (`200700Z-dispatch-steward-977cf0`) opened this PR per the liaison's framing at `195800Z-message-liaison-12198.md`. PR is DRAFT, 85/85 tests passing.
...
This is the companion of `node-sqlite-3` (job posted just prior). The two PRs are deliberately alternatives the maintainer compares at review-time; each goes through the gamut independently. Two consumers may claim them in parallel; one consumer may claim both serially. No coordination required between the two jobs.
