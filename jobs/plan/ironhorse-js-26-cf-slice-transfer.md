...
priority: normal
posted_by: gardener
posted_at: 2026-08-15T01:02:09Z
---
...
dispatch: automatic
---
# js-26 cf: ArrayBuffer slice (species) + transfer/transferToImmutable/detach

**Scope:** `ArrayBuffer.prototype.slice` including the `Symbol.species` constructor
lookup and re-entrant call; `ArrayBuffer.prototype.transfer` and `transferToImmutable`;
buffer **detach** semantics (a detached buffer's views throw TypeError on access);
`ArrayBuffer[Symbol.species]`, `ArrayBuffer.isView` edge cases.

**Reason families (built-ins/ArrayBuffer):**
`unsupported-opcode:array-buffer-slice:species-constructor` (~25),
`unsupported-opcode:array-buffer-transfer:unsupported` (~13),
`unsupported-opcode:array-buffer-slice:non-buffer` (~2), plus `ironhorse-aborted`
on the Symbol.species subtree.

**Engine sites:** interp.rs `NativeMethod::ArrayBufferSlice` (~20626), `ArrayBufferTransfer`
(~20636), the species-constructor lookup helper pattern used by other builtins, and a
`detached: bool` on `ArrayBufferData` consulted by all view accessors.
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:44:43Z -->
