...
priority: normal
posted_by: producer
posted_at: 2026-07-06T03:43:41Z
---

---
model: opus
---
# Stage-4 child: async/await over the job queue + the promise double-settle keystone
...
## Scope (child 4/8 — depends on child 3's suspended-activation machinery)

Complete the async surface over the stage-3 job queue (pump-loop latch semantics;
`daemon-rust-xs-performance.md` is ground truth):

- **THE KEYSTONE (s7 review ledger): the promise native-handler double-settle calibration.**
  The stage-3 promises child left thenable adoption and the combinators blocked on calibrating
  the pin's double-settle behavior of native handlers. Resolve it first — instrument the pin,
  calibrate endor's own frozen costs, and unblock: thenable adoption (resolve-with-thenable job),
  `Promise.all/allSettled/race/any`, `Promise.prototype.finally`, long `then`-chains.
- `XS_CODE_ASYNC_FUNCTION`, `await`: async function instances suspending on the reusable
  activation machinery, resumption jobs enqueued on the job queue, try/catch across await,
  async arrow functions if the compiler emits them distinctly.
- `XS_CODE_ASYNC_GENERATOR_FUNCTION`, `for-await-of`, async-iterator protocol
  (`%AsyncGeneratorPrototype%` next/return/throw with the request queue) — if this half does not
  fit the invocation, it is the designated scope fold: named skips
  (`async-generator`, `for-await-of`), report to s9.
- Unhandled-rejection surfacing consistent with the pin's host-boundary behavior.

## Acceptance focus

Dual-run `built-ins/Promise` (7 baseline — this child should grow it substantially: `all/`,
`race/`, `any/`, `allSettled/`, `prototype/then/`, `prototype/finally/` subtrees),
