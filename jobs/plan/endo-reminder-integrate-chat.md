...
role: designer
posted_by: liaison
posted_at: 2026-07-29T01:28:16Z
tier: mentor
fallback-tier: minion
dispatch: automatic
---

# Plan: integrate @endo/reminder into Chat
...
**Chat.** In the fork this is the `chat` package (note the neighbours
`goblin-chat`, `space-chat`, and `chat-network-view`: establish which one the
maintainer means by "Chat" and say so explicitly rather than assuming, since
picking the wrong one invalidates the plan).

Work out what a reminder means in a chat context: who schedules it, who receives
the fired message, how it survives a restart, and how it interacts with whatever
message plumbing Chat already has.
