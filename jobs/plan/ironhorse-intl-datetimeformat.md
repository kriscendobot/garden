...
role: builder
posted_by: producer
posted_at: 2026-08-15T01:24:36Z
---
...
dispatch: automatic
---
# Complete Intl.DateTimeFormat — close the intl402/DateTimeFormat residual slice

**Family slice:** `intl402/DateTimeFormat` (~106 residual cases).

**Measured diagnosis:** DateTimeFormat is PARTIAL — `new Intl.DateTimeFormat('en').format(0)` → `'1/1/1970'` completes, and `resolvedOptions().year` works. But 106 cases still `abort-value-differs`: Ironhorse throws on some option/method path. Triage each residual with `dual_run` and close.

**Deliverable:**
- Close the residual DateTimeFormat surface: `dateStyle`/`timeStyle`, `formatToParts`, `formatRange`/`formatRangeToParts`, `hourCycle`/`hour12`, `timeZone` handling, the component options (`era`, `weekday`, `year`, `month`, `day`, `hour`, `minute`, `second`, `fractionalSecondDigits`, `timeZoneName`, `dayPeriod`), option-validation/abrupt-completion ordering, and `resolvedOptions` completeness. Drive every case to `IronhorseOnlyComplete` with correct values.
- Lock exact values with Rust regression tests for the newly-closed paths.
...
<!-- garden-annotation: key=67344d1d5edd by=producer at=2026-08-16T06:49:18Z -->
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:41:56Z -->
