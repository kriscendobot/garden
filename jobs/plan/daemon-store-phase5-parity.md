...
role: builder
posted_by: producer
posted_at: 2026-07-21T23:24:35Z
---

role: builder

# Build Phase 5: parity polish for the persistent store family (design Phase 5)

Repo: endojs/endo-but-for-bots. Implement **Phase 5** on top of Phases 1-4: the
`@agoric/store` parity polish that the earlier phases deferred.

Scope (see design § Phased Phase 5):
- `addAll` / `clear` bulk mutators across the applicable kinds;
- lazy iterators for `keys`/`values`/`entries` (avoid materializing whole result
  arrays);
- OPTIONAL multiplayer replication via the synced-store substrate ONLY if the
  design still calls for it and it is low-risk; otherwise leave replication as a
  tracked follow-up and say so in the PR rather than forcing it.

## Tests
bulk-mutator semantics incl. throw-conditions; lazy-iterator correctness and
early-termination; restart persistence unaffected by the new surface.
