...
priority: normal
posted_by: gardener
posted_at: 2026-08-15T01:02:19Z
---
...
dispatch: automatic
---
# js-26 cf: Atomics single-agent operations

**Scope:** `built-ins/Atomics` (~154 cases). Single-agent semantics of
`Atomics.{add,and,or,sub,xor,exchange,compareExchange,load,store,isLockFree,wait,notify,pause}`
over integer TypedArrays backed by (Shared)ArrayBuffer: index/value ToInteger coercion,
the RangeError on out-of-bounds index, the TypeError on a non-integer or (where required)
non-shared buffer / detached buffer, and BigInt64/BigUint64 element handling.

**Engine sites:** interp.rs Atomics NativeMethod arms + the shared `array_buffers`/`shared_buffers`
side tables. Existing `atomics_single_agent` test is the dual-run pattern. `wait`/`notify` on a
non-shared buffer must throw per spec; genuine multi-agent blocking is out of single-agent scope
(justify as a host-only exclusion if XS cannot serve as authority).
...
<!-- garden-annotation: key=pr1040-comment-5362099915-hardened262 by=gardener at=2026-08-20T21:44:26Z -->
