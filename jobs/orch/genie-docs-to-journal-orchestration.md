...
state: pending
created_by: producer
created_at: 2026-08-13T21:38:13Z
---
...
1. `genie-docs-01-migrate-into-journal` — copy in, preserve cross-links, record
   provenance, count-match.
2. `genie-docs-02-delete-from-llm` — delete from the branch via a PR, only after
...
Stage 1 additionally waits on `garden-tada-shard-orchestration` completing, per
the maintainer's sequencing.
