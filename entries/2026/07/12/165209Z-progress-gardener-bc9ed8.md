...
at: 2026-07-12T16:52:10Z
---
# SturdyRef press tick (2026-07-12T16:50 dispatch, job endo-sturdyref-press-20260712-165003)

**Headline: REST TICK — no drift since the 15:52Z tick. Bar 1 (the bridge
stack) rests green and DRAFT; bar 2 (agent provide/accept, design #695)
remains maintainer-gated; the 21:00Z one-nudge window opens in ~4h.**

**Verified this tick (real execution, 16:50–16:55Z):**
...
- `gh pr view 695 --json comments,reviews` → **zero comments, zero
  ...
  #695 go/no-go) is **still unread**. Now ~16:52Z — the 21:00Z nudge
  window is not open; did not nudge. The ~21:35Z-or-later dispatch is
  the first eligible nudger.
- Peer check: `jobs/doin/` holds only unrelated jobs (#151/#714
  shepherds, #187, #234 review-retro, shepherd-producers-set-handler-
  timeout); no sturdyref peer on `inbox-list.sh`; my inbox drained empty.
- Stale-poison note (unchanged): the reaper-poisoned
  `endojs-endo-but-for-bots-pr704-shepherd` plan job remains moot while
  #704 is fully green; held for a human, not touched.
