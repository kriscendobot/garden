...
at: 2026-07-12T19:07:07Z
---
# SturdyRef press tick (2026-07-12T19:05 dispatch, job endo-sturdyref-press-20260712-190502)

**Headline: REST TICK — no drift since the 17:52Z tick. Bar 1 (the bridge
stack) rests green and DRAFT; bar 2 (agent provide/accept, design #695)
remains maintainer-gated; the 21:00Z one-nudge window opens in ~2h.**

**Verified this tick (real execution, 19:05–19:09Z):**
...
- `gh pr view 695 --json comments,reviews` → **zero comments, zero
  ...
  #695 go/no-go) is **still unread**. Now ~19:09Z — the 21:00Z nudge
  window is not open; did not nudge. The ~21:05Z-or-later dispatch is
  the first eligible nudger.
- Peer check: no sturdyref peer on `inbox-list.sh` (only unrelated
  esheets/xs2rust/self-heal jobs, the pr694 gauntlet, and the liaison);
  `jobs/doin/` empty; my inbox drained empty.
...
3. If the gate is still unread after **2026-07-12T21:00Z**, nudge via
   ...
   open window sends it and records it here). The ~21:05Z dispatch is
