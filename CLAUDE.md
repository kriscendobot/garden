...
| **deslop #N** | post a [deslopper](roles/deslopper/AGENT.md) job (a `myrmidon`-tier fixer variant) that rewrites flagged Botese (AI-slop cliché) phrases — "load-bearing invariant", "seam where", and kin — into plain prose, running the deterministic apply-then-re-grep loop to a fixpoint. Search-gated: the triager runs `thesaurus-cliche-grep.sh` first and posts the job **only** on a grep hit ([botese-normalization](skills/botese-normalization/SKILL.md)). The dual of *americanize*: the [thesaurus](roles/jurors/thesaurus/AGENT.md) jury seat detects, the deslopper fixes. |
...
| **ferry #N** | carry approved work upstream under the maintainer's identity. Dispatched OFF the generic board: the liaison stages a `journal/jobs/ferry/<name>.md` directive (never `post-job.sh`), and the maintainer runs `scripts/ferry.sh` on the credentialed host to execute it. Authorization required (§ The ferry). |
...
Ferrying — carrying an approved PR from the fork upstream — lands commits under the **maintainer's** identity, so it is a separate, permissioned surface with its own **dedicated dispatch**, deliberately OFF the generic job board ([designs/dedicated-ferry-dispatch.md](designs/dedicated-ferry-dispatch.md), job `dedicated-ferry-dispatch`). The liaison stages a ferry by writing a `journal/jobs/ferry/<name>.md` directive (never `post-job.sh`, never a subagent), and the maintainer runs **`scripts/ferry.sh`** — a host-native loop, on the host that holds the maintainer's (`kriskowal`) credentials, **outside the container**, using the operator's own ambient git/gh session — which dispatches `claude -p` wearing the boatman role against each directive. The directive must carry `identity_switch_authorized: true`, a flag no agent may originate — only the maintainer. The boatman is **locked out of the ordinary board mechanically**: `post-job.sh`, `claim-job.sh`, and `gardener.sh` all refuse a `role: boatman` job loudly, so a ferry can never be race-claimed by an arbitrary gardener and pushed under the bot. Inside the container the fleet's `gh` wrapper pins every call to the bot identity; `scripts/ferry.sh` sidesteps that pin entirely by running host-native (no wrapper on PATH, no `GARDEN_GH_IDENTITY` override needed — ambient `gh` already acts as the maintainer). The dispatched boatman still re-verifies host preconditions (`gh auth status` shows the human, push permission) and blocks rather than pushing as the bot if they are missing. See `roles/boatman/AGENT.md` § Dispatch, README § The ferry, and [designs/fleet-gh-identity.md](designs/fleet-gh-identity.md). Landing kriskowal credentials on a bot host to widen its blast radius is a separate, security-weighted decision.
...
- **No PR workflows for the garden's own repo.** The garden is a meta library, not application code. Both `main2` (development) and `journal2` are pushed directly to `origin` (`github.com/kriscendobot/garden`; transferred from `kriskowal/garden` on 2026-07-28, and the old path still redirects); we do not generally open pull requests against ourselves. PR workflows are reserved for fork worktrees of *other* repos, where the [boatman](roles/boatman/AGENT.md) ferries work upstream.
  - **Carve-out — a design carrying open questions gets a review PR.** The one exception: a design landed on this repo whose content carries a non-empty `## Open questions` section — *unresolved, maintainer-facing* questions the maintainer must decide, a real design fork and not settled documentation — is presented as a PR so inline review can happen, not landed bare. The mechanics reuse [frozen-base-branch](skills/frozen-base-branch/SKILL.md): snapshot the commit immediately before the design lands as the base, put the design commit(s) on a head branch, open the PR. The designer decides bare-vs-PR at design-completion time (it checks for open questions before landing), never retroactively. A design with an *empty or absent* open-questions section still lands bare, direct-to-`main2`, as most garden designs do. The designer marks such a PR with `<!-- garden-design-open-questions -->` in the PR body so the completion machinery does **not** stage a design panel for it (the content is already on `main2`; the PR is a maintainer answer-surface, not a pending merge). Full statement and mechanics: [`roles/designer/AGENT.md`](roles/designer/AGENT.md) § Operating norms. Grounding: `design-muse-worker-kind` landed a `designs/muse-worker-kind.md` (commit `d2a970ae8b`) with six undecided open questions and no review surface, forcing a hand-made retroactive PR ([#74](https://github.com/kriscendobot/garden/pull/74)) — that PR is what caught the design and let the maintainer reject it; the design was subsequently **dropped outright** per the #74 review (the file no longer lives on `main2`), but the incident remains the reason this carve-out exists.
...
**The latency watch does not widen surveillance.** `scripts/jobs/comment-latency-watch.sh` replays the same deterministic should-ack predicate over only the already-armed comment, issue-inbox, and trusted-mention sources. It never feeds text to an LLM and never posts a reaction: GitHub's `eyes` reaction `created_at` is the authoritative acknowledgment timestamp. Its only writes are reconstructible host-local cache/heartbeat files, the host-shared gh-api cooldown latch (armed for the primary-quota window when a source reports GitHub primary-quota exhaustion, and honored before every sweep), and coalesced `watchdog-notice.sh` alerts. Adding the checker therefore does not authorize a new repository or sender; every source retains the gates above.
