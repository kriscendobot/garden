...
Each driver is a bash process bound to one PR-creation lane on one host.
...
The driver script that owns these files is [`<garden-root>/roles/driver/driver.sh`](../<garden-root>/roles/driver/driver.sh); its role file is [`<garden-root>/roles/driver/AGENT.md`](../<garden-root>/roles/driver/AGENT.md).
...
- `<lane>` is the small positive integer the supervisor passed as the driver's positional argument (lanes 1, 2, 3, ... up to the host's cap).
...
```yaml
---
lane: 1                                  # positive integer; the supervisor's handle
host: kmkmbp2021                         # hostname -s of the running driver's host
pr: kriskowal/garden#3                   # owner/name#N when bound; otherwise null
design: designs/driver.md                # design doc path (garden-relative) or null
state: panel                             # the state-machine state from designs/driver.md
...
- `lane`: matches the positional argument the driver was launched with.
- `host`: identifies which host's supervisor owns the lane; cross-host coordination is the supervisor's job.
- `pr`: `owner/name#N`. Null when the lane is unbound (idle, waiting for the supervisor to bind it).
- `design`: the design document the lane's PR descends from, relative to the garden root. Null for source-touching PRs without a design (the design-only flow always has one).
- `state`: the most recent state the driver's state-machine ticked through. Names match `designs/driver.md` § The driver state machine.
- `role_awaited`: when the state has dispatched a worker via the role-specific job board, the role of the worker. Null when no worker is in flight (idle, between transitions, terminal states).
...
- The driver script (`<garden-root>/roles/driver/driver.sh`) rewrites the file on every state transition.
