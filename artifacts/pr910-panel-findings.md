...
- **Disposition:** **fixed** (`commit c5e8ea35f`). `hashBytes` no longer casts `undefined` past tsc; it now `throw`s a located `@endo/errors` `Fail` naming the missing power (`range().getInfo() requires a makeSha256 power … thread cryptoPowers.makeSha256 through makeMount/makeRevocableMount`) instead of surfacing a bare `TypeError: makeSha256 is not a function`. The type-laundering cast (`/** @type {() => Sha256} */ (makeSha256)()`) is removed and the value is narrowed by the guard. `filePowers` was considered as a default but its `sha256(path)` hashes a whole file *by path*, not a byte digester, so it cannot back `hashBytes(selectedBytes)`; a construction-time hard-require was rejected because ~25 `makeMount`/`makeRevocableMount` sites legitimately never call `getInfo` and would otherwise fail at construction. Two mount tests added (`packages/daemon/test/mount.test.js`): one threads `makeCryptoPowers(crypto).makeSha256` and asserts a derived `range(6n,11n).getInfo()` returns `{algorithm:'sha256', size:5n, hash:<base64 sha256('world')>}`; one asserts a mount built *without* the power fails `range().getInfo()` with a `/makeSha256/` message rather than a bare TypeError. tsc + eslint + the mount/conformance/help suites pass.
...
- **Disposition:** **fixed** (`commit c5e8ea35f`). `generate-help-text-data.mjs` now resolves the repo's Prettier config (`prettier.resolveConfig(outPath)`) and runs `prettier.format(...)` over the assembled source before `writeFileSync`, so the committed artifact matches CI's `prettier --check` and a regeneration is idempotent instead of re-breaking lint. Verified by regenerating `help-text-data.js` and confirming `prettier --check` passes on the output (the diff is now only the 3 new DMN-03 methods, not a 854-line reformat churn).
...
- **Disposition:** **fixed** (`commit c5e8ea35f`). Added `## glob(pattern)`, `## grep(pattern, paths?, options?)`, and `## glorp(globPattern, grepPattern, options?)` sections to the `EndoMount` block of `packages/daemon/src/help.md` (documenting each from its live guard/impl semantics — glob's sort+cap, grep's optional glob-composable `paths` and `maxResults`, glorp's required-positional fused form), then regenerated `help-text-data.js` with the DMN-02-fixed generator so `E(mount).help('glob'|'grep'|'glorp')` returns text again. The `help-text.test.js` / `helpdown.test.js` suites pass.
...
- **Disposition:** fixed — commit `2179eb2b9`. Added `.changeset/readableblob-range-attenuation.md` exactly per the releaser's spec: `@endo/platform` minor, `@endo/daemon` major, `@endo/git`/`@endo/exo-git`/`@endo/agent-tools`/`@endo/daemon-cas` patch, with the `fetch`→`range` (length→end semantics) / `rangeReadText`→`textRange` / `rangeRead`→`range(offset, offset+length)` migration note and the `streamBase64`-on-`BlobRefInterface` addition called out. Validated with `@changesets/parse` (repo-wide `changeset status` is blocked by a pre-existing unrelated broken changeset, `.changeset/lucky-planes-resolve.md`, missing frontmatter — out of slice).
...
- **Disposition:** fixed — commit `2179eb2b9`. Revised `.changeset/readable-blob-declarations.md` in place (not folded, since it carries a distinct concern — keeping the host-side CAS `readRange` helper out of generated Git code-mode types — and bumps `@endo/agentry`, which the new entry does not). The stale "`getInfo` and streaming `fetch` … while `rangeRead` and `rangeReadText` remain" prose now reads: git blob declarations expose `getInfo` plus the `range`/`textRange` attenuation surface, per readableblob-range-attenuation.md.
...
- **Disposition:** fixed — commit `2179eb2b9`. `packages/daemon-cas/README.md:39-41` now reads "`ContentStoreBlob.readRange` is an internal safe-number helper that backs the public bigint `RichReadableBlob.range()` attenuation at the daemon boundary", mirroring the `packages/platform/src/fs/types.ts:133` fix. (The `fetch(sha256)` store-fetch references elsewhere in the README are the unrelated `ContentStore.fetch`, not the retired range method — left intact.)
...
- **Disposition:** fixed — commit `2179eb2b9`. `packages/platform/src/fs/extended/DESIGN.md`: the §4.10 batching example (`E(blob).fetch(0n, expectedSize)` → `E(blob).range(0n, expectedSize)`), the §6 normative bullet (`fetch(offset, length)` → `range(start, end)` / `textRange`, with a supersession note that it returns a further readable blob — half-open `end`, not `length` — and whole-value streaming is now `streamBase64`), and the §7 `BlobRef.fetch` reference all updated. The line-count references in the panel (555/559/618/652) drifted with the rebase; located and fixed by content.
...
- **Disposition:** fixed — commit `2179eb2b9`, all four docs. `fs-interface-reconciliation.md`: the "post-consolidation" *shipped* catalog rows now name `rangeAttenuationMethodGuards` / `RichReadableBlobInterface` and list `getInfo`/`range`/`textRange` for `EndoMountFile`, `EndoBlob`, and the `EndoReadable` adjacent row (the `endo-fs` Surface-3 sketch's `BlobRef.fetch` — lines 1265/1283/456 — left intact, matching the finding's precise scope, as that is a genuinely separate aspirational cap-FS design). `fs-interface-consolidation.md:428`: the missed `EndoMountReadableBlob` checklist item now reads `getInfo`/`range`/`textRange` with the supersession note (its neighbours were already updated by this PR). `agentry-git-eval-scenarios.md`: rewrote the "ReadableBlob byte and line windows" **spec** section a future builder targets — `fetch`/`rangeRead`/`rangeReadText` → `range`/`textRange` (with the return-shape change: a further readable blob, not a materialized `Uint8Array`/`string`), updated the propagation follow-up + open-question, and added a 2026-08-06 revision-log entry rather than editing the historical 2026-07-17 directive quote. `designs/README.md`: the agentry-eval summary-table row and the "now names" changelog sentence → `range`/`textRange`. **Left intact by design:** dated past-tense changelog narrative (`README.md:422`, recording the 2026-06-18 consolidation state accurately) and the endo-fs cap-FS sketch — per the integrator's own "specify, not merely mention" bar; rewriting a dated record would falsify history.
