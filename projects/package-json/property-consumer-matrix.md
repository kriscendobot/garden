...
Rows grounded in ingested library sources: the Node runtime column traces to [`../../library/sources/node--doc-api-packages.md`](../../library/sources/node--doc-api-packages.md) (`cc37ad5`); the npm column and the property schema trace to [`../../library/sources/npm--configuring-npm-package-json.md`](../../library/sources/npm--configuring-npm-package-json.md) (`ce7681f`); the Endo column traces to the compartment-mapper README (`endojs/endo` `packages/compartment-mapper/README.md` at `ca77052`, read directly). Columns for tools not yet library-ingested (bundlers, Yarn Berry / pnpm / Bun, TypeScript, Deno) are synthesis from their public documentation, flagged **(synthesis)** and queued for library-backing by the follow-on jobs in [the project README](README.md).
...
| `types` / `typings` | TypeScript **(synthesis)** | TS's legacy type-entry field; superseded by the `types` export condition for `exports`-based packages. Ignored by runtimes/PMs. |
| `typesVersions` | TypeScript **(synthesis)** | Maps TS compiler-version ranges to type-file layouts; pre-`exports` mechanism. |
| `exports` | Node runtime; modern bundlers; TypeScript (`node16`/`nodenext`/`bundler`); Endo; Deno **(partial)** | The central field. Encapsulates unlisted subpaths. Condition resolution order differs across consumers (see inconsistencies). Endo supports only `import`/`browser`/`endo` conditions and no `*` patterns yet. |
...
### Bun (PM + runtime) (synthesis)

As a PM, reads npm `dependencies`/`overrides` and Yarn `resolutions`, `workspaces`, `trustedDependencies` (a Bun-specific field gating lifecycle scripts). As a runtime, honors `exports`/`imports`/`type`/`main` and the `bun` export condition.

### Deno (runtime) (synthesis)

Uses its own `deno.json`/`deno.jsonc` for imports maps and config, but for npm packages (via `npm:` specifiers) it honors `exports`/`main`/`type` and resolves conditions including `deno`, `import`, `node`, `default`. Has historically diverged on some `exports` edge cases.

### Endo `@endo/compartment-mapper` (runtime)

Reads `name`, `type`, `main`, `exports`, `browser`, `dependencies`, `files` (source: compartment-mapper README at `ca77052`). Only three conditions are supported: `import` (prefer ESM over CJS), `browser` (also swaps in the `browser` property over `main`), and `endo` (signals the tool is in use). If no `exports` applies to `"."`, `main` is the default. `imports` and `*` patterns in `exports`/`imports` are documented as future TODOs. A `parsers` property overrides extension-to-language mapping per package.

### TypeScript (compiler) (synthesis)

Reads `types`/`typings`, `typesVersions`, and (under `moduleResolution` `node16`/`nodenext`/`bundler`) the `types` condition within `exports`, plus `type` to decide the module format of emitted/consumed `.ts`. `bundler` resolution mirrors what bundlers do (honors `exports` conditions, allows extensionless imports); `node16`/`nodenext` mirror Node's strict ESM resolution (mandatory extensions, dual `import`/`require` type resolution).
