...
- **This is the bot's own repo: no PR ceremony by default.** Designs and builds land as direct commits to `main` (`git push origin HEAD:main`), matching the existing history (`stripe-credits`, `account-creation-open-signup`, `mcp-endo-guest` all landed this way). PRs appear only when a change wants review before deploy (reference: PR #3, an oauth2-proxy config fix).
