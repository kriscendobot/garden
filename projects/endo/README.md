...
- [cap-talk-capability-provenance.md](cap-talk-capability-provenance.md): what the cap-talk mailing-list archive (founded 1998 by Jonathan Shapiro of EROS) says that bears on Endo's design: the destroyable-indirection revocation primitive as the caretaker/[revocation-by-withdrawal](../../library/concepts/revocation-by-withdrawal.md) ancestor; "only connectivity begets connectivity" and objects-are-allocated-not-created as antecedents of Endo's reachability/storage-accounting model; designation-not-identity and the "syscall-gating is not an ocap" hygiene. Cross-links the library `cap-talk-1998` sections. Scholar job `scholar-ingest-cap-talk` (2026-09-16).
