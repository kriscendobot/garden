...
- [cap-talk-capability-provenance.md](cap-talk-capability-provenance.md): what the cap-talk archive says that bears on Endo's design, from caretaker revocation, connectivity discipline, and resource accounting through facets, enforced dependency injection, deep attenuation, CapTP/sturdyref boundaries, browser brokers, powerboxes, and durable-session failure. Cross-links the era indexes from `cap-talk-1998` through the complete `cap-talk-2004-2008` monthly index and the Miller/Close paper sections. Scholar jobs `scholar-ingest-cap-talk*` (2026-09-16).
