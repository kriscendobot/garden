---
title: The readonly-pattern motivating example; the *low-integrity-value* / *low value* concept adapted from Abadi 1999's security-protocol verification; the HLA higher-order concurrent imperative language with assert expressions + goodness bit; the OCPL program logic with high/low locations and progressive/non-progressive Hoare triples; the lift Ψ logical relation lifting location-predicates to value-predicates; the AdequacySafety and RobustSafety metatheorems (RobustSafety guarantees verified code linked with untrusted adversarial context remains assertion-safe)
