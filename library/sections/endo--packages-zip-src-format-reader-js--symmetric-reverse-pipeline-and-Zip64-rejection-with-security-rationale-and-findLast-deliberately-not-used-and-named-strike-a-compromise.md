---
title: "@endo/zip/src/format-reader.js — symmetric reverse pipeline (recordToFile → decompressFile → decodeFile reverses cycle 294's encodeFile → compress → makeRecord); Zip64 rejection with named security rationale; findLast deliberately not used despite being available; the named-strike-a-compromise for Windows-vs-spec filename quirk; six named integrity checks"
...
# `@endo/zip/src/format-reader.js` (full file)

A 479-line file implementing the symmetric reverse counterpart to cycle 294's `format-writer.js` (264 lines). **The cluster's largest file** at ~1.8× the writer's line count — once again, the parser-task-asymmetry (§the-named-task-asymmetry-shape from cycle 292) shows up at the format level too. The 12 of 12 zip cluster source-file deep ingests complete with this cycle (the remaining `compression.js` is a 4-line constant declaration trivially covered by cycle 191's cluster-scope ingest).
...
- **§the-`/* eslint no-bitwise: ["off"] */` + `max-lines: ["off"]`** — extends cycle 286's named-ESLint-directive count (now §six-cycles-with-named-eslint-directive-as-acknowledged-exception or similar).
- **§the-symmetric-pair-with-mirror-naming** (cycle 292's observation reaffirmed; now extended from the buffer-reader/buffer-writer pair to the format-reader/format-writer pair).
- **§the-`q = JSON.stringify`-alias** (cycle 292 observation reaffirmed; **§two-cycles-with-`q`-alias-in-error-messages-in-the-zip-cluster**: 292 buffer-reader + 296 format-reader).
...
Slot machine library `@game/replay/src/format-reader.js`: symmetric-reverse-pipeline (recordToFile → decompressFile → decodeFile reverses the writer's encode → compress → makeRecord); inverse bit-extraction with `>> N & 0xMM` per field; named-six-named-format-extension-trigger-fields with named-rejection of the extension format ("Cannot read GameV2"); explicit-comment-against-the-spec-prescribed-method with named-security-rationale ("undesirable, attackable ambiguities"); the-named-tool-IS-available-but-deliberately-not-used discipline; named-organization-attribution in the security comment ("Acme is not comfortable supporting"); named platform-deviation-in-the-wild (the named bug class); named-CVE-style-link as named-vulnerability-anchor; the-named-strike-a-compromise discipline with bounded tolerance; MAX_VALUE_NBITS named constants in decimal; module-scope TextDecoder singleton; named-locale-warning-comment; named-internal-closure-helper for context-rich-error-messages; named six-named-integrity-checks; named-feature-detection-via-bit-flag with named-feature-rejection; named-incomplete-directory-support TODO; named `max-lines: ["off"]` acknowledged-deviation; named four-typedef-block at the top; named-canonical-`@import`-form (vs inline `import(...)` from cycle 284).
