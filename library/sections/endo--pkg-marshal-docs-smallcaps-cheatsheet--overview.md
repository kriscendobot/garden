...
> Abstract: Quick-reference for the smallcaps wire format. Maps JS value categories to their smallcaps encodings: primitives, undefined/NaN/Infinity, BigInt, Symbol (well-known and registered), Error, capability references (slots), and the container types. A one-screen lookup card for anyone reading or writing smallcaps-encoded data.
