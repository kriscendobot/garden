---
title: "garden/scripts/watcher/endo-but-for-bots/watcher.sh — ninth garden source; §nine-cycles-with-garden-repo-source-ingest; §nine-named-shapes-of-garden-self-documentation; §the-named-per-feed-watcher-stub-shape (the ninth named shape); §the-named-Phase-1-stub-with-named-deferred-Phase-2-5; §the-named-five-step-contract-the-stub-documents (poll + classify + route + reactji + self-heal); §six-named-event-types-the-watcher-classifies (push + review + comment + label + assigned-issue + CI status); §the-named-eyes-reactji-discipline (deterministic + before-routing); §the-named-end-to-end-exercise-before-implementation-discipline; §three-cycles-with-named-script-location-discovery-shapes (298 single-level + 300 decomposed + 304 three-deep); §two-cycles-with-`set -uo pipefail`-WITHOUT-`-e` (300 + 304); §the-named-parameterized-prefix-shape (`watcher[<slug>]:`); §the-named-quadruple-claim-and-single-realization (cycles 297 + 299 + 300 + 301 named the watcher concept; cycle 304 IS the implementation stub)"
...
# `garden/scripts/watcher/endo-but-for-bots/watcher.sh` (ninth garden source ingest)

A 60-line bash script — **the ninth garden source ingested**. **§nine-cycles-with-garden-repo-source-ingest** (281 designs/driver.md + 297 WORKTREES.md + 298 dispatch-prepare/teardown pair + 299 CLAUDE.md + 300 daemons start/stop/config-example + 301 roles/COMMON.md + 302 skills/library-lookup/SKILL.md + 303 roles/liaison/AGENT.md + 304 scripts/watcher/endo-but-for-bots/watcher.sh). **§nine-named-shapes-of-garden-self-documentation** (proposed-design + standing-reference + implementation-source + project-instructions + operational-daemon-control + standing-subagent-instructions + skill-procedural-playbook + role-specific-orchestrator-instructions + per-feed-watcher-stub).

**§the-named-quadruple-claim-and-single-realization**: four prior cycles named the watcher concept (cycle 297 WORKTREES.md named the standing-monitor exception; cycle 299 CLAUDE.md named the monitoring safety constraint mentioning endojs/endo-but-for-bots; cycle 300 daemons config.sh.example listed `GARDEN_WATCHER_FEEDS=(endo-but-for-bots)`; cycle 301 COMMON.md named the standing-monitor exception again). Cycle 304 IS the actual implementation (stub) realizing those claims. **§the-named-multi-cycle-claim-converges-to-single-realization-cycle**.
...
- **§nine-cycles-with-garden-repo-source-ingest**: 281 + 297 + 298 + 299 + 300 + 301 + 302 + 303 + 304.
- **§nine-named-shapes-of-garden-self-documentation**: proposed-design + standing-reference + implementation-source + project-instructions + operational-daemon-control + standing-subagent-instructions + skill-procedural-playbook + role-specific-orchestrator-instructions + per-feed-watcher-stub.
- **§three-cycles-with-named-script-location-discovery-shapes**: 298 single-level + 300 decomposed + 304 three-deep.
- **§two-cycles-with-`set -uo pipefail`-WITHOUT-`-e`**: 300 + 304. **§the-named-two-named-strictness-tiers-by-script-cluster** (dispatch-scripts cluster with -e + daemons/watcher cluster without -e).
- **§three-named-prefix-shapes** across the garden's bash scripts: cycle 298 (no prefix) + cycle 300 (simple prefix `start:` / `stop:`) + cycle 304 (parameterized prefix `watcher[<slug>]:`).
- **§three-cycles-with-named-bash-parameter-expansion-discipline**: 298 (`${REPO%/*}` / `${REPO#*/}`) + 300 (`${1:-}` default-empty) + 304 (`${VAR:-default}` default substitution).
- **§two-cycles-with-named-double-dash-separator-discipline**: 297 (dispatches/<role>--<short-id>) + 304 (events/<repo>--<pr>.log).
- **§two-cycles-with-named-design-and-implementation-cross-reference-pair**: 281 designs/driver.md + 304 watcher pointing back at the design.
- **§the-named-quadruple-claim-and-single-realization**: cycles 297 + 299 + 300 + 301 named the watcher concept; cycle 304 IS the implementation stub.
- **§the-named-nine-cycle-bridge**: 296 + 297 + 298 + 299 + 300 + 301 + 302 + 303 + 304.
