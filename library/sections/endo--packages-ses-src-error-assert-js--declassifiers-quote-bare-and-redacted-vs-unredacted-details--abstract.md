...
The §file-header comment block (lines 1-12) opens with the same *no-special-privilege* axiom as cycle 96's `console.js`: *Subject to the conditions and limitations of [the Apache 2.0 license]...*. But unlike `console.js`, the §next lines (lines 9-12) carry a candid admission:
...
This is the §honest-mutable-state-with-narrow-exposure discipline. The §declassifiers WeakMap (lines 70-75) pairs an *opaque marker token* with the underlying value it intentionally exposes; the §quote function (lines 78-85) returns a frozen object whose `toString` invokes `bestEffortStringify(value)`; the §bare function (lines 92-102) returns its argument verbatim if it matches `canBeBare = /^[\w:-]( ?[\w:-])*$/` and otherwise falls back to `quote`. The §canBeBare regex is the *safe-as-prose* gate: a string matches if it contains only word characters, colons, or hyphens with optional single-space separators — the structural intent is *names that look like identifiers or short prose without metacharacters that could be confused with substitutions*. The §hiddenDetailsMap WeakMap (line 122) holds the parts of each details-token outside the token itself, so the token carries no own properties that could leak. The §DetailsTokenProto (lines 158-167) is the prototype of every details-token; its `toString` produces type-tag substitutions like `(a TypeError)` for non-declassified Error arguments. The §redactedDetails template tag (lines 182-189) is the canonical `X` / `details` tag: it captures the literal parts and substitution values, builds a frozen DetailsToken, and stores the parts in the hidden map. The §unredactedDetails template tag (lines 205-211) is the variant used by `errorTaming: 'unsafe'`: it wraps every substitution in `quote` so the substitution's underlying value survives into the rendered message instead of being redacted to a type-tag.
