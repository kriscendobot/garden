---
title: See Also
source: packages/exo/README.md
source_repo: endojs/endo
source_commit: 14a0b631832ff516b4cafa3946a4a3c0ccbcf052
source_date: 2026-01-04
source_authors: [Kris Kowal]
ingested: 2026-05-14
ingested_by: scholar
topics: [exo]
status: current
---

> Abstract: Pointers to packages/exo/docs/ for deeper documentation: exo-taxonomy.md and types.md cover the broader Exo design space and TypeScript surface respectively. These are queued for future ingestion.


- [Exo Taxonomy](./docs/exo-taxonomy.md) - Complete API reference and
  make/define/prepare patterns

Source: [packages/exo/README.md](https://github.com/endojs/endo/blob/14a0b631832ff516b4cafa3946a4a3c0ccbcf052/packages/exo/README.md) at commit `14a0b631`.
