...
| [capability-security](capability-security.md) | Object-capability discipline as practiced in Endo / Agoric; agent-runtime applications of capability discipline. | 179 |
...
| [content-addressed-storage](content-addressed-storage.md) | Storing data as content-addressed Merkle trees of fixed-size 1KB blocks (kriskowal/cask): block format, blobs/dirs/arrays/maps as block trees, content-agnostic GC (the pinned/deadline retention regimes, snapshot+quarantine concurrent GC, store-wrapper mark/sweep, and caskdbstore's WAL-quarantine GC), cells, reducer-style persistent structures, schema hashes, the caskdir directory format (v1 compact, v2 Rabin-chunked/table, the benchmark-driven adaptive hybrid), and the array/columnar machinery (caskarray + operational transform, the Rabin-chunked sortedarray with SDIF/SOPS sync, the swap-to-end allocator + hashtreetouint64 + indexheap + sessiontable, the adaptive-width BigIntArray, the membership member table under the caskhead root). | 65 |
| [networking](networking.md) | Transport-layer design (kriskowal/cask): UDP block transfer reacting against TCP's limits, the layered casknet protocol stack, Noise-IK/PSK encryption (with the cryptography.md design proposal behind it), 256-bit priority load shedding, casktel telemetry; the current plaintext casksock local protocol and the never-implemented v2 UDP proposal; the SDIF/SOPS sorted-array diff-and-sync verbs; the membership-gating member table, the three-gate (membership/session/capability) access model, and the deferred cluster-provisioning design. | 36 |
