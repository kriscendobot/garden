...
| [testing](testing.md) | How to run and write tests in endo. | 19 |
...
| [data-structures](data-structures.md) | General-purpose data structures and the interfaces over them: kriskowal/collections (idiomatic uniform JS collection interfaces) and cask's columnar parallel-array pattern (deepened from cask's parallel-arrays design doc). Cross-cutting, independent of the capability-security corpus. | 23 |
| [reactive-bindings](reactive-bindings.md) | Synchronous, incremental binding of object properties and collection contents (kriskowal/frb): one- and two-way bindings, incremental query chains, the small binding query language. | 21 |
| [content-addressed-storage](content-addressed-storage.md) | Storing data as content-addressed Merkle trees of fixed-size 1KB blocks (kriskowal/cask): block format, blobs/dirs/arrays/maps as block trees, content-agnostic GC, cells, reducer-style persistent structures, schema hashes. | 15 |
| [networking](networking.md) | Transport-layer design (kriskowal/cask): UDP block transfer reacting against TCP's limits, the layered casknet protocol stack, Noise-IK/PSK encryption, 256-bit priority load shedding, casktel telemetry. | 14 |
