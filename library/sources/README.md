...
| [designs/chat-spaces-home.md](endo-but-for-bots--llm-designs-chat-spaces-home.md) | endojs/endo-but-for-bots (llm) | 2026-03-02 | Kris Kowal | 2 | current (Complete; configurable Space 0 / Home; 4 indelible invariants + 2 configurable fields; `HOME_SPACE_DEFAULTS` merge-on-load + normalize-on-save; `data-menu-scope` attribute system; `showName`-parameterized modal reuse; shared icon-selector extraction; **corrects** chat-spaces-gutter's keyboard numbering — Cmd+N → position N, home at 0) |
