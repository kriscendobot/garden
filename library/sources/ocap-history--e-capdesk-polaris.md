---
source_kind: web-survey
source_url: https://erights.org/elang/index.html
source_date: 2026-06-11
ingested: 2026-06-11
ingested_by: scholar
section_count: 1
status: current
notes: |
  Survey synthesized from the library's already-ingested Miller papers
  (papers--miller-tribble-shapiro-concurrency-among-strangers-2005--history-and-related-work,
  papers--miller-tulloh-shapiro-structure-of-authority-2004--multiplicative-pola-and-security-as-modularity,
  papers--swasey-garg-dreyer-ocpl-2017--related-work-iris-foundation-and-future-firefox-membrane),
  Wikipedia on E programming language (en.wikipedia.org/wiki/E_(programming_language)),
  and the Waterken project page (waterken.sourceforge.net). erights.org itself was
  unreachable (ECONNREFUSED) at retrieval time. HPL technical reports (HPL-2004-116,
  HPL-2006-116) were also unreachable; their contents are accessible via the Miller
  papers' citations.

  2026-06-27 update: the primary erights.org E-language page at this source_url is
  now reachable through the erights.github.io GitHub Pages mirror and has been
  ingested as its own source, [erights--elang-index](erights--elang-index.md) (Mark
  Miller's documentation index). This survey is retained, unchanged, as the
  secondary-source MARKET-HISTORY synthesis (adoption narrative for the B2 essay);
  it is materially distinct from the primary documentation index. The notes above
  remain an accurate record of how this survey was synthesized — it was never
  fetched from erights.org; source_url is the canonical pointer to the page the
  survey is about.
---

## Abstract

Survey of the E programming language, CapDesk, and Polaris as the canonical first wave of working object-capability implementations, with a focus on what each demonstrated technically and why none achieved commercial adoption. Intended as the factual anchor for the B2 essay's "thirty-year history of technical success and market failure" objection, grounded in named sources rather than unsourced assertions.

| Section | Topics | Status |
|---------|--------|--------|
| [e-capdesk-polaris-market-history](../sections/ocap-history--e-capdesk-polaris-market-history.md) | capability-theory, capability-security | current |

## See also

- [erights--elang-index](erights--elang-index.md) — the **primary** erights.org E-language documentation index (Mark S. Miller), fetched via the erights.github.io mirror. This survey is the secondary-source market-history companion to that primary page.
- [papers--stiegler-polaris-virus-safe-computing-2004](papers--stiegler-polaris-virus-safe-computing-2004.md) — the **primary** HP Labs technical report (HPL-2004-221) behind this survey's Polaris narrative, ingested 2026-06-28. This survey remains the secondary-source market/adoption synthesis; the primary report supplies the technical mechanism (designation=authorization, PowerBox, Pets in restricted accounts, the permission-vs-authority distinction).
- [combex--tech-index](combex--tech-index.md), [combex--edesk](combex--edesk.md), [combex--darpa-browser](combex--darpa-browser.md), [combex--opportunity](combex--opportunity.md), and [papers--stiegler-karp-yee-miller-polaris-2004](papers--stiegler-karp-yee-miller-polaris-2004.md) — the **primary** CapDesk / Polaris source cluster (fetched 2026-06-28 via the Internet Archive, `source_fetched_via=wayback`), of which this survey is the secondary-source market-history companion (job `scholar-ingest-source-combex-capdesk-polaris-primaries`).
