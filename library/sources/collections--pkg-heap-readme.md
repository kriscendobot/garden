---
source: packages/heap/README.md
source_repo: kriskowal/collections
source_commit: 4688abadf04b3bda247c61bc64ad38e2d3363809
source_date: 2020-11-06
source_authors: [Kris Kowal]
ingested: 2026-06-24
ingested_by: scholar
section_count: 1
status: current
---

> Abstract: README for `@collections/heap`, a binary max-heap laid out in an array giving logarithmic add/remove and fast access to the highest value. Invert the comparator for a min heap.

| Section | Topics | Status |
|---------|--------|--------|
| [overview](../sections/collections--pkg-heap-readme--overview.md) | data-structures | current |

Source: [packages/heap/README.md](https://github.com/kriskowal/collections/blob/4688abadf04b3bda247c61bc64ad38e2d3363809/packages/heap/README.md) at commit `4688abad`.
