---
source: packages/lfu-map/README.md
source_repo: kriskowal/collections
source_commit: 4688abadf04b3bda247c61bc64ad38e2d3363809
source_date: 2020-09-26
source_authors: [Kris Kowal]
ingested: 2026-06-24
ingested_by: scholar
section_count: 1
status: current
---

> Abstract: README for `@collections/lfu-map`, a bounded Map backed by an LfuSet that drops the least frequently used entry. The map-shaped member of the frequency-eviction cache family.

| Section | Topics | Status |
|---------|--------|--------|
| [overview](../sections/collections--pkg-lfu-map-readme--overview.md) | data-structures | current |

Source: [packages/lfu-map/README.md](https://github.com/kriskowal/collections/blob/4688abadf04b3bda247c61bc64ad38e2d3363809/packages/lfu-map/README.md) at commit `4688abad`.
