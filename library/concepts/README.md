...
- [content-addressed-block-store](content-addressed-block-store.md) — `draft`; a store where every unit is a fixed-size block named by its content hash, larger values are Merkle trees of blocks; kriskowal/cask's 1KB block unifies storage and transport and makes GC content-agnostic (added 2026-06-24 by the future-forks begin-ingest).
- [merkle-tree-of-blocks](merkle-tree-of-blocks.md) — `draft`; a tree whose nodes are content-addressed blocks holding child hashes; cask's blobs/dirs/arrays/maps/sets are all Merkle trees of 1KB blocks over the arraytree/hashtree backbones (added 2026-06-24).
- [functional-reactive-bindings](functional-reactive-bindings.md) — `draft`; kriskowal/frb: synchronous, incremental one- and two-way bindings of object properties and collection contents declared in a small query language; functional/generic/reactive/synchronous/incremental/unwrapped (added 2026-06-24).
...
- [parallel-arrays-columnar](parallel-arrays-columnar.md) — `draft`; ECS-style data layout: typed columns plus separate slot-index arrays so multiple orderings coexist over one dataset; cask's sendbuffer and adaptive-width hashtreetouint* tables (added 2026-06-24).
- [rabin-chunking](rabin-chunking.md) — `draft`; content-defined chunking via a Rabin-fingerprint rolling hash so mid-stream edits invalidate O(log n) blocks; cask's blob chunker (added 2026-06-24).
